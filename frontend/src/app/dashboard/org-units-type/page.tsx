...
import {
  ...
  Search, Settings, Trash2, X, ShieldCheck, AlertTriangle
} from "lucide-react";
...
export default function OrgUnitsTypePage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("⚠️ Confirmation de suppression définitive ? Le Kernel vérifiera l'absence de liens actifs dans le SMI.")) return;
    ...
    try {
      ...
      toast.error("Rupture : Des unités organiques dépendent encore de ce type.", { id: toastId });
    }
  };
    ...
    <div className="ml-72 min-h-screen bg-[#0B0F1A] text-white italic font-sans p-8 selection:bg-purple-500/30">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center h-96">
          ...
          <p className="text-[10px] font-black uppercase text-slate-500 tracking-[0.5em] animate-pulse">Synchronisation du mapping organique...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            >
              <div className="flex items-center gap-8 relative z-10">
                ...
                <div 
                  ...
                  style={{ backgroundColor: `${type.OUT_Color || '#a855f7'}15`, color: type.OUT_Color || '#a855f7' }}
                >
                ...
                <div className="text-left space-y-2">
                  <div className="flex items-center gap-4">
                    <h3 className="font-black uppercase italic text-2xl tracking-tighter text-white">{type.OUT_Label}</h3>
                  ...
                  <p className="text-[11px] text-slate-500 font-bold uppercase italic tracking-tight opacity-70 leading-relaxed max-w-sm">
                    {type.OUT_Description || "Périmètre de responsabilité non spécifié dans le registre SDE."}
                  </p>
                </div>
              </div>
          ...
          {!loading && filteredTypes.length === 0 && (
            <div className="col-span-full py-32 text-center border-4 border-dashed border-white/5 rounded-[4rem] opacity-20">
              ...
              <p className="font-black uppercase italic tracking-[0.3em]">Néant Organique Détecté</p>
            </div>
          )}
        </div>
      )}
...
function OrgUnitModal({ item, onClose, onSuccess }: { item: OrgUnitType | null; onClose: () => void; onSuccess: () => void; }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = error.response?.data?.message || "Erreur critique d'écriture Matrix.";
        ...
        <div className="flex items-center justify-between mb-12 relative z-10">
          <div className="flex items-center gap-6">
            ...
            <div>
              ...
              <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mt-2">Configuration Hiérarchique §5.3</p>
            </div>
          </div>
          <button onClick={onClose} className="p-2 text-slate-500 hover:text-white transition-colors cursor-pointer border-none bg-transparent">
            <X size={40} strokeWidth={1} />
          </button>
        </div>

        <form onSubmit={handleSubmit} className="space-y-10 italic font-bold relative z-10">
          
          {/* Libellé du Grade (OUT_Label) */}
          <div className="space-y-3">
            <label className="text-[10px] text-slate-500 uppercase tracking-[0.3em] ml-6">Désignation du Grade Organique *</label>
            <input 
              required 
          ...
          <div className="grid grid-cols-2 gap-10">
            ...
            <div className="space-y-3">
              ...
              <input 
                ...
                onChange={(e) => setForm({ ...form, OUT_Level: parseInt(e.target.value) || 1 })} 
              />
            </div>

            {/* Code Couleur (OUT_Color) */}
            <div className="space-y-3">
              <label className="text-[10px] text-slate-500 uppercase tracking-[0.3em] ml-6">Code Couleur Cartographique</label>
              <div className="flex gap-4">
          ...
          <div className="space-y-3">
            <label className="text-[10px] text-slate-500 uppercase tracking-[0.3em] ml-6">Périmètre de Responsabilité §5.3</label>
            <textarea 
              rows={4} 
          ...
          <div className="p-6 bg-amber-500/5 border border-amber-500/20 rounded-3xl flex items-start gap-4">
             ...
             <p className="text-[9px] font-bold text-amber-500/80 uppercase leading-relaxed italic">
               Note : L&apos;ID du Tenant est injecté automatiquement. Assurez-vous que les champs Level et Color sont présents dans votre schéma Prisma pour éviter les erreurs de persistance.
             </p>
          </div>
          ...
          >
            {saving ? <Loader2 className="animate-spin" /> : <><ShieldCheck size={22} /> Inscrire dans le Registre SMI</>}
          </button>
        </form>
      </div>
    </div>
