...
const StatusBadge = ({ status }: { status: string }) => {
  ...
  return (
    <span
      className={cn(
        "px-5 py-2 rounded-xl text-[9px] font-black border uppercase italic tracking-[0.2em] whitespace-nowrap",
        config[status] || config.BROUILLON,
      )}
    >
...
const MetricCard = ({
  ...
  const colors = {
    ...
    emerald:
      "text-emerald-500 bg-emerald-500/5 border-emerald-500/10 hover:border-emerald-500/40",
    amber:
      "text-amber-500 bg-amber-500/5 border-amber-500/10 hover:border-amber-500/40",
  ...
  return (
    <div
      className={cn(
        "p-10 rounded-[3rem] border transition-all duration-700 relative overflow-hidden group shadow-2xl backdrop-blur-md flex flex-col justify-between h-64",
        colors[color],
      )}
    >
      ...
      <div className="flex items-center gap-5 relative z-10">
        <div
          className={cn("p-4 rounded-2xl border shadow-inner", colors[color])}
        >
          <LucideIcon size={24} strokeWidth={2.5} />
        </div>
        <p className="text-[11px] text-slate-500 tracking-[0.4em] font-black uppercase italic leading-none">
          {title}
        </p>
      </div>
      <p className="text-7xl font-black text-white leading-none tracking-tighter relative z-10 italic">
        {val}
      </p>
    </div>
  );
...
export default function GEDPage() {
  ...
  const nextAutoRef = useMemo(
    () => `SAG-GED-${String(documents.length + 1).padStart(3, "0")}`,
    [documents],
  );
  ...
  const filteredDocs = useMemo(() => {
    return documents.filter((d) => {
      const matchSearch =
        (d.DOC_Title || "").toLowerCase().includes(search.toLowerCase()) ||
        (d.DOC_Reference || "").toLowerCase().includes(search.toLowerCase());
      const matchCat =
        categoryFilter === "ALL" || d.DOC_Category === categoryFilter;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    if (!selectedFile)
      return toast.error("CONTRÔLE §7.5.3 : FICHIER PHYSIQUE MANQUANT");
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "ERREUR DE RÉCEPTION MATRIX", {
        id: tid,
      });
    }
  };
    ...
    return (
      <div className="ml-72 flex h-screen flex-col items-center justify-center bg-[#0B0F1A] gap-8">
        <Loader2
          ...
          strokeWidth={1.5}
        />
        <p className="font-black italic uppercase tracking-[0.6em] text-blue-500 animate-pulse">
          Noyau GED Actif...
        </p>
      </div>
    );
      ...
      <header className="px-12 py-10 border-b border-white/5 flex justify-between items-end sticky top-0 bg-[#0B0F1A]/95 backdrop-blur-3xl z-40 shadow-2xl">
        <div className="space-y-4 text-left">
          <div className="flex items-center gap-4 text-blue-500 bg-blue-500/5 w-fit px-5 py-2 rounded-full border border-blue-500/10">
            ...
            <span className="text-[9px] font-black uppercase tracking-[0.3em]">
              Sovereign Documentation Protocol
            </span>
          </div>
        ...
        <div className="flex gap-8 items-center">
          <div className="relative w-125 group">
            <Search
              className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500 group-focus-within:text-blue-600 transition-colors"
              size={20}
            />
            <input
              ...
              <button
                ...
                className={cn(
                  ...
                  categoryFilter === cat
                    ...
                    : "bg-white/5 border-white/10 text-slate-500 hover:border-blue-600 hover:text-white",
                )}
              >
                {cat}
              </button>
            ...
            <tbody className="divide-y divide-white/5">
              {filteredDocs.length > 0 ? (
                filteredDocs.map((doc) => (
                  <tr
                    key={doc.DOC_Id}
                    className="hover:bg-blue-600/5 transition-all group animate-in fade-in duration-500"
                  >
                    <td className="p-12">
                      <div className="flex flex-col gap-3">
                        <div className="flex items-center gap-3">
                          <Hash size={12} className="text-blue-600" />
                          <span className="text-[11px] text-blue-500 font-black tracking-widest italic uppercase">
                            {doc.DOC_Reference}
                          </span>
                        </div>
                        <span className="text-xl text-white font-black italic tracking-tighter leading-none">
                          {doc.DOC_Title}
                        </span>
                    ...
                    <td className="p-12">
                      <div className="flex items-center gap-3 text-[11px] text-amber-500/80 font-black italic tracking-tight bg-amber-500/5 px-4 py-2 rounded-xl border border-amber-500/10 w-fit">
                        <Clock size={16} />
                        {doc.DOC_NextReviewDate
                          ? new Date(doc.DOC_NextReviewDate).toLocaleDateString(
                              "fr-FR",
                              {
                                year: "numeric",
                                month: "long",
                    ...
                    <td className="p-12">
                      <div className="flex items-baseline gap-2">
                        <span className="text-slate-600 text-[10px] font-black uppercase tracking-widest opacity-50 italic">
                          Version
                        </span>
                        <span className="font-black text-white text-3xl italic tracking-tighter leading-none">
                          {doc.DOC_CurrentVersion}
                        </span>
                      </div>
                    </td>
                    <td className="p-12">
                    ...
                    <td className="p-12 text-right px-16">
                      <div className="flex justify-end gap-4 opacity-0 group-hover:opacity-100 translate-x-10 group-hover:translate-x-0 transition-all duration-500">
                        <button className="p-5 bg-white/5 hover:bg-blue-600 transition-all rounded-2xl text-slate-400 hover:text-white border border-white/5 cursor-pointer shadow-xl">
                          <Eye size={22} />
                        </button>
                        <button className="p-5 bg-white/5 hover:bg-emerald-600 transition-all rounded-2xl text-slate-400 hover:text-white border border-white/5 cursor-pointer shadow-xl">
                          <Download size={22} />
                        </button>
                        <button className="p-5 bg-white/5 hover:bg-red-600 transition-all rounded-2xl text-slate-400 hover:text-white border border-white/5 cursor-pointer shadow-xl">
                          <Trash2 size={22} />
                        </button>
                      </div>
                    </td>
                ...
                <tr>
                  <td colSpan={6} className="p-48 text-center bg-slate-900/10">
                    <Archive
                      size={120}
                      className="mx-auto text-slate-900 mb-10 opacity-10 animate-pulse"
                    />
          ...
          >
            ...
            <div className="flex justify-between items-center border-b border-white/10 pb-12 relative z-10">
              <div className="flex items-center gap-8 text-left">
                <div className="p-6 bg-blue-600 rounded-3xl text-white shadow-2xl animate-pulse">
                  <ShieldCheck size={40} strokeWidth={2.5} />
                </div>
                <div>
                  <h2 className="text-5xl italic font-black uppercase tracking-tighter leading-none text-white">
                    Indexation <span className="text-blue-600">SMI</span>
                  </h2>
                  <p className="text-[11px] text-slate-500 uppercase tracking-[0.5em] mt-4 font-black">
                    ISO 9001 §7.5.3 • Protocole de Captage Numérique
                  </p>
                </div>
              </div>
              <button
                ...
                className="p-5 bg-white/5 hover:bg-red-600/20 text-slate-500 hover:text-red-500 transition-all rounded-3xl border-none cursor-pointer"
              >
            ...
            <div className="space-y-10 relative z-10">
              <div className="grid grid-cols-2 gap-12">
                <div className="space-y-4">
                  <label className="text-[12px] text-slate-500 ml-8 italic font-black uppercase tracking-[0.3em]">
                    Classification SDE *
                  </label>
                  <select
                    required
                    name="DOC_Category"
                    className="w-full bg-slate-900 border border-white/10 p-8 rounded-3xl text-[13px] text-white outline-none focus:border-blue-600 font-black uppercase italic cursor-pointer shadow-inner appearance-none"
                  >
                ...
                <div className="space-y-4">
                  ...
                  <div className="relative">
                    <input
                      name="DOC_Reference"
                      readOnly
                      className="w-full bg-blue-600/5 border border-blue-600/20 p-8 rounded-3xl text-[13px] text-blue-400 outline-none font-black uppercase italic tracking-[0.4em] shadow-inner"
                      value={nextAutoRef}
                    />
                    <Hash
                      className="absolute right-8 top-1/2 -translate-y-1/2 text-blue-600/30"
                      size={24}
                    />
                  </div>
                </div>
              ...
              <div className="space-y-4">
                <label className="text-[12px] text-slate-500 ml-8 italic font-black uppercase tracking-[0.3em]">
                  Désignation Officielle du Document *
                </label>
                <input
                  required
                  name="DOC_Title"
                  className="w-full bg-slate-900 border border-white/10 p-8 rounded-3xl text-[15px] text-white outline-none focus:border-blue-600 uppercase font-black italic tracking-tight shadow-inner"
                  placeholder="EX: PROTOCOLE DE MAÎTRISE DES RISQUES..."
                />
              </div>
              ...
              <div className="grid grid-cols-2 gap-12">
                ...
                <div className="space-y-4">
                  <label className="text-[12px] text-slate-500 ml-8 italic font-black uppercase tracking-[0.3em]">
                    Version d&apos;Indexation
                  </label>
                  <input
                    name="DOC_CurrentVersion"
                    defaultValue="1"
                    className="w-full bg-slate-900 border border-white/10 p-8 rounded-3xl text-[13px] text-white outline-none focus:border-blue-600 font-black italic uppercase shadow-inner text-center"
                  />
                </div>
              </div>
                ...
                <label
                  ...
                  className={cn(
                    ...
                    selectedFile
                      ...
                      : "border-white/5 hover:border-blue-600 hover:bg-blue-600/10",
                  )}
                >
                  <FileUp
                    size={80}
                    className={cn(
                      "mb-8 transition-transform group-hover:scale-110 duration-700",
                      selectedFile ? "text-emerald-500" : "text-blue-600",
                    )}
                  />
                  <p className="text-[14px] text-slate-200 font-black italic uppercase text-center tracking-[0.3em] px-16 leading-relaxed">
                    {selectedFile
                      ? `CAPTURE RÉUSSIE : ${selectedFile.name.toUpperCase()}`
                      : "DÉPOSEZ LE FICHIER PHYSIQUE (PDF/DOCX) POUR SCELLAGE SDE"}
                  </p>
                  <p className="text-[9px] text-slate-500 mt-6 font-black uppercase tracking-[0.5em]">
                    Taille max : 50MB • Format ISO standard
                  </p>
                </label>
                <input
                  id="ged-upload"
