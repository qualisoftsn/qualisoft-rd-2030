...
export default function SitesPage() {
  ...
  const fetchSites = useCallback(async () => {
    try {
      ...
      const msg =
        "Rupture de liaison avec le registre des sites. Vérifiez l'état du Kernel.";
      setErrorMessage(msg);
      toast.error(msg);
    } finally {
      setLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `Entité "${formData.S_Name}" scellée avec succès dans le SDE.`,
      );
      ...
      let message =
        "Une anomalie technique empêche l'enregistrement souverain.";
      ...
      if (err.response) {
        ...
        if (Array.isArray(backendMsg)) {
          ...
          message =
            "Privilèges insuffisants pour modifier le référentiel géographique.";
        }
      }
  ...
  const handleDelete = async (id: string, name: string) => {
    ...
    try {
      ...
      const msg =
        err.response?.data?.message || "Échec de l'opération de suppression.";
    ...
    return (
      <div className="flex h-screen flex-col items-center justify-center bg-slate-50 gap-4">
        <div className="relative">
          ...
          />
          <ShieldCheck
            className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-blue-600/30"
            size={20}
          />
        </div>
    ...
    <div className="p-8 space-y-8 bg-slate-50 min-h-screen italic font-sans selection:bg-blue-600/30">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 animate-in slide-in-from-top-4 duration-500">
        <div>
          ...
          <p className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.5em] mt-4 ml-1 opacity-70 flex items-center gap-2">
            ...
            Cartographie opérationnelle des pôles d&apos;activité • Périmètre
            SMI
          </p>
        </div>
      ...
      <div className="bg-blue-600/5 border border-blue-600/10 p-4 rounded-3xl flex items-center gap-4 animate-in fade-in duration-1000">
        ...
        <p className="text-[10px] font-bold text-blue-800 uppercase tracking-tighter leading-relaxed">
          [Image of organizational site hierarchy] - Cette cartographie assure
          la segmentation territoriale de vos processus et la traçabilité des
          audits multi-sites selon le référentiel ISO.
        </p>
      </div>
        ...
        <div className="lg:col-span-1">
          <div className="bg-white rounded-[2.5rem] p-8 shadow-2xl border border-slate-200 sticky top-8 animate-in slide-in-from-left-4 duration-700">
            <div className="mb-8 border-b border-slate-100 pb-5">
              ...
              <p className="text-[8px] text-slate-400 font-bold uppercase mt-2 tracking-widest">
                Indexation au SDE Matrix
              </p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                <div className="relative group">
                  <Building
                    className="absolute left-4 top-4 text-slate-300 group-focus-within:text-blue-600 transition-colors duration-300"
                    size={16}
                  />
                  <input
                    required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, S_City: e.target.value })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, S_Country: e.target.value })
                    }
                  />
                </div>
              </div>
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl border border-slate-200 overflow-hidden min-h-160 animate-in slide-in-from-right-4 duration-700">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-[8px] font-black text-slate-400 uppercase tracking-widest mt-1">
                    Sovereign Asset Core • Tenant ID Secured
                  </p>
                </div>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 p-8 gap-8">
              {sites.map((site) => (
                <div
                  key={site.S_Id}
                  className="group relative p-8 rounded-4xl border border-slate-100 bg-slate-50/40 hover:bg-white hover:border-blue-600/30 hover:shadow-[0_20px_50px_rgba(37,99,235,0.1)] transition-all duration-500 ease-out"
                >
                    ...
                    <div className="flex gap-6 items-start">
                      ...
                      <div className="text-left space-y-1">
                        <div className="flex items-center gap-3">
                          <h4 className="text-[15px] font-black uppercase italic text-slate-900 group-hover:text-blue-700 transition-colors tracking-tighter leading-none">
                            {site.S_Name}
                          </h4>
                          {!site.S_IsActive && (
                        ...
                        <p className="text-[11px] text-slate-500 mt-3 font-bold italic leading-relaxed max-w-64 opacity-80">
                          {site.S_Address || "Aucune adresse spécifiée"}
                        </p>
                        <div className="flex items-center gap-2 mt-5">
                          <span
                            className={`w-2 h-2 rounded-full ${site.S_IsActive ? "bg-emerald-500" : "bg-slate-300"} animate-pulse`}
                          ></span>
                  ...
                  <div className="absolute bottom-4 right-8 opacity-0 group-hover:opacity-40 transition-opacity">
                    <p className="text-[7px] font-black text-slate-400 uppercase">
                      SDE_ID: {site.S_Id.substring(0, 8)}...
                    </p>
                  </div>
                </div>
              ))}
              ...
              {sites.length === 0 && !loading && (
                <div className="col-span-1 md:col-span-2 py-40 flex flex-col items-center justify-center text-slate-400 gap-8 border-4 border-dashed border-slate-100 rounded-[4rem] m-2 animate-in zoom-in duration-700">
                  <div className="w-28 h-28 bg-slate-50 rounded-full flex items-center justify-center shadow-inner relative">
                    <Globe
                      size={56}
                      className="opacity-10 text-slate-900 animate-spin-slow"
                    />
                    <AlertCircle
                      className="absolute top-0 right-0 text-blue-600/40"
                      size={24}
                    />
                  </div>
                  <div className="text-center space-y-3">
                    <p className="font-black uppercase text-sm italic tracking-[0.3em] text-slate-600">
                      Néant géographique détecté
                    </p>
                    <p className="text-[10px] font-bold italic opacity-40 uppercase tracking-tighter max-w-xs mx-auto leading-relaxed">
                      L&apos;infrastructure Qualisoft Elite nécessite au moins
                      une implantation physique pour activer les modules SMI.
                      Utilisez le terminal d&apos;indexation.
                    </p>
                  </div>
                </div>
              )}
