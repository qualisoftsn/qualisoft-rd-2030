...
import { 
  ...
  Loader2, CheckCircle2, Layers, ArrowRight, X, AlertCircle, Zap,
  Plus
} from 'lucide-react';
...
export default function NewActionPage() {
  ...
  return (
    <div className="min-h-screen bg-[#0B0F1A] text-white italic font-sans ml-72 p-10 selection:bg-blue-600/30">
      ...
      >
        <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> Retour au Hub
      </button>
      ...
      <div className="max-w-4xl mx-auto text-left">
        ...
        <div className="flex gap-6 mb-12">
          {steps.map((s) => (
            <div key={s.id} className={`flex-1 flex items-center gap-4 p-5 rounded-3xl border transition-all duration-500 ${
              step >= s.id ? 'bg-blue-600/10 border-blue-500/30 text-blue-400 shadow-[0_0_20px_rgba(37,99,235,0.1)]' : 'bg-slate-900/40 border-white/5 text-slate-600'
            }`}>
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all ${step >= s.id ? 'bg-blue-600 text-white' : 'bg-slate-800'}`}>
                <s.icon size={20} />
              </div>
              <span className="text-[10px] font-black uppercase tracking-widest">{s.title}</span>
              {step > s.id && <CheckCircle2 size={16} className="ml-auto text-emerald-500" />}
            </div>
          ))}
        </div>
            ...
            <div className="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
                <div className="space-y-4">
                  <label className="text-[11px] font-black uppercase text-slate-500 tracking-widest ml-6 italic">Origine SMQ</label>
                  <select 
                ...
                <div className="space-y-4">
                  <label className="text-[11px] font-black uppercase text-slate-500 tracking-widest ml-6 italic">Typologie d&apos;action</label>
                  <select 
                    className="w-full bg-slate-950 border border-white/10 rounded-3xl p-6 text-xs font-black uppercase italic outline-none focus:border-blue-500 text-white appearance-none cursor-pointer"
              ...
              <div className="text-left pt-6">
                <label className="text-[11px] font-black uppercase text-slate-500 tracking-widest mb-6 block italic ml-6 leading-none">Indice de Priorité Stratégique</label>
          ...
          {step === 2 && (
            <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500 text-left">
              <div className="bg-blue-600/5 border-2 border-blue-500/20 rounded-[3rem] p-10 relative overflow-hidden group">
                <Zap className="absolute -right-12 -bottom-12 text-blue-500/5 rotate-12 group-hover:scale-110 transition-transform duration-1000" size={250} />
                ...
                >
                  <option value="" className="bg-[#0B0F1A]">SÉLECTIONNER UN AXE DU SMI...</option>
                ...
                <p className="mt-8 text-[11px] text-blue-500/50 font-black uppercase tracking-[0.2em] relative z-10 italic leading-none">
                  * LE RATTACHEMENT À UN PROCESSUS EST UN PRÉREQUIS À L&apos;AUDITABILITÉ.
                </p>
                ...
                {selectedProcess && (
                  <div className="mt-8 p-6 bg-blue-600/10 rounded-4xl border border-blue-500/20 animate-in zoom-in-95 relative z-10 shadow-lg">
                    <p className="text-[10px] text-blue-300 uppercase font-black italic tracking-widest leading-none mb-3">Contexte Processus</p>
                    <p className="text-lg font-black text-white italic leading-none mb-2">{selectedProcess.PR_Libelle}</p>
                    <p className="text-[10px] text-blue-400 font-bold italic tracking-widest">PILOTE : {selectedProcess.PR_Pilote?.U_FirstName} {selectedProcess.PR_Pilote?.U_LastName}</p>
                  </div>
                )}
              </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-4">
                  ...
                  >
                    ...
                    {users.map((u: any) => (
                      <option key={u.U_Id} value={u.U_Id} className="bg-[#0B0F1A]">{u.U_FirstName} {u.U_LastName}</option>
                    ))}
                  </select>
                </div>
                ...
                <div className="space-y-4">
                  <label className="text-[11px] font-black uppercase text-slate-500 ml-6 italic tracking-widest flex items-center gap-2">
                    <Calendar size={14} className="text-blue-500" /> Échéance de Traitement *
                  </label>
                  <input 
                    type="date"
                    required
          ...
          {step === 3 && (
            <div className="space-y-10 animate-in fade-in slide-in-from-right-4 duration-500 text-left">
              <div className="bg-slate-950/40 rounded-[3rem] p-10 border border-white/5 shadow-inner">
                <h3 className="text-[11px] font-black uppercase text-slate-500 tracking-[0.4em] mb-6 italic leading-none">Récapitulatif d&apos;indexation</h3>
                <div className="grid grid-cols-2 gap-8 text-sm italic uppercase font-black tracking-tight text-white">
                  <div className="space-y-1"><p className="text-[9px] text-slate-600">Action :</p><p className="truncate">{formData.ACT_Title}</p></div>
                  <div className="space-y-1"><p className="text-[9px] text-slate-600">Processus :</p><p className="text-blue-500 truncate">{selectedProcess?.PR_Libelle}</p></div>
                  <div className="space-y-1"><p className="text-[9px] text-slate-600">Priorité :</p><p className={formData.ACT_Priority === 'URGENT' ? 'text-red-500' : ''}>{formData.ACT_Priority}</p></div>
                  <div className="space-y-1"><p className="text-[9px] text-slate-600">Date Limite :</p><p>{formData.ACT_Deadline}</p></div>
                </div>
              </div>

              <div className="space-y-6">
                <h3 className="text-xl font-black uppercase italic tracking-tighter ml-4 leading-none text-white">Décomposition Granulaire (WBS)</h3>
                
                {formData.tasks.map((task, idx) => (
                  <div key={idx} className="flex gap-4 bg-slate-950/80 p-6 rounded-4xl border border-white/5 items-end shadow-xl animate-in slide-in-from-bottom-2 duration-300 transition-all group">
                    <div className="flex-1 space-y-3">
                      <label className="text-[10px] font-black uppercase text-slate-600 ml-4 italic tracking-widest">Tâche de travail {idx + 1}</label>
                      <input 
                    ...
                    <div className="w-64 space-y-3">
                       <label className="text-[10px] font-black text-slate-600 ml-2 italic tracking-widest uppercase">Opérateur</label>
                       <select 
                ...
                >
                  <Plus size={16} className="inline mr-2 group-hover:scale-125 transition-transform" /> Ajouter une unité de travail
                </button>
              </div>
            </div>
          )}
