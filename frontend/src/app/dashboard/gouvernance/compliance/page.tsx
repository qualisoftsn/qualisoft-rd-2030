...
/**
 * 💡 MODULE : REGISTRE DE VEILLE LÉGALE ET RÉGLEMENTAIRE
 * -------------------------------------------------------------------------
 * RÔLE : Suivi des exigences légales et évaluation de conformité (§6.1.3 & §9.1.2 ISO).
 * ARCHITECTURE SDE :
 * - Page "One-Pager" (100vh sans scroll global).
 * - Utilitaire `cn` globalisé et sécurisé pour Tailwind.
 * - Typage strict (suppression des 'any' abusifs).
 * -------------------------------------------------------------------------
 * DATE : 02 Mars 2026 | 00:20 GMT
 */
...
import { 
  ...
  CheckCircle2, ShieldAlert, Save, X, Link2, RefreshCcw,
  BookOpen, Target
} from 'lucide-react';
...
// 🛠️ UTILITAIRE GLOBAL
// ============================================================================
// Remplace la fonction défectueuse qui causait le crash du composant
const cn = (...classes: (string | boolean | undefined | null)[]) => {
  return classes.filter(Boolean).join(' ');
};

// ============================================================================
// 🛡️ TYPES STRICTS (RÉFÉRENTIEL SDE MATRIX)
// ============================================================================
...
export type ComplianceStatus = 'CONFORME' | 'NON_CONFORME' | 'A_EVALUER' | 'HORS_PERIMETRE';

export interface ComplianceRequirement {
  REQ_Id: string;
  REQ_Title: string;
  REQ_Source: string;
  REQ_Status: ComplianceStatus;
  REQ_Deadline: string | null;
  REQ_Observations: string;
  REQ_ProcessusId: string;
  REQ_Processus?: ProcessusBase;
}
...
export default function CompliancePage() {
  ...
  // Formulaire local strictement typé
  const [form, setForm] = useState({
    REQ_Title: '',
    REQ_Source: '',
    REQ_Deadline: '',
    REQ_Status: 'A_EVALUER' as ComplianceStatus,
    REQ_Observations: '',
    REQ_ProcessusId: ''
  });
  ...
  const stats = useMemo(() => {
    ...
    // Est critique si 'NON_CONFORME' ou si l'échéance est dépassée
  ...
  const handleDelete = async (id: string) => {
    if (confirm("Supprimer définitivement cette exigence légale ?")) {
      try {
        await apiClient.delete(`/requirements/${id}`);
        toast.success("Exigence supprimée du registre.");
        fetchData();
      } catch (err) {
  ...
  if (loading) return (
    <div className="ml-80 h-screen flex flex-col items-center justify-center bg-[#0B0F1A] text-blue-500">
      ...
      <span className="italic font-black uppercase tracking-[0.5em] text-[10px] animate-pulse">
        Audit du Noyau Légal...
      </span>
    </div>
  );
    ...
    <div className="ml-80 p-6 bg-[#0B0F1A] h-screen flex flex-col text-white italic text-left font-sans relative selection:bg-blue-600/30 overflow-hidden">
      ...
      <div className="shrink-0 grid grid-cols-3 gap-4 mb-4 h-24 animate-in fade-in duration-500">
        <div className="bg-[#151A2D] border border-white/10 p-5 rounded-3xl flex flex-col justify-center shadow-inner group">
          <p className="text-[9px] font-black uppercase text-slate-500 tracking-widest mb-2 flex items-center gap-2 italic m-0">
            <BookOpen size={12}/> Volume Exigences
          </p>
          <p className="text-3xl font-black italic tracking-tighter leading-none m-0 text-white">
            {stats.total}
          </p>
        </div>
        <div className="bg-rose-950/20 border border-rose-500/20 p-5 rounded-3xl flex flex-col justify-center shadow-inner group">
          <p className="text-[9px] font-black uppercase text-rose-500 tracking-widest mb-2 flex items-center gap-2 italic m-0">
            <AlertCircle size={12}/> Non-Conformités / Retards
          </p>
          <p className="text-3xl font-black italic tracking-tighter leading-none m-0 text-rose-500">
            {stats.critical}
          </p>
        </div>
        <div className="bg-emerald-950/20 border border-emerald-500/20 p-5 rounded-3xl flex flex-col justify-center shadow-inner group">
          <p className="text-[9px] font-black uppercase text-emerald-500 tracking-widest mb-2 flex items-center gap-2 italic m-0">
            <CheckCircle2 size={12}/> Taux de Conformité
          </p>
          <div className="flex items-center gap-3">
      ...
      <div className="flex-1 min-h-0 bg-[#151A2D] border border-white/5 rounded-4xl p-4 flex flex-col shadow-4xl backdrop-blur-sm">
        <div className="flex items-center gap-3 mb-4 px-2 shrink-0">
           ...
           <h3 className="text-[11px] font-black uppercase text-slate-400 tracking-[0.3em] m-0">
                ...
                <div key={req.REQ_Id} className={cn(
                  ...
                  <div className="flex gap-5 items-center flex-1 min-w-0">
                    ...
                    <div className={cn(
                      ...
                      isDanger ? 'bg-rose-500/10 border-rose-500/30 text-rose-500' : 
                      'bg-amber-500/10 border-amber-500/30 text-amber-500'
                    )}>
                      {req.REQ_Status === 'CONFORME' ? <CheckCircle2 size={20} /> :
                       isDanger ? <ShieldAlert size={20} className="animate-pulse" /> : 
                       <Loader2 size={20} />}
                    </div>
                  ...
                  <div className="flex items-center gap-8 shrink-0 ml-4">
                    <div className="text-right hidden xl:block">
                      <p className="text-[8px] font-black text-slate-600 uppercase italic mb-1 tracking-[0.2em] m-0">
                        Échéance Conformité
                      </p>
                    ...
                    <div className="flex gap-2">
                      <button 
                        ...
                        className="p-2.5 bg-black/40 rounded-lg hover:bg-blue-600 hover:text-white text-slate-400 transition-all border border-white/5 cursor-pointer"
                      >
                        <Edit3 size={14}/>
                      </button>
                      <button 
                        onClick={() => handleDelete(req.REQ_Id)} 
                        className="p-2.5 bg-black/40 rounded-lg hover:bg-rose-600 hover:text-white text-slate-400 transition-all border border-white/5 cursor-pointer"
                      >
                        <Trash2 size={14}/>
                      </button>
                    </div>
          ...
          <div className="bg-[#0B0F1A] border border-white/10 w-full max-w-2xl rounded-[3rem] p-10 shadow-4xl italic flex flex-col max-h-[90vh]">
            <div className="flex justify-between items-center mb-8 shrink-0">
              ...
              <button 
                ...
                className="bg-black/40 p-3 rounded-xl border border-white/5 text-slate-500 hover:text-white hover:bg-rose-500/20 cursor-pointer transition-colors"
              >
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto custom-scrollbar pr-2 space-y-6 text-left">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2 md:col-span-2">
                  <label className="text-[9px] font-black uppercase text-slate-500 ml-4 italic tracking-widest">
                    Libellé de l&apos;exigence (§6.1.3)
                  </label>
                  <input 
                    type="text" 
                    value={form.REQ_Title} 
                    onChange={e => setForm({...form, REQ_Title: e.target.value})} 
                    placeholder="EX: DÉCRET 2024-XXX SUR L'ENVIRONNEMENT..." 
                    className="w-full bg-[#151A2D] border border-white/5 p-4 rounded-2xl outline-none focus:border-blue-500 italic font-black text-white uppercase text-xs" 
                  />
                </div>

                <div className="space-y-2">
                  <label className="text-[9px] font-black uppercase text-slate-500 ml-4 italic tracking-widest">
                    Source (Loi, Norme, Arrêté)
                  </label>
                  <input 
                    type="text" 
                    value={form.REQ_Source} 
                    onChange={e => setForm({...form, REQ_Source: e.target.value})} 
                ...
                <div className="space-y-2">
                  ...
                  <select 
                    value={form.REQ_ProcessusId} 
                    onChange={e => setForm({...form, REQ_ProcessusId: e.target.value})} 
                    className="w-full bg-[#151A2D] border border-white/5 p-4 rounded-2xl outline-none focus:border-blue-500 italic font-black text-slate-300 appearance-none cursor-pointer text-xs"
                  >
                    <option value="" className="bg-[#0B0F1A]">-- SÉLECTIONNER PROCESSUS --</option>
                    {processes.map(p => (
                      <option key={p.PR_Id} value={p.PR_Id} className="bg-[#0B0F1A]">
                ...
                <div className="space-y-2">
                  ...
                  <select 
                    value={form.REQ_Status} 
                    onChange={e => setForm({...form, REQ_Status: e.target.value as ComplianceStatus})} 
                    className="w-full bg-[#151A2D] border border-white/5 p-4 rounded-2xl outline-none focus:border-blue-500 italic font-black text-slate-300 appearance-none cursor-pointer text-xs"
                  >
                ...
                <div className="space-y-2">
                  <label className="text-[9px] font-black uppercase ml-4 italic tracking-widest text-amber-500">
                    Échéance de mise en conformité
                  </label>
                  <input 
                    type="date" 
                    value={form.REQ_Deadline} 
                    onChange={e => setForm({...form, REQ_Deadline: e.target.value})} 
                    className="w-full bg-[#151A2D] border border-white/5 p-4 rounded-2xl outline-none focus:border-amber-500 font-black text-amber-500 text-xs cursor-pointer color-scheme-dark" 
                  />
                </div>
                ...
                <div className="space-y-2 md:col-span-2">
                  ...
                  <textarea 
                    rows={3} 
                    value={form.REQ_Observations} 
                    onChange={e => setForm({...form, REQ_Observations: e.target.value})} 
                    className="w-full bg-[#151A2D] border border-white/5 p-4 rounded-2xl outline-none focus:border-blue-500 italic font-bold text-slate-300 resize-none text-xs" 
                    placeholder="Note d'analyse d'impact..." 
                  />
                </div>
              </div>
              ...
              <div className="pt-6 shrink-0 border-t border-white/5 mt-6">
                 <button 
                   type="submit" 
                   disabled={submitting} 
                   className="w-full bg-blue-600 p-5 rounded-4xl font-black uppercase italic tracking-[0.2em] flex items-center justify-center gap-3 border-none text-white cursor-pointer shadow-lg hover:bg-blue-500 transition-all text-xs disabled:opacity-50"
                 >
      ...
      {/* 🧪 INJECTION CSS SOUVERAIN (Format sûr pour Next.js App Router) */}
