...
import { 
  Plus, Activity, ShieldCheck, X, Save, 
  Loader2, BookOpen, Search, Calendar, 
        ...
        <section className="bg-slate-900/40 border border-white/5 rounded-[3.5rem] overflow-hidden shadow-2xl backdrop-blur-md transition-all hover:border-blue-500/20">
          <div className="p-10 border-b border-white/5 bg-white/2 flex items-center justify-between">
              <div className="flex items-center gap-4">
                ...
                <h3 className="text-sm tracking-[0.2em] italic uppercase font-black">REGISTRE DES HABILITATIONS & QUALIFICATIONS</h3>
              </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse">
              <thead className="text-[10px] text-slate-600 border-b border-white/5 uppercase italic">
                <tr>
                  <th className="p-10 font-black tracking-widest">COLLABORATEUR</th>
                  <th className="p-10 font-black tracking-widest text-center">FORMATION / TITRE DÉLIVRÉ</th>
                  <th className="p-10 font-black tracking-widest text-right">STATUT GPEC</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-white/5">
                {filteredFormations.length > 0 ? (
                  filteredFormations.map((f) => (
                    <tr key={f.FOR_Id} className="hover:bg-blue-600/5 transition-all group cursor-default">
                      <td className="p-10">
                        <div className="flex items-center gap-4">
                      ...
                      <td className="p-10 text-center">
                        <div className="flex flex-col gap-1">
                          <span className="italic uppercase text-white font-black text-[12px]">{f.FOR_Title}</span>
                          <span className="text-[9px] text-slate-500 font-bold tracking-widest italic">{f.FOR_Provider}</span>
                        </div>
                      </td>
                  ...
                  <tr>
                    <td colSpan={3} className="p-32 text-center">
                       <AlertCircle size={48} className="mx-auto text-slate-800 mb-6 opacity-20" />
        ...
        <div className="fixed inset-0 bg-black/98 backdrop-blur-2xl z-100 flex items-center justify-center p-6 animate-in fade-in duration-300">
            ...
            onSubmit={async (e: any) => {
              ...
              try {
                ...
                await apiClient.post('/formations', {
                  ...
                  FOR_Expiry: data.FOR_Expiry ? new Date(data.FOR_Expiry as string).toISOString() : null
                });
          ...
          >
            <div className="flex justify-between items-center border-b border-white/10 pb-10">
               <div>
                  <h2 className="text-4xl italic font-black uppercase leading-none tracking-tighter">PLANIFICATION <span className="text-blue-600">SESSION</span></h2>
                  <p className="text-[10px] text-slate-500 uppercase tracking-[0.3em] mt-3 italic font-black">INDEXATION DU CAPITAL HUMAIN</p>
               </div>
               <button type="button" onClick={() => setIsModalOpen(false)} className="p-4 bg-white/5 rounded-2xl text-slate-500 hover:text-red-500 transition-all border-none cursor-pointer"><X size={24}/></button>
            </div>

            <div className="space-y-8 font-black italic">
              {/* LIBELLÉ FORMATION */}
              <div className="flex flex-col gap-3">
                <label className="text-[11px] text-slate-500 uppercase tracking-widest ml-4">INTITULÉ DE LA FORMATION / HABILITATION *</label>
                <input required name="FOR_Title" placeholder="EX: HABILITATION ÉLECTRIQUE B2V" className="bg-white/5 border border-white/10 p-6 rounded-2xl text-[13px] text-white outline-none focus:border-blue-600 uppercase font-black" />
              </div>
              ...
              <div className="grid grid-cols-2 gap-8">
                <div className="flex flex-col gap-3">
                  ...
                  <select required name="FOR_UserId" className="bg-white/5 border border-white/10 p-6 rounded-2xl text-[12px] text-white outline-none cursor-pointer font-black uppercase italic">
                ...
                <div className="flex flex-col gap-3">
                  <label className="text-[11px] text-slate-500 uppercase tracking-widest ml-4">DATE DE SESSION *</label>
                  <input required name="FOR_Date" type="date" className="bg-white/5 border border-white/10 p-6 rounded-2xl text-[12px] text-white outline-none focus:border-blue-600 font-black" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-8">
                <div className="flex flex-col gap-3">
                  <label className="text-[11px] text-slate-500 uppercase tracking-widest ml-4">ÉCHÉANCE DE RECYCLAGE</label>
                  <input name="FOR_Expiry" type="date" className="bg-white/5 border border-white/10 p-6 rounded-2xl text-[12px] text-white outline-none focus:border-blue-600 font-black" />
                </div>
                <div className="flex flex-col gap-3">
                  <label className="text-[11px] text-slate-500 uppercase tracking-widest ml-4">ORGANISME PRESTATEUR *</label>
                  <select required name="FOR_Provider" className="bg-white/5 border border-white/10 p-6 rounded-2xl text-[12px] text-white outline-none cursor-pointer font-black italic uppercase">
                    <option value="INTERNE" className="bg-[#0F172A]">FORMATION INTERNE</option>
                    <option value="BUREAU VERITAS" className="bg-[#0F172A]">BUREAU VERITAS</option>
                    <option value="APAVE" className="bg-[#0F172A]">APAVE</option>
                    <option value="SGS" className="bg-[#0F172A]">SGS</option>
                    <option value="AFNOR" className="bg-[#0F172A]">AFNOR</option>
                    <option value="AUTRE" className="bg-[#0F172A]">AUTRE (HORS LISTE)</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            <div className="pt-6">
              <button type="submit" disabled={loading} className="w-full bg-blue-600 py-10 rounded-[2.5rem] font-black text-[13px] tracking-[0.5em] flex items-center justify-center gap-4 hover:bg-blue-500 transition-all shadow-3xl shadow-blue-900/40 border-none text-white italic cursor-pointer active:scale-95 disabled:opacity-50 group">
                 {loading ? <Loader2 className="animate-spin" size={24} /> : <Award size={24} className="group-hover:rotate-12 transition-transform" />} 
                 SCELLER L&apos;INSCRIPTION
              </button>
              <p className="text-[9px] text-slate-600 text-center mt-6 tracking-widest">EN VALIDANT, VOUS MISEZ SUR LE CAPITAL COMPÉTENCE DU SMI.</p>
            </div>
          </form>
        </div>
      )}
