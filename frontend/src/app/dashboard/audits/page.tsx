 ...
 * --------------------------
 * Fichier : app/dashboard/audits/page.tsx
 * Rôle : Registre principal et planificateur des audits du SMI.
 * * Fonctionnalités clés :
 * 1. Planification : Formulaire rapide pour programmer un nouvel audit (Titre, Date, Processus, Site). Génère automatiquement une référence (ex: AUD-2026-123).
 * 2. Registre : Affiche la liste de tous les audits planifiés ou réalisés.
 * 3. Navigation Contextuelle : Chaque carte d'audit propose des liens rapides pour aller "Collecter des preuves" ou "Rédiger le rapport".
 * * Public cible : Responsable Qualité, Auditeurs, Direction.
 */
...
export default function AuditsPage() {
  ...
  // const [users, setUsers] = useState<User[]>([]); // Conservé si besoin futur
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      console.error("Erreur Sync Audits", err); 
    } finally { 
      setLoading(false); 
    }
  }, []);
  ...
  if (loading) return (
    <div className="ml-72 flex h-screen items-center justify-center bg-[#0B0F1A] text-blue-500 font-black italic uppercase">
      <Loader2 className="animate-spin mr-3" /> Analyse du Plan d&apos;Audit...
    </div>
  );
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        <div className="lg:col-span-1 bg-slate-900/40 p-10 rounded-[3rem] border border-white/5 shadow-2xl h-fit sticky top-10">
          <h2 className="text-xl font-black uppercase italic mb-8 flex items-center gap-3">
            <Plus className="text-blue-500" /> Planifier
          </h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            <input 
              required 
              placeholder="Titre de l'audit" 
              className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-xs font-bold outline-none focus:border-blue-500 text-white" 
            ...
            <select 
              required 
              className="w-full bg-white/5 border border-white/10 rounded-2xl p-5 text-xs font-bold outline-none focus:border-blue-500 text-slate-400 cursor-pointer"
            ...
            <button type="submit" className="w-full py-5 bg-blue-600 text-white rounded-3xl font-black uppercase italic text-xs shadow-xl transition-all active:scale-95 cursor-pointer hover:bg-blue-500">
              Programmer
            </button>
          </form>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <h2 className="text-2xl font-black uppercase italic mb-8 flex items-center gap-3 text-white">
            <ClipboardCheck className="text-blue-500" /> Registre ({audits.length})
          </h2>
            ...
            {audits.map((audit) => (
              <div key={audit.AU_Id} className="p-8 bg-slate-900/40 rounded-[2.5rem] border border-white/5 group hover:border-blue-500/30 transition-all">
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <span className="text-[8px] font-black bg-blue-600/10 text-blue-500 border border-blue-500/20 px-3 py-1 rounded-full uppercase italic">
                      Ref: {audit.AU_Reference}
                    </span>
                    <h3 className="text-2xl font-black uppercase italic mt-3 tracking-tighter text-white">
                      {audit.AU_Title}
                    </h3>
                  </div>
                  <div className="flex gap-2">
                    <Link 
                      ...
                      className="p-3 bg-white/5 rounded-xl hover:bg-white/10 text-slate-400 shadow-xl transition-all cursor-pointer" 
                    ...
                    <Link 
                      ...
                      className="p-3 bg-blue-600 text-white rounded-xl shadow-xl hover:bg-blue-500 transition-all cursor-pointer"
                ...
                <div className="grid grid-cols-3 gap-4 pt-6 border-t border-white/5">
                  <div className="flex items-center gap-2 text-slate-400 font-bold uppercase text-[9px]">
                    ...
                    <span className="truncate">{audit.AU_Site?.S_Name || 'N/A'}</span>
                  </div>
                  <div className="flex items-center gap-2 text-slate-400 font-bold uppercase text-[9px]">
                    <FolderTree size={14} className="text-blue-500"/>
                    <span className="truncate">{audit.AU_Processus?.PR_Libelle || 'N/A'}</span>
                  </div>
                  <div className="flex items-center gap-2 text-slate-400 font-bold uppercase text-[9px]">
                    <Calendar size={14} className="text-blue-500"/>
                    {new Date(audit.AU_DateAudit).toLocaleDateString()}
                  </div>
                </div>
              </div>
            ))}
