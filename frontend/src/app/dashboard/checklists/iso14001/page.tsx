  ...
  const filteredItems = useMemo(() => {
    return checklistItems.filter(item => {
      const matchesSearch = 
        ...
        (item.LC_Reference && item.LC_Reference.toLowerCase().includes(searchTerm.toLowerCase()));
        ...
        <div className="flex justify-between items-center mb-4">
          <div>
            ...
            <p className="text-[10px] text-slate-400 mt-1 italic">
              Suivi de la conformité aux exigences ISO 14001:2015 et réglementation sénégalaise
            </p>
          </div>
          <div className="text-right">
            <span className="text-3xl font-black">{stats?.complianceRate || 0}%</span>
            <p className="text-[10px] text-slate-400 mt-1">Taux de conformité environnementale</p>
          </div>
        </div>
                          ...
                          <div className="lg:col-span-2">
                              ...
                              <div className="flex-1">
                                ...
                                {isSenegalSpecific && (
                                  <span className="text-[9px] font-black bg-amber-500/20 text-amber-300 px-2 py-0.5 rounded mt-1 inline-block">
                                    <MapPin size={10} className="inline mr-1" /> Spécifique Sénégal
                                  </span>
                                )}
                              </div>
                            </div>
                            ...
                            {item.LC_Reference && (
                              <div className="text-[10px] text-slate-500 italic">
                                <span className="font-black text-green-400">Référence légale:</span> {item.LC_Reference}
                              </div>
                            )}
                          </div>
                          ...
                          <div className="space-y-4">
                            ...
                            <div>
                              ...
                              {hasEvidence ? (
                                <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-lg p-3">
                                  ...
                                  >
                                    <FileText size={14} /> Voir la preuve (photo, certificat, relevé)
                                  </a>
                                </div>
                              ) : (
                                <label className="flex flex-col items-center justify-center w-full h-24 border-2 border-dashed rounded-xl bg-white/5 border-white/10 hover:border-green-500/30 hover:bg-green-500/5 cursor-pointer transition-all">
                                  <UploadCloud size={24} className="text-green-400 mb-2" />
                                  <span className="text-[10px] font-black text-slate-400">
                                    {uploadingEvidence === item.LC_Id ? 'Téléchargement...' : 'Uploader preuve'}
                                  </span>
                                  <input 
                                    type="file" 
                                    className="hidden" 
                            ...
                            <div className="pt-2 border-t border-white/5 flex justify-end">
                              <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase ${
                                ...
                                {isCompliant ? 'Maîtrisé' : response ? 'À améliorer' : 'Non évalué'}
                              </span>
                            </div>
                          </div>
                        </div>
    ...
    <div className={`${color} rounded-2xl p-6`}>
      <div className="flex items-center justify-between mb-4">
        ...
        <div className={`text-[10px] font-black px-3 py-1 rounded-full ${
          trend.startsWith('+') ? 'bg-emerald-500/20 text-emerald-300' : 
          trend.startsWith('-') ? 'bg-amber-500/20 text-amber-300' : 'bg-slate-500/20 text-slate-300'
        }`}>
          {trend}
        </div>
      </div>
...
function EngagementCard({ title, description, status, color }: any) {
  ...
  return (
    <div className={`${color} rounded-xl p-5`}>
      ...
      <div className={`${config.bg} ${config.color} text-[9px] font-black uppercase tracking-widest px-3 py-1 rounded-full inline-block`}>
        {status}
      </div>
    </div>
  );
