...
const useDocumentLibrary = () => {
  ...
  const fetchDocs = useCallback(async () => {
    try {
      ...
      if (filters.processus !== "ALL")
        params.append("processus", filters.processus);
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      pending: documents.filter((d) => d.DOC_Status === "PENDING_REVIEW")
        .length,
      overdue: documents.filter(
        (d) =>
          d.DOC_NextReviewDate &&
          isPast(new Date(d.DOC_NextReviewDate)) &&
          d.DOC_Status !== "OBSOLETE",
      ).length,
...
export default function LibraryPage() {
  const { documents, loading, filters, setFilters, stats, refetch } =
    useDocumentLibrary();
          ...
          <div className="flex gap-4 items-center">
            ...
            <button
              onClick={() =>
                setFilters((f) => ({
                  ...f,
                  dateRange: f.dateRange === "overdue" ? "all" : "overdue",
                }))
              }
              className={cn(
          ...
          onApprove={async (approved) => {
            try {
              ...
              await apiClient.post(
                `/documents/${modalState.doc!.DOC_Id}/versions/${modalState.doc!.DOC_Versions[0].DV_Id}/approve`,
                {
                  approved,
                },
              );
...
function DocumentCard({
  ...
  const isOverdue = doc.DOC_NextReviewDate
    ? isPast(new Date(doc.DOC_NextReviewDate)) && doc.DOC_Status !== "OBSOLETE"
    : false;
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-3 mb-1">
            ...
            <span
              className={cn(
                "text-[9px] px-2 py-0.5 rounded-full font-bold uppercase bg-opacity-20 border",
          ...
          <p className="text-[10px] text-slate-500 truncate">
            {doc.DOC_Description || "Pas de description"}
          </p>
        </div>
        <div className="flex items-center gap-6 text-[10px] font-bold text-slate-400">
          ...
          <div className="flex gap-2">
            ...
            <button
              onClick={onDownload}
              className="p-2 hover:bg-blue-600 rounded-lg transition-colors cursor-pointer text-white"
              title="Télécharger"
            >
              <Download size={16} />
            </button>
            {doc.DOC_Status === "APPROVED" && (
              <button
                onClick={onRevise}
                className="p-2 hover:bg-amber-600 rounded-lg transition-colors cursor-pointer text-white"
                title="Réviser"
              >
                <FileEdit size={16} />
              </button>
            ...
            {doc.DOC_Status === "PENDING_REVIEW" && (
              <button
                onClick={onApprove}
                className="p-2 hover:bg-emerald-600 rounded-lg transition-colors cursor-pointer text-white"
                title="Approuver"
              >
                <CheckSquare size={16} />
              </button>
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between items-start mb-6">
        ...
        <div
          className={cn(
            "px-3 py-1.5 rounded-full text-[9px] font-black uppercase border bg-opacity-10",
      ...
      <h3
        className="text-2xl font-black uppercase italic text-white mb-3 leading-tight line-clamp-2"
        title={doc.DOC_Title}
      >
        {doc.DOC_Title}
      </h3>
      ...
      <p
        className="text-[11px] text-slate-400 font-bold mb-6 line-clamp-2 uppercase"
        title={doc.DOC_Description}
      >
        {doc.DOC_Description ||
          "Aucune description analytique disponible pour ce document qualité."}
      </p>
      ...
      <div className="space-y-4 mb-6">
        <div className="flex justify-between items-center text-[10px] font-bold uppercase">
          ...
          <span className="text-slate-300">
            {doc.DOC_Owner?.U_FirstName} {doc.DOC_Owner?.U_LastName}
          </span>
        </div>
        <div className="flex justify-between items-center text-[10px] font-bold uppercase">
          <span className="text-slate-600 flex items-center gap-2">
          ...
          <span className="text-emerald-400">
            v{latestVersion?.DV_VersionNumber}.0
          </span>
        </div>
        <div className="flex justify-between items-center text-[10px] font-bold uppercase">
          <span className="text-slate-600 flex items-center gap-2">
            <Calendar size={12} /> Prochaine Revue
          </span>
          <span className={cn(isOverdue ? "text-red-400" : "text-slate-300")}>
            {doc.DOC_NextReviewDate
              ? format(new Date(doc.DOC_NextReviewDate), "dd MMM yyyy", {
                  locale: fr,
                })
              : "Non définie"}
            {isOverdue && " (EN RETARD)"}
      ...
      <div className="flex gap-3 pt-6 border-t border-white/5">
        <button
          onClick={onPreview}
          className="flex-1 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-[10px] font-black uppercase transition-all flex justify-center items-center gap-2 cursor-pointer text-white"
        >
          <Eye size={14} /> Voir
        </button>
        <button
        ...
        <button
          onClick={onDownload}
          className="flex-1 py-3 bg-blue-600/20 hover:bg-blue-600 text-blue-400 hover:text-white rounded-xl text-[10px] font-black uppercase transition-all flex justify-center items-center gap-2 cursor-pointer"
        >
          <Download size={14} />
        </button>
        {doc.DOC_Status === "APPROVED" && (
          <button
            onClick={onRevise}
            className="flex-1 py-3 bg-amber-600/20 hover:bg-amber-600 text-amber-400 hover:text-white rounded-xl text-[10px] font-black uppercase transition-all flex justify-center items-center gap-2 cursor-pointer"
          >
        ...
        {doc.DOC_Status === "PENDING_REVIEW" && (
          <button
            onClick={onApprove}
            className="flex-1 py-3 bg-emerald-600/20 hover:bg-emerald-600 text-emerald-400 hover:text-white rounded-xl text-[10px] font-black uppercase transition-all flex justify-center items-center gap-2 cursor-pointer"
          >
...
function CreateModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    formData.append(
      "DOC_ReviewFrequencyMonths",
      form.DOC_ReviewFrequencyMonths.toString(),
    );
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0B0F1A]/95 backdrop-blur-3xl p-6">
      <div className="bg-[#151B2B] border border-white/10 w-full max-w-2xl rounded-[3rem] p-10 shadow-3xl max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-8">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-white/5 rounded-full transition-colors cursor-pointer text-white"
          >
            <X size={28} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            ...
            <p className="text-lg font-black uppercase italic text-white mb-2">
              {file ? file.name : "Déposez notre fichier ici"}
            </p>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="col-span-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, DOC_Title: e.target.value }))
                }
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    DOC_Category: e.target.value as DocumentCategory,
                  }))
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setForm((f) => ({
                    ...f,
                    DOC_ReviewFrequencyMonths: parseInt(e.target.value),
                  }))
                }
            ...
            <div className="col-span-2">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, DOC_Description: e.target.value }))
                }
            ...
            <div className="col-span-2">
              ...
              <div className="flex gap-2 mb-3 flex-wrap">
                {form.DOC_Tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-3 py-1 bg-blue-500/20 text-blue-400 rounded-full text-[10px] font-bold uppercase flex items-center gap-2"
                  >
                    {tag}
                    <button
                      type="button"
                      onClick={() =>
                        setForm((f) => ({
                          ...f,
                          DOC_Tags: f.DOC_Tags.filter((t) => t !== tag),
                        }))
                      }
              ...
              <div className="flex gap-2">
                ...
                <button
                  type="button"
                  onClick={addTag}
                  className="px-6 bg-white/5 hover:bg-white/10 rounded-2xl font-black uppercase text-xs transition-colors cursor-pointer text-white"
                >
                  Ajouter
                </button>
              </div>
...
function RevisionModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `Version ${doc.DOC_Versions[0].DV_VersionNumber + 1}.0 créée`,
      );
      onSuccess();
      onClose();
    } catch (e) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0B0F1A]/95 backdrop-blur-3xl p-6">
      <div className="bg-[#151B2B] border border-white/10 w-full max-w-2xl rounded-[3rem] p-10 shadow-3xl">
        <div className="flex justify-between items-center mb-8">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-white/5 rounded-full cursor-pointer text-white"
          >
            <X size={28} />
          </button>
        </div>
        ...
        <div className="bg-amber-500/10 border border-amber-500/20 rounded-2xl p-6 mb-8">
          <p className="text-[11px] font-black text-amber-400 uppercase mb-2">
            Document actuel : {doc.DOC_Reference} (v
            {doc.DOC_Versions[0].DV_VersionNumber}.0)
          </p>
          <p className="text-[10px] text-amber-200/60 italic">
            La nouvelle version sera soumise pour approbation avant activation.
            L&apos;ancienne version restera active jusqu&apos;à approbation de
            la nouvelle.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          >
            ...
            <p className="text-sm font-black uppercase text-white">
              {file ? file.name : "Nouvelle version du fichier"}
            </p>
            <input
              id="revise-file"
              type="file"
          ...
          <div className="flex gap-4">
            <button
              type="button"
              onClick={onClose}
              className="flex-1 py-5 rounded-2xl border border-white/10 font-black uppercase text-xs hover:bg-white/5 transition-all text-white cursor-pointer"
            >
              Annuler
            </button>
            <button
      ...
      <div className="bg-[#151B2B] border border-white/10 w-full max-w-3xl rounded-[3rem] p-10 shadow-3xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          {doc.DOC_Versions.map((version) => (
            <div
              key={version.DV_Id}
              className="bg-[#0B0F1A] border border-white/5 rounded-2xl p-6"
            >
              <div className="flex justify-between items-start mb-4">
                <div>
                  <h3 className="text-xl font-black uppercase italic text-white">
                    Version {version.DV_VersionNumber}.0
                  </h3>
                  <p className="text-[10px] text-slate-500 font-bold uppercase">
                    {format(
                      new Date(version.DV_CreatedAt),
                ...
                <span
                  className={cn(
                    ...
                    version.DV_Status === "APPROVED"
                      ? "bg-emerald-500/10 border-emerald-500/30 text-emerald-400"
                      : version.DV_Status === "PENDING_REVIEW"
              ...
              <p className="text-sm text-slate-400 italic mb-4">
                {version.DV_ChangeDescription ||
                  "Création initiale du document"}
              </p>

              <div className="flex gap-4 text-[10px] font-bold text-slate-500 uppercase">
                <span className="flex items-center gap-2">
                  <User size={12} /> {version.DV_CreatedBy.U_FirstName}{" "}
                  {version.DV_CreatedBy.U_LastName}
                </span>
                {version.DV_ApprovedBy && (
                  <span className="flex items-center gap-2 text-emerald-400">
                    <CheckCircle2 size={12} /> Approuvé par{" "}
                    {version.DV_ApprovedBy.U_FirstName}{" "}
                    {version.DV_ApprovedBy.U_LastName}
                  </span>
                )}
                <button
...
function PreviewModal({
  ...
  useEffect(() => {
    const loadPreview = async () => {
      try {
        const res = await apiClient.get(`/documents/${doc.DOC_Id}/preview`, {
          responseType: "blob",
        });
        ...
        <div className="flex justify-between items-center p-6 border-b border-white/5">
          <div>
            ...
            <p className="text-xs text-slate-500 font-bold uppercase">
              Version {doc.DOC_Versions[0].DV_VersionNumber}.0 •{" "}
              {doc.DOC_Reference}
            </p>
          </div>
          <div className="flex gap-3">
            <button
              onClick={onDownload}
              className="px-6 py-3 bg-blue-600 rounded-2xl text-xs font-black uppercase flex items-center gap-2 hover:bg-blue-500 transition-colors cursor-pointer text-white"
            >
              <Download size={16} /> Télécharger
            </button>
            <button
              onClick={onClose}
              className="p-3 hover:bg-white/10 rounded-full transition-colors cursor-pointer text-white"
            >
              <X size={24} />
            </button>
          </div>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#0B0F1A]/95 backdrop-blur-3xl p-6">
      <div className="bg-[#151B2B] border border-white/10 w-full max-w-xl rounded-[3rem] p-10 shadow-3xl">
        ...
        <div className="bg-white/5 rounded-2xl p-6 mb-8">
          <h3 className="font-black uppercase italic text-white mb-2">
            {doc.DOC_Title}
          </h3>
          <p className="text-sm text-slate-400">
            Version soumise : {doc.DOC_Versions[0].DV_VersionNumber}.0
          </p>
          <p className="text-sm text-slate-400">
            Auteur : {doc.DOC_Versions[0].DV_CreatedBy.U_FirstName}{" "}
            {doc.DOC_Versions[0].DV_CreatedBy.U_LastName}
          </p>
        </div>
