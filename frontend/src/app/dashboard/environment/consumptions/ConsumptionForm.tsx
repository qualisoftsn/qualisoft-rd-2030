...
export default function ConsumptionForm({ onClose, onSuccess, sites }: ConsumptionFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    const loadingToast = toast.loading("ENREGISTREMENT ISO 14001...");
    try {
      await apiClient.post('/consumptions', formData);
      toast.success("CONSOMMATION INDEXÉE", { id: loadingToast });
      onSuccess();
      onClose();
    } catch (err: any) {
      toast.error(err.response?.data?.message || "ERREUR D'ENREGISTREMENT", { id: loadingToast });
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="bg-[#0F172A] w-full max-w-2xl rounded-[4rem] border border-white/10 p-16 space-y-10 shadow-4xl animate-in zoom-in-95 duration-500">
        ...
        <div className="flex justify-between items-start border-b border-white/10 pb-8">
          <div>
            ...
            <p className="text-[9px] text-slate-500 tracking-[0.3em] mt-3">INDEXATION CONSOMMATION §9.1.1</p>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="grid grid-cols-2 gap-8">
            <div className="space-y-3">
              ...
              <select 
                className="w-full bg-white/5 border border-white/10 p-6 rounded-4xl text-sm text-white outline-none focus:border-amber-500 transition-all font-black italic uppercase cursor-pointer"
          ...
          <div className="grid grid-cols-3 gap-6">
            ...
            <div className="space-y-3">
              <label className="text-[10px] text-slate-500 ml-6 tracking-widest uppercase">UNITÉ</label>
            ...
            <div className="space-y-3">
              <label className="text-[10px] text-slate-500 ml-6 tracking-widest uppercase">COÛT (XOF)</label>
          ...
          <div className="grid grid-cols-2 gap-8">
             ...
             <div className="space-y-3">
                <label className="text-[10px] text-slate-500 ml-6 tracking-widest uppercase">ANNÉE</label>
