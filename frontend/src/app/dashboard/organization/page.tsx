...
import { 
  ...
  Clock, FolderTree, UserCircle, LucideIcon
} from 'lucide-react';
...
interface OrgStats {
  ...
  coverageRate: number; // % d'unités avec pilote/responsable assigné
...
export default function OrganizationDashboard() {
  ...
  // 🛰️ PROTOCOLE DE SYNCHRONISATION (Mocké pour le build, prêt pour API)
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        setLoading(true);
        // Simulation d'appel : await apiClient.get('/organization/dashboard')
        setTimeout(() => {
          setStats({
            totalUnits: 12,
            totalSites: 3,
            ...
            alerts: [
              {
                id: '1',
                type: 'warning',
                message: 'Unité sans responsable identifié (SMI non scellé)',
              ...
              },
              {
                id: '2',
                type: 'danger',
                message: 'Site sans unité rattachée (Zone morte)',
                unit: 'Site de Thiès',
              ...
              },
              {
                id: '3',
                type: 'info',
                message: 'Mise à jour structurelle requise par la Direction',
  ...
  // 🏥 CALCUL DE LA SANTÉ ORGANISATIONNELLE (Logique ISO)
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50 italic">
        <div className="text-center space-y-6">
          ...
          <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.5em]">
  ...
  return (
    <div className="p-6 lg:p-10 space-y-10 bg-slate-50 min-h-screen italic font-sans text-left pb-24 selection:bg-blue-100">
      ...
      <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-8">
        <div className="space-y-4">
          <div className="flex items-center gap-3">
             ...
             <span className="px-4 py-1.5 rounded-full bg-slate-900 text-white text-[9px] font-black uppercase tracking-widest">
                Structure Certifiée ISO
             </span>
          </div>
          <h1 className="text-5xl lg:text-4xl font-black uppercase tracking-tighter text-slate-900 leading-none">
        ...
        <div className="flex gap-4">
          <Link 
            ...
            className="px-8 py-5 bg-white border border-slate-200 rounded-3xl text-slate-900 font-black uppercase text-[11px] tracking-widest shadow-sm hover:border-blue-600 hover:text-blue-600 transition-all flex items-center gap-3 group"
          >
          ...
          <button 
            ...
            className="px-8 py-5 bg-slate-900 text-white rounded-3xl font-black uppercase text-[11px] tracking-widest shadow-2xl hover:bg-blue-600 transition-all flex items-center gap-3 border-none cursor-pointer"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
        ...
        {/* 🏛️ COLONNE GAUCHE : DISTRIBUTION ET SITES */}
        <div className="lg:col-span-2 space-y-10">
          ...
          <div className="bg-white rounded-[4rem] p-10 shadow-2xl border border-white relative overflow-hidden">
            ...
            <div className="relative z-10 mb-10">
              ...
              <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.3em] mt-2">
                Poids des responsabilités par type d&apos;unité organique
              </p>
            </div>
                ...
                <div key={idx} className="group">
                  <div className="flex justify-between items-end mb-3">
                    <span className="text-[11px] font-black text-slate-900 uppercase italic flex items-center gap-3">
                      <div className={`w-4 h-4 rounded-full ${item.color} shadow-lg shadow-black/10`} />
                      {item.type}
                    </span>
            ...
            <div className="mt-12 pt-8 border-t border-slate-50 flex justify-between items-center">
              <div className="flex items-center gap-3 text-[10px] font-black uppercase text-slate-400">
                ...
                Intégrité du SMI : Opérationnelle
              </div>
              <button 
                onClick={() => router.push('/dashboard/organization/chart')}
                className="text-blue-600 text-[10px] font-black uppercase tracking-widest hover:translate-x-1 transition-transform flex items-center gap-2 border-none bg-transparent cursor-pointer"
              >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div 
                key={idx} 
                className="bg-white rounded-[2.5rem] p-8 border border-white shadow-xl hover:shadow-2xl hover:border-blue-600/20 transition-all group cursor-pointer"
                onClick={() => router.push(`/dashboard/sites/${idx + 1}`)}
              >
                <div className="flex justify-between items-start mb-6">
                  <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${
                    site.status === 'actif' ? 'bg-emerald-50 text-emerald-600' : 'bg-amber-50 text-amber-600'
                  }`}>
                    <Building2 size={24} />
                  </div>
                  {site.status === 'warning' && <AlertTriangle size={18} className="text-amber-500 animate-pulse" />}
                </div>
                <h3 className="text-xl font-black uppercase italic text-slate-900 mb-2 leading-tight">{site.name}</h3>
                <p className="text-[10px] font-black text-slate-400 uppercase mb-6 flex items-center gap-2">
                  <MapPin size={12} className="text-blue-600" /> {site.location}
                </p>
                <div className="flex gap-6 pt-6 border-t border-slate-50">
                  <div className="text-center">
                    <span className="text-sm font-black text-slate-900 block">{site.units}</span>
                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Unités</span>
                  </div>
                  <div className="text-center">
                    <span className="text-sm font-black text-slate-900 block">{site.users}</span>
                    <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest">Staff</span>
                  </div>
                </div>
              </div>
            ))}
        ...
        {/* ⚖️ COLONNE DROITE : SANTÉ ET ALERTES */}
        <div className="space-y-10">
          
          {/* CARTE SCORE DE SANTÉ */}
          <div className="bg-slate-900 rounded-[4rem] p-10 text-white relative overflow-hidden shadow-2xl group">
            <div className="absolute -right-16 -top-16 text-white/5 group-hover:rotate-12 transition-transform duration-700">
              <ShieldCheck size={300} />
            </div>
            
            <div className="relative z-10">
              <h3 className="text-[10px] font-black uppercase mb-4 tracking-[0.4em] text-slate-400 italic">
              ...
              <div className="flex items-end gap-3 mb-8">
                ...
                <span className="text-xs font-black text-emerald-400 mb-4 flex items-center gap-1 uppercase">
              ...
              <div className="h-4 bg-white/10 rounded-full overflow-hidden mb-8 p-1 border border-white/5">
                <div 
                  className="h-full bg-linear-to-r from-blue-600 to-emerald-500 rounded-full transition-all duration-1000"
              ...
              <div className="space-y-4 text-[10px] font-black uppercase tracking-widest">
                <div className="flex justify-between items-center py-3 border-b border-white/10">
                  <span className="text-slate-400 italic">Leadership Scellé</span>
                  <span className="text-emerald-400">{stats.coverageRate}%</span>
                </div>
                <div className="flex justify-between items-center py-3 border-b border-white/10">
                  <span className="text-slate-400 italic">Maillage Sites</span>
                  <span className="text-blue-400">100%</span>
                </div>
                <div className="flex justify-between items-center py-3">
                  <span className="text-slate-400 italic">Flux de Modification</span>
                  <span className="text-amber-400">+{stats.recentChanges}</span>
                </div>
              </div>
            </div>
          </div>

          {/* REGISTRE DES ANOMALIES STRUCTURELLES */}
          <div className="bg-white rounded-[4rem] p-10 border border-slate-100 shadow-2xl">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-xl font-black uppercase italic text-slate-900 flex items-center gap-3 tracking-tighter">
                <AlertCircle className="text-rose-600" size={24} />
                Anomalies Matrix
              </h3>
              <span className="bg-rose-100 text-rose-700 text-[10px] font-black px-3 py-1.5 rounded-xl uppercase">
            ...
            <div className="space-y-6">
              {stats.alerts.map((alert) => (
                <div 
                  key={alert.id} 
                  className={`p-6 rounded-4xl border-l-8 shadow-sm transition-all hover:scale-[1.02] ${
                    alert.type === 'danger' ? 'bg-rose-50 border-rose-600' :
                    alert.type === 'warning' ? 'bg-amber-50 border-amber-600' :
                    'bg-blue-50 border-blue-600'
                  }`}
                >
                  <div className="flex justify-between items-start mb-3">
                    <span className={`text-[8px] font-black uppercase px-2 py-1 rounded-md ${
                      alert.type === 'danger' ? 'bg-rose-200 text-rose-900' :
                      alert.type === 'warning' ? 'bg-amber-200 text-amber-900' :
                      'bg-blue-200 text-blue-900'
                    }`}>
                      {alert.type === 'danger' ? 'Critique' : alert.type === 'warning' ? 'Alerte' : 'Info'}
                    </span>
                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{alert.unit}</span>
                  </div>
                  <p className="text-sm font-black text-slate-800 italic mb-4 leading-tight uppercase">
                    {alert.message}
                  </p>
                  <button className="text-[10px] font-black uppercase tracking-widest text-slate-600 hover:text-slate-900 flex items-center gap-2 transition-all border-none bg-transparent cursor-pointer">
            ...
            {stats.alerts.length === 0 && (
              <div className="py-16 text-center">
                ...
                <p className="text-[11px] font-black uppercase italic text-slate-400 tracking-widest">Architecture Scellée & Stable</p>
              </div>
            )}
          </div>

          {/* ACCÈS RAPIDES SOUVERAINS */}
          <div className="bg-linear-to-br from-blue-700 to-blue-900 rounded-[4rem] p-10 text-white shadow-2xl">
            <h3 className="text-[11px] font-black uppercase mb-8 tracking-[0.4em] text-blue-200 italic text-center">
            ...
            <div className="space-y-4">
              ...
              <QuickAction icon={FolderTree} label="Rectifier l&apos;Organigramme" onClick={() => router.push('/dashboard/organization/chart')} />
            </div>
          </div>
      ...
      <footer className="mt-20 pt-10 border-t border-slate-200 text-center">
        <p className="text-[9px] font-black uppercase tracking-[0.6em] text-slate-300 italic">
          Qualisoft Elite Sovereign Infrastructure — Protocol v2.4.2 — 2026
        </p>
      </footer>
    </div>
  );
...
function KPICard({ icon: Icon, label, value, trend, trendUp, color, subtext }: {
  ...
  return (
    <div className="bg-white rounded-[3rem] p-8 shadow-xl border border-white hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group">
      <div className="flex justify-between items-start mb-6">
        <div className={`w-16 h-16 rounded-3xl flex items-center justify-center ${colors[color]} group-hover:scale-110 transition-transform duration-500`}>
          <Icon size={32} />
        </div>
        <div className={`flex items-center gap-2 text-[9px] font-black uppercase px-3 py-1.5 rounded-full ${
      ...
      <div className="space-y-2">
        ...
        <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">{subtext}</p>
      </div>
    </div>
  );
}
...
function QuickAction({ icon: Icon, label, onClick }: {
  ...
  return (
    <button 
      onClick={onClick}
      className="w-full p-5 bg-white/5 hover:bg-white/10 rounded-2xl flex items-center gap-4 transition-all group text-left border border-white/5 hover:border-white/20 outline-none cursor-pointer"
    >
      <div className="w-12 h-12 bg-white/10 rounded-xl flex items-center justify-center group-hover:bg-blue-600 group-hover:text-white transition-all duration-500">
        <Icon size={20} />
      </div>
