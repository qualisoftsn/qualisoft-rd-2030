...
export default function OrganizationDashboard() {
  ...
  const globalHealth = useMemo(() => {
    ...
    const score = Math.round(
      (stats.coverageRate + (stats.totalUsers > 0 ? 100 : 0)) / 2,
    );
    return score;
  }, [stats]);
    ...
    <div className="p-6 lg:p-10 space-y-10 bg-slate-50 min-h-screen italic font-sans text-left pb-24 selection:bg-blue-100">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
        <KPICard
          ...
          trendUp={true}
          color="blue"
          subtext="Niveaux hiérarchiques"
        />
        <KPICard
          icon={MapPin}
          label="Sites Gérés"
          value={stats.totalSites}
          trend="Couverture 100%"
          trendUp={true}
          color="emerald"
        ...
        <KPICard
          ...
          trendUp={true}
          color="amber"
          subtext="Inscrits dans le SMI"
        />
        <KPICard
          icon={Target}
          label="Conformité"
          value={`${stats.coverageRate}%`}
          trend={stats.coverageRate >= 85 ? "Stable" : "Alerte"}
          trendUp={stats.coverageRate >= 85}
          color="purple"
          subtext="Unités avec leadership"
        />
      </div>
        ...
        <div className="lg:col-span-2 space-y-10 text-left">
          <div className="bg-white rounded-[4rem] p-10 shadow-2xl border border-white relative overflow-hidden">
            ...
            <div className="relative z-10 mb-10 text-left">
              <h2 className="text-3xl font-black uppercase italic text-slate-900 tracking-tighter flex items-center gap-4">
                <Layers className="text-blue-600" size={32} /> Pyramide
                Structurelle
              </h2>
              <p className="text-[10px] font-black uppercase text-slate-400 tracking-[0.3em] mt-2 italic text-left">
                ...
                <div key={idx} className="group text-left">
                  <div className="flex justify-between items-end mb-3">
                    <span className="text-[11px] font-black text-slate-900 uppercase italic flex items-center gap-3">
                      <div
                        className={`w-4 h-4 rounded-full ${item.color} shadow-lg shadow-black/10`}
                      />{" "}
                      {item.type}
                    </span>
                  ...
                  <div className="h-4 bg-slate-100 rounded-full overflow-hidden border border-slate-50 p-1">
                    <div
                      ...
                      style={{
                        width: `${(item.count / stats.totalUnits) * 100}%`,
                      }}
                    />
                  </div>
                </div>
            ...
            <div className="mt-12 pt-8 border-t border-slate-50 flex justify-between items-center text-left">
              <div className="flex items-center gap-3 text-[10px] font-black uppercase text-slate-400">
                <ShieldCheck size={16} className="text-emerald-500" /> Intégrité
                du SMI : Opérationnelle
              </div>
              <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-left">
            ...
            {[
              ...
              <div
                key={idx}
                className="bg-white rounded-[2.5rem] p-8 border border-white shadow-xl hover:shadow-2xl hover:border-blue-600/20 transition-all group cursor-pointer text-left"
                onClick={() => router.push(`/dashboard/sites/${idx + 1}`)}
              >
                <div className="flex justify-between items-start mb-6">
                  <div
                    className={`w-12 h-12 rounded-2xl flex items-center justify-center ${site.status === "actif" ? "bg-emerald-50 text-emerald-600" : "bg-amber-50 text-amber-600"}`}
                  >
                    <Building2 size={24} />
                  </div>
                  {site.status === "warning" && (
                ...
                <h3 className="text-xl font-black uppercase italic text-slate-900 mb-2 leading-tight text-left">
                  {site.name}
                </h3>
        ...
        <div className="space-y-10 text-left">
          <div className="bg-slate-900 rounded-[4rem] p-10 text-white relative overflow-hidden shadow-2xl group text-left">
            ...
            <div className="relative z-10 text-left">
              <h3 className="text-[10px] font-black uppercase mb-4 tracking-[0.4em] text-slate-400 italic text-left">
              ...
              <div className="flex items-end gap-3 mb-8">
                ...
                <span className="text-xs font-black text-emerald-400 mb-1 flex items-center gap-1 uppercase italic">
          ...
          <div className="bg-white rounded-[4rem] p-10 border border-slate-100 shadow-2xl text-left">
            <div className="flex justify-between items-center mb-8">
              <h3 className="text-xl font-black uppercase italic text-slate-900 flex items-center gap-3 tracking-tighter">
                <AlertCircle className="text-rose-600" size={24} /> Anomalies
                Matrix
              </h3>
            ...
            <div className="space-y-6">
              {stats.alerts.map((alert) => (
                <div
                  key={alert.id}
                  className={`p-6 rounded-4xl border-l-8 shadow-sm transition-all hover:scale-[1.02] text-left ${alert.type === "danger" ? "bg-rose-50 border-rose-600" : alert.type === "warning" ? "bg-amber-50 border-amber-600" : "bg-blue-50 border-blue-600"}`}
                >
                  <div className="flex justify-between items-start mb-3">
                    <span
                      className={`text-[8px] font-black uppercase px-2 py-1 rounded-md ${alert.type === "danger" ? "bg-rose-200 text-rose-900" : alert.type === "warning" ? "bg-amber-200 text-amber-900" : "bg-blue-200 text-blue-900"}`}
                    >
                      {alert.type === "danger"
                        ? "Critique"
                        : alert.type === "warning"
                  ...
                  <p className="text-sm font-black text-slate-800 italic mb-4 leading-tight uppercase text-left">
                    {alert.message}
                  </p>
          ...
          <div className="bg-linear-to-br from-blue-700 to-blue-900 rounded-[4rem] p-10 text-white shadow-2xl text-left">
            ...
            <div className="space-y-4">
              <QuickAction
                icon={Plus}
                label="Valider une Unité"
                onClick={() => router.push("/dashboard/organization/units/new")}
              />
              <QuickAction
                icon={UserCircle}
                label="Enrôler des Citoyens"
                onClick={() => router.push("/dashboard/users/assign")}
              />
              <QuickAction
                icon={Globe}
                label="Déployer un Site"
                onClick={() => router.push("/dashboard/sites")}
              />
              <QuickAction
                icon={FolderTree}
                label="Rectifier l'Arbre"
                onClick={() => router.push("/dashboard/organization/chart")}
              />
            </div>
          </div>
...
function KPICard({
  ...
  return (
    <div className="bg-white rounded-[3rem] p-8 shadow-xl border border-white hover:shadow-2xl hover:-translate-y-2 transition-all duration-500 group text-left">
      <div className="flex justify-between items-start mb-6">
        <div
          className={`w-16 h-16 rounded-3xl flex items-center justify-center ${colors[color]} group-hover:scale-110 transition-transform duration-500`}
        >
          <Icon size={32} />
        </div>
        <div
          className={`flex items-center gap-2 text-[9px] font-black uppercase px-3 py-1.5 rounded-full italic ${trendUp ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}
        >
...
function QuickAction({ icon: Icon, label, onClick }: any) {
  return (
    ...
    >
      ...
      <span className="text-[11px] font-black uppercase italic tracking-widest text-white group-hover:translate-x-1 transition-transform">
        {label}
      </span>
      <ChevronRight
        size={16}
        className="ml-auto opacity-30 group-hover:opacity-100 group-hover:translate-x-1 transition-all"
      />
    </button>
  );
}
