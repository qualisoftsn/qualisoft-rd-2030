...
import { 
  ...
  Settings, Layers, CheckCircle, 
  Loader2, Activity, ShieldCheck, Info, XCircle
} from 'lucide-react';
import { toast, Toaster } from 'sonner';
import { ProcessType, ProcessFamily } from '@/types/elite-sde';
...
export default function ProcessTypePage() {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("⚠️ Action critique : Supprimer ce type de processus ?")) return;
    ...
    try {
      ...
      toast.error("Échec : Le type est probablement lié à des processus actifs.", { id: toastId }); 
    }
  };
    ...
    <div className="ml-72 min-h-screen bg-[#0B0F1A] text-white italic font-sans p-10 selection:bg-blue-600/30">
      ...
      <header className="flex justify-between items-end mb-16 border-b border-white/5 pb-10 animate-in slide-in-from-top-4 duration-700">
        <div className="space-y-4">
          <div className="flex items-center gap-3 text-blue-500 bg-blue-500/5 w-fit px-4 py-1 rounded-full border border-blue-500/10">
            <ShieldCheck size={14} />
            <span className="text-[9px] font-black uppercase tracking-widest text-left">SDE Mapping Active</span>
          </div>
        ...
        <div className="flex gap-4">
          <button onClick={loadTypes} className="p-5 bg-white/5 rounded-2xl text-slate-400 hover:text-white transition-all border border-white/5 cursor-pointer">
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center h-96">
          <Loader2 className="animate-spin text-blue-600 mb-8" size={60} strokeWidth={1.5} />
          <p className="font-black uppercase text-[11px] tracking-[0.6em] italic text-slate-500 animate-pulse">Scanning SDE Architecture...</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-10 animate-in fade-in slide-in-from-bottom-6 duration-700">
          {types.length > 0 ? types.map((type) => {
            ...
            return (
              <div key={type.PT_Id} className="bg-[#0F172A]/40 border border-white/5 rounded-[3rem] p-10 hover:border-blue-600/40 transition-all group relative overflow-hidden shadow-2xl">
                ...
                <div className="flex justify-between items-start mb-10 relative z-10">
                  ...
                  <div className="flex gap-3 opacity-0 group-hover:opacity-100 transition-all translate-x-4 group-hover:translate-y-0">
                    <button onClick={() => { setEditingType(type); setShowModal(true); }} className="p-4 bg-white/5 rounded-xl text-slate-400 hover:text-blue-400 hover:bg-blue-400/10 transition-all border-none cursor-pointer"><Edit size={20}/></button>
                    <button onClick={() => handleDelete(type.PT_Id)} className="p-4 bg-white/5 rounded-xl text-slate-400 hover:text-red-500 hover:bg-red-500/10 transition-all border-none cursor-pointer"><Trash2 size={20}/></button>
                  </div>
                </div>

                <div className="relative z-10 text-left">
                  <h3 className="text-2xl font-black uppercase italic mb-3 tracking-tighter group-hover:text-blue-400 transition-colors">{type.PT_Label}</h3>
                  <div className="flex items-center gap-3 mb-8">
                     <div className="bg-blue-600/10 border border-blue-500/20 px-4 py-1.5 rounded-full">
                        <span className="text-[10px] font-black uppercase text-blue-400 italic tracking-[0.2em]">{type.PT_Family}</span>
                     </div>
                  </div>
                  ...
                  <div className="flex items-center justify-between border-t border-white/5 pt-8">
                    <div className="flex items-center gap-4">
                      ...
                      <div className="w-5 h-5 rounded-lg border border-white/10" style={{ backgroundColor: safeColor }}></div>
                      <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest italic">{safeColor}</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <Activity size={14} className={type.PT_IsActive ? 'text-emerald-500' : 'text-red-500'} />
                      <span className={`text-[10px] font-black uppercase italic tracking-tighter ${type.PT_IsActive ? 'text-emerald-500' : 'text-red-500'}`}>
            ...
            <div className="col-span-full py-32 text-center border-4 border-dashed border-white/5 rounded-[4rem] opacity-20 italic">
               ...
               <p className="font-black uppercase tracking-[0.5em] text-sm">Néant Structurel — ISO §4.4</p>
            </div>
          )}
        </div>
      )}
 ...
 */
function ProcessTypeModal({ type, onClose, onSuccess }: { type: ProcessType | null; onClose: () => void; onSuccess: () => void }) {
  ...
  return (
    <div className="fixed inset-0 bg-black/98 backdrop-blur-2xl z-100 flex items-center justify-center p-8 animate-in fade-in duration-300">
      <div className="bg-[#0B0F1A] border border-white/10 rounded-[4rem] max-w-2xl w-full p-16 shadow-2xl italic font-bold text-left relative overflow-hidden">
        ...
        <div className="absolute top-0 right-0 p-16 opacity-5 pointer-events-none rotate-12"><Settings size={220} /></div>
        ...
        <div className="flex items-center justify-between mb-16 relative z-10">
          <div className="flex items-center gap-6 text-left">
            ...
            <div>
              ...
              <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mt-2">Architecture SDE — ISO 9001 §4.4</p>
            </div>
          </div>
          <button onClick={onClose} className="p-4 text-slate-500 hover:text-white transition-colors cursor-pointer border-none bg-transparent">
             <XCircle size={48} strokeWidth={1} />
          </button>
        </div>
        
        <form onSubmit={handleSubmit} className="space-y-10 relative z-10 text-left font-sans">
          <div className="space-y-4">
            <label className="text-[10px] text-slate-500 uppercase tracking-[0.4em] ml-6 italic">Désignation du Segment *</label>
            <input 
              required 
          ...
          <div className="grid grid-cols-2 gap-10">
            <div className="space-y-4">
              <label className="text-[10px] text-slate-500 uppercase tracking-[0.4em] ml-6 italic">Classification *</label>
              <select 
                ...
                onChange={e => setFormData({...formData, PT_Family: e.target.value as ProcessFamily})}
              >
            ...
            <div className="space-y-4">
              <label className="text-[10px] text-slate-500 uppercase tracking-[0.4em] ml-6 italic">Identité Visuelle</label>
              <div className="flex gap-4">
                <input 
                  type="color" 
          ...
          <div className="space-y-4">
            <label className="text-[10px] text-slate-500 uppercase tracking-[0.4em] ml-6 italic">Scope SMI</label>
            <textarea 
              rows={4} 
