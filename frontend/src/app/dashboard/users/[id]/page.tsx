...
import { 
  ...
  Building, Fingerprint, Activity, Lock} from 'lucide-react';
...
export default function EditUserPage() {
  ...
  const loadUser = useCallback(async () => {
    ...
    // eslint-disable-next-line @typescript-eslint/no-unused-vars
      ...
      toast.error("RUPTURE SDE : Échec de lecture du dossier.");
    } finally {
      setLoading(false);
    }
  }, [id]);
  ...
  return (
    <div className="ml-72 h-screen bg-[#0B0F1A] text-white p-10 italic font-sans flex flex-col overflow-hidden">
      ...
      <header className="flex justify-between items-center border-b border-white/10 pb-6 mb-8 shrink-0">
        ...
        <button onClick={() => router.push('/dashboard/users')} className="flex items-center gap-3 text-slate-400 hover:text-white transition-all text-[10px] font-black uppercase italic border border-white/10 px-6 py-3 rounded-2xl bg-white/5 shadow-xl">
        ...
        <div className="grid grid-cols-2 gap-x-12 gap-y-10">
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-6">
              <input value={user?.U_FirstName || ''} onChange={e => setUser({...user, U_FirstName: e.target.value})} placeholder="Prénom" className="bg-white/5 border border-white/10 rounded-2xl p-5 text-sm font-bold italic outline-none focus:border-blue-500" />
              <input value={user?.U_LastName || ''} onChange={e => setUser({...user, U_LastName: e.target.value})} placeholder="Nom" className="bg-white/5 border border-white/10 rounded-2xl p-5 text-sm font-bold italic outline-none focus:border-blue-500" />
            </div>
          </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3 text-indigo-500 mb-2">
              <Lock size={18} />
              <span className="text-[11px] font-black uppercase tracking-widest italic">Sécurité</span>
            </div>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3 text-emerald-500 mb-2">
              <ShieldCheck size={18} />
              <span className="text-[11px] font-black uppercase tracking-widest italic">Rôle Matrix</span>
            </div>
            <select value={user?.U_Role || 'USER'} onChange={e => setUser({...user, U_Role: e.target.value})} className="bg-[#151A2D] border border-white/10 rounded-2xl p-5 text-sm font-bold text-white italic outline-none focus:border-blue-500">
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center gap-3 text-amber-500 mb-2">
              <Activity size={18} />
              <span className="text-[11px] font-black uppercase tracking-widest italic">Configuration ISO</span>
            </div>
            <div className="grid grid-cols-2 gap-6">
               <div className="bg-white/5 border border-white/5 rounded-2xl p-5 text-sm font-bold text-slate-500 italic flex items-center gap-3">
                 <Building size={16} /> {user?.U_Site?.S_Name || 'Master Site'}
               </div>
               <select value={user?.U_IsActive ? "true" : "false"} onChange={e => setUser({...user, U_IsActive: e.target.value === "true"})} className="bg-[#151A2D] border border-white/10 rounded-2xl p-5 text-sm font-bold text-white italic outline-none focus:border-blue-500">
        ...
        <div className="mt-auto pt-10 border-t border-white/5 flex gap-6 items-center">
          <div className="flex-1 flex items-center gap-4 text-slate-600">
             ...
             <div className="flex flex-col leading-none">
               ...
               <span className="text-[7px] italic">UUID Scellé : {user?.U_Id}</span>
             </div>
          </div>
          <button disabled={saving} type="submit" className="bg-blue-600 hover:bg-white hover:text-blue-600 px-16 py-6 rounded-4xl font-black uppercase italic tracking-[0.4em] transition-all flex items-center justify-center gap-6 shadow-3xl cursor-pointer active:scale-95">
