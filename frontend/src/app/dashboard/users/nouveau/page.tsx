...
export default function NewUserPage() {
  ...
  const filteredOrgUnits = useMemo(() => {
    ...
    return allOrgUnits.filter(
      (unit) => unit.OU_SiteId === formData.U_SiteId && unit.OU_IsActive,
    );
  ...
  const loadReferentials = useCallback(async () => {
    ...
    try {
      ...
      setAllOrgUnits(
        (orgUnitsRes.data || []).filter((u: OrgUnit) => u.OU_IsActive),
      );
      setProcesses(
        (processesRes.data || []).filter((p: Processus) => p.PR_IsActive),
      );
    } catch (error: any) {
  ...
  useEffect(() => {
    setFormData((prev) => ({ ...prev, U_AssignedProcessId: "" }));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(
        `Habilitation scellée pour ${formData.U_FirstName} ${formData.U_LastName}`,
      );
      setFormData(initialFormState);
    } catch (err: any) {
      toast.error(
        err.response?.data?.message || "Erreur d'habilitation critique",
      );
    } finally {
    ...
    <div className="p-10 bg-[#0B0F1A] min-h-screen ml-72 text-white italic font-sans text-left overflow-x-hidden">
      ...
      <div className="mb-12 grid grid-cols-1 md:grid-cols-3 gap-6">
        {[
          ...
          {
            label: "Unités Libres",
            val: allOrgUnits.length,
            icon: Layers,
            color: "text-amber-500",
          },
          {
            label: "Processus Pilotés",
            val: processes.length,
            icon: GitBranch,
          ...
          <div
            key={i}
            className="bg-white/5 border border-white/5 p-8 rounded-4xl flex items-center justify-between shadow-2xl"
          >
            <div
              className={`p-4 rounded-2xl bg-white/5 ${s.color} border border-white/5`}
            >
              <s.icon size={28} />
            </div>
            <div className="text-right">
              ...
              <p className="text-[10px] uppercase text-slate-600 font-black">
                {s.label}
              </p>
            </div>
          </div>
      ...
      <form
        ...
        className="grid grid-cols-1 lg:grid-cols-12 gap-12 animate-in fade-in slide-in-from-bottom-6 duration-1000"
      >
            ...
            <div className="space-y-6">
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input
                    required
                    className="w-full bg-black/40 border border-white/10 rounded-2xl p-5 text-sm font-black text-white uppercase italic focus:border-blue-500 outline-none transition-all shadow-inner"
                    placeholder="EX: JEAN"
                    value={formData.U_FirstName}
                    onChange={(e) =>
                      setFormData({ ...formData, U_FirstName: e.target.value })
                    }
                  />
                </div>
                <div className="space-y-2">
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      setFormData({ ...formData, U_LastName: e.target.value })
                    }
                  />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <input
                  required
                  type="email"
                  className="w-full bg-black/40 border border-white/10 rounded-2xl p-5 text-sm font-bold text-white italic focus:border-blue-500 outline-none transition-all shadow-inner"
                  placeholder="prenom.nom@qualisoft.sn"
              ...
              <div className="bg-amber-600/5 border border-amber-600/20 p-8 rounded-[2.5rem] mt-10">
                <label className="text-[10px] font-black text-amber-500 uppercase ml-2 italic flex items-center gap-2">
                  🔐 Clé Master Provisoire
                </label>
                <input
                  readOnly
                  className="w-full bg-black/40 border border-white/5 mt-4 rounded-xl p-5 text-sm font-black text-amber-500 italic cursor-not-allowed"
                  value={formData.U_Password}
                />
                <p className="text-[9px] text-slate-600 mt-4 italic font-bold">
                  L&apos;agent devra Valider son propre mot de passe lors de la
                  première synchronisation cockpit.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-slate-900/40 border border-white/5 p-12 rounded-[4.5rem] shadow-4xl backdrop-blur-3xl space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
              <div className="space-y-3">
                ...
                <select
                  required
                  className="w-full bg-black/40 border border-white/10 p-5 rounded-2xl text-[11px] font-black text-white italic outline-none focus:border-emerald-500 shadow-inner cursor-pointer"
                  value={formData.U_Role}
                  onChange={(e) =>
                    setFormData({ ...formData, U_Role: e.target.value })
                  }
                >
              ...
              <div className="space-y-3">
                ...
                <select
                  required
                  className="w-full bg-black/40 border border-white/10 p-5 rounded-2xl text-[11px] font-black text-white italic outline-none focus:border-blue-500 shadow-inner cursor-pointer"
                  value={formData.U_SiteId}
                  onChange={(e) =>
                    setFormData({ ...formData, U_SiteId: e.target.value })
                  }
                >
              ...
              <div className="space-y-3">
                ...
                <select
                  ...
                  className="w-full bg-black/40 border border-white/10 p-5 rounded-2xl text-[11px] font-black text-white italic outline-none focus:border-amber-500 disabled:opacity-30 cursor-pointer shadow-inner"
                  value={formData.U_OrgUnitId}
                  onChange={(e) =>
                    setFormData({ ...formData, U_OrgUnitId: e.target.value })
                  }
                >
              ...
              <div className="space-y-3">
                <label
                  className={`text-[10px] font-black uppercase ml-2 italic flex items-center gap-2 ${formData.U_Role === "PILOTE" ? "text-blue-500 animate-pulse" : "text-slate-500"}`}
                >
                  Affectation Cockpit *
                </label>
                <select
                  ...
                  className="w-full bg-black/40 border border-white/10 p-5 rounded-2xl text-[11px] font-black text-white italic outline-none focus:border-blue-600 disabled:opacity-20 cursor-pointer shadow-inner"
                  value={formData.U_AssignedProcessId}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      U_AssignedProcessId: e.target.value,
                    })
                  }
                >
            ...
            <div className="pt-12 flex flex-col items-center gap-8">
              <button
                ...
                className="w-full bg-blue-600 hover:bg-blue-500 text-white py-8 rounded-[2.5rem] font-black uppercase text-xs italic shadow-4xl flex items-center justify-center gap-4 transition-all active:scale-95 disabled:opacity-50 border-none cursor-pointer"
              >
                {isSubmitting ? (
                ...
                Confirmer l&apos;Habilitation Maître
              </button>
              <div className="flex items-center gap-3 opacity-30 italic">
                <ShieldAlert size={16} />
                <span className="text-[10px] font-black uppercase tracking-widest text-slate-500">
                  Action irréversible • Journal Audit Qualité Actif
                </span>
              </div>
            </div>
          </div>
