 ...
 * - CRUD SDE : Redirection sécurisée pour modification et révocation active.
 * -------------------------------------------------------------------------
 * DESIGN : Elite High-Density / No-Scroll / Full-Viewport Isolation Absolue.
 * -------------------------------------------------------------------------
 */
...
// Interface alignée strictement sur types/elite-sde.ts
interface User {
  U_Id: string; 
  U_FirstName: string; 
  U_LastName: string; 
...
export default function UsersPage() {
  ...
  /**
   * 🛡️ GESTIONNAIRE DE ROUTAGE SÉCURISÉ (Empêche le crash silencieux Next.js)
   */
  const goToDetails = (user: User) => {
    // Sécurité : Vérifie si le backend a renvoyé 'id' au lieu de 'U_Id'
    const targetId = user.U_Id || (user as any).id;
    
    if (!targetId) {
      toast.error("ERREUR D'INTÉGRITÉ : L'identifiant SDE de cet agent est manquant ou corrompu.");
      return;
    }
    router.push(`/dashboard/users/${targetId}`);
  };
  ...
  return (
    // 🧱 STRUCTURE FULL-VIEWPORT ABSOLUE (ml-72 pour la sidebar, w-full h-full pour occuper tout l'écran)
    <div className="ml-72 h-screen flex flex-col bg-[#0B0F1A] text-white italic font-sans overflow-hidden box-border">
      <div className="flex-1 w-full h-full flex flex-col p-8">
        ...
        {/* 📋 REGISTRE RACI (La zone qui occupe 100% de l'espace restant) */}
          ...
          <div className="flex-1 overflow-y-auto custom-scrollbar relative z-10 w-full">
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-white/5 text-[11px]">
                {filtered.map(user => (
                  <tr key={user.U_Id || (user as any).id} className={`group hover:bg-blue-600/5 transition-all ${!user.U_IsActive ? 'opacity-30 grayscale' : ''}`}>
                    ...
                    <td className="px-8 py-4 text-center">
                      <span className={`px-3 py-1 rounded-lg border text-[9px] font-black uppercase italic ${
                        ...
                        {user.U_Role ? user.U_Role.replace('_', ' ') : 'USER'}
                      </span>
                    </td>
                    <td className="px-8 py-4">
                    ...
                    <td className="px-8 py-4 text-right">
                      {/* 🛠️ BOUTONS TOUJOURS VISIBLES (Retrait du opacity-0 piège) */}
                      <div className="flex justify-end gap-3 transition-all">
                        <button 
                          ...
                          title="Modifier le dossier de l'agent"
                        >
                          <Edit size={18}/>
                        </button>
                        <button 
                          disabled={isDeleting === (user.U_Id || (user as any).id)}
                          onClick={() => handleDelete(user.U_Id || (user as any).id)}
                        ...
                        >
                          {isDeleting === (user.U_Id || (user as any).id) ? <Loader2 size={18} className="animate-spin" /> : <Trash2 size={18}/>}
                        </button>
                      </div>
                    </td>
                  </tr>
