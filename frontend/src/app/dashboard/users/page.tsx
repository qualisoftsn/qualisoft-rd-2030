    ...
    <div className="ml-72 h-screen bg-[#0B0F1A] text-white italic font-sans flex flex-col p-6 overflow-hidden">
      ...
      <header className="flex justify-between items-center border-b border-white/10 pb-4 mb-6 shrink-0 w-full">
        ...
        <div className="flex gap-4">
          {!isSubdomain && (
            <button onClick={() => router.push('/dashboard/matrix-control')} className="bg-white/5 hover:bg-white/10 px-6 py-2 rounded-xl text-[9px] font-black uppercase border border-white/10 transition-all flex items-center gap-3 italic text-slate-400 cursor-pointer">
      ...
      <div className="flex gap-4 mb-6 shrink-0 w-full">
        ...
        {!isSubdomain && tenants.length > 0 && (
          <div className="w-1/3 relative">
            <select value={selectedTenant} onChange={(e) => setSelectedTenant(e.target.value)} className="w-full h-full bg-blue-600/10 border border-blue-500/20 rounded-2xl py-4 pl-6 pr-12 text-[11px] font-black uppercase outline-none focus:border-blue-500 focus:bg-blue-600/20 transition-all italic tracking-wider appearance-none cursor-pointer text-blue-400">
              <option value="" className="bg-[#151A2D] text-white">🌍 TOUS LES TENANTS (MASTER VIEW)</option>
              {tenants.map((t: any) => <option key={t.T_Id || t.id} value={t.T_Id || t.id} className="bg-[#151A2D] text-white">{t.T_Name || t.name}</option>)}
            </select>
      ...
      <style jsx global>{`.custom-scrollbar::-webkit-scrollbar { width: 4px; } .custom-scrollbar::-webkit-scrollbar-track { background: transparent; } .custom-scrollbar::-webkit-scrollbar-thumb { background: rgba(37, 99, 235, 0.1); border-radius: 10px; }`}</style>
    </div>
  );
}
    ...
    <div className={`p-5 rounded-3xl border flex items-center justify-between shadow-2xl transition-all group w-full ${c[color]}`}>
      <div className="flex items-center gap-4">
        ...
        <div className="flex flex-col">
          ...
          <span className="text-[7px] font-bold text-slate-600 uppercase mt-0.5 italic">{sub}</span>
        </div>
      </div>
