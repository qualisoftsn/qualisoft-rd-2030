...
import { 
  ...
  AlertTriangle, FolderTree, Briefcase, Boxes, Activity, 
  ShieldCheck, Rocket, Hash, Globe, Fingerprint
} from 'lucide-react';
...
export default function OrgUnitsPage() {
  ...
  const [selectedUnit, setSelectedUnit] = useState<ExtendedOrgUnit | null>(null);
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormData({ OU_Name: '', OU_Code: '', OU_TypeId: '', OU_SiteId: '', OU_ParentId: null });
      fetchData();
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    } catch (err: any) {
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      const payload = {
        ...
        OU_ParentId: formData.OU_ParentId === "" || formData.OU_ParentId === "null" ? null : formData.OU_ParentId
      };
      await apiClient.patch(`/org-units/${selectedUnit.OU_Id}`, payload);
      toast.success("Mutation validée.");
      setIsEditModalOpen(false);
      fetchData();
    } catch (err: unknown) { toast.error("Échec de la mutation."); } finally { setSubmitting(false); }
  };
  ...
  const handleArchive = async () => {
    ...
    try {
      ...
      fetchData();
    } catch (err: unknown) { toast.error("Rupture : Dépendances actives détectées."); } finally { setSubmitting(false); }
  };
  ...
  const renderUnitTree = (unitsList: ExtendedOrgUnit[], level = 0) => {
    return unitsList
      .filter(u => {
        ...
        return u.OU_Name.toLowerCase().includes(query) || (u.OU_Code?.toLowerCase() || "").includes(query);
      })
      .map((unit) => {
        ...
        return (
          <div key={unit.OU_Id} className="select-none animate-in fade-in duration-300 text-left">
            <div 
            ...
            >
              <div className="flex items-center gap-5 flex-1">
                ...
                <div className={`p-3 rounded-2xl ${hasUsers ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
                  <Building2 size={22} />
                </div>

                <div className="flex-1">
                  <div className="flex items-center gap-3">
                    <h3 className="text-[15px] font-black uppercase italic text-slate-900 tracking-tight leading-none">{unit.OU_Name}</h3>
                    {unit.OU_Code && <span className="text-[9px] font-black bg-slate-100 text-slate-500 px-2 py-0.5 rounded-md border border-slate-200">{unit.OU_Code}</span>}
                  </div>
                  <p className="text-[10px] font-bold text-slate-400 uppercase italic tracking-[0.2em] mt-2">
                    <span className="text-blue-500/60">[{unit.OU_Type?.OUT_Label}]</span> — {unit.OU_Site?.S_Name}
                  </p>
                </div>

                <div className="flex items-center gap-3 opacity-0 group-hover:opacity-100 transition-all">
                  <button type="button" onClick={() => { setSelectedUnit(unit); setIsDetailModalOpen(true); }} className="p-3 bg-white border border-slate-200 text-slate-400 hover:text-blue-600 rounded-xl cursor-pointer shadow-sm transition-all"><Search size={18} /></button>
                  ...
                  }} className="p-3 bg-white border border-slate-200 text-slate-400 hover:text-amber-600 rounded-xl cursor-pointer shadow-sm transition-all"><Edit3 size={18} /></button>
                  <button type="button" onClick={() => { setSelectedUnit(unit); setIsDeleteModalOpen(true); }} className="p-3 bg-white border border-slate-200 text-slate-400 hover:text-red-600 rounded-xl cursor-pointer shadow-sm transition-all"><Archive size={18} /></button>
                </div>
              </div>
            </div>
  ...
  if (loading) return (
    <div className="flex h-screen bg-slate-50 flex-col items-center justify-center gap-6">
      ...
      <p className="font-black italic text-blue-600 uppercase tracking-[0.6em] animate-pulse">Synchronisation Structurelle...</p>
    </div>
  );
    ...
    <div className="p-8 space-y-10 bg-slate-50 min-h-screen italic font-sans font-bold text-left selection:bg-blue-600/20">
      ...
      <header className="flex justify-between items-end border-b-4 border-slate-100 pb-10 animate-in slide-in-from-top-4 duration-700">
        <div>
          <div className="flex items-center gap-4 text-blue-600 mb-4 bg-blue-50 w-fit px-4 py-1.5 rounded-full border border-blue-200">
            ...
            <span className="text-[9px] font-black uppercase tracking-widest">SDE Matrix Isolation Active</span>
          </div>
          ...
          <p className="text-slate-500 font-black text-[11px] uppercase tracking-[0.6em] mt-6 italic opacity-60">Hiérarchie Elite — ISO 9001 §5.3</p>
        </div>
        <button onClick={() => fetchData()} className="px-12 py-6 bg-white border-2 border-slate-200 rounded-4xl font-black uppercase text-[12px] text-slate-600 hover:bg-slate-50 hover:border-blue-600/30 transition-all flex items-center gap-5 shadow-xl italic cursor-pointer active:scale-95 border-none">
          <GitGraph size={24} className="text-blue-600" /> Rafraîchir l&apos;Arbre
        </button>
      </header>
      ...
      {units.length === 0 && !loading ? (
        <div className="bg-white rounded-[5rem] p-32 text-center border-4 border-dashed border-slate-200 shadow-4xl animate-in fade-in zoom-in-95 max-w-5xl mx-auto mt-20 relative overflow-hidden">
          ...
          <Rocket size={80} className="text-blue-600 mx-auto mb-12 animate-bounce" />
          <h2 className="text-6xl font-black uppercase text-slate-900 mb-8 tracking-tighter italic leading-none">Néant Structurel</h2>
          <button onClick={handleAutoInit} disabled={submitting} className="bg-blue-600 hover:bg-blue-700 text-white px-16 py-8 rounded-[2.5rem] font-black uppercase text-xs tracking-[0.4em] shadow-3xl transition-all flex items-center gap-6 mx-auto cursor-pointer border-none group active:scale-95">
              {submitting ? <Loader2 className="animate-spin" /> : <ShieldCheck size={32} />} Déployer la Direction Générale
          </button>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 items-start">
          <div className="lg:col-span-4 sticky top-10">
            <div className="bg-white rounded-[4rem] p-14 shadow-4xl border border-slate-100 overflow-hidden relative">
              <div className="absolute top-0 right-0 p-10 opacity-5 -mr-8 -mt-8 rotate-12"><Boxes size={120} /></div>
              <h2 className="text-2xl font-black uppercase text-slate-900 flex items-center gap-5 italic leading-none mb-14 relative z-10">
                <Plus size={32} className="text-blue-600" strokeWidth={3} /> Nouveau Segment
              </h2>
              <form onSubmit={handleCreate} className="space-y-10 relative z-10">
                <div className="space-y-4">
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-6 tracking-[0.2em]">Désignation Unité</label>
                  <input required className="w-full bg-slate-50 border-2 border-slate-100 rounded-3xl px-8 py-6 text-[15px] font-black uppercase italic outline-none focus:border-blue-500 shadow-inner"
                ...
                <div className="space-y-4">
                  <label className="text-[10px] font-black uppercase text-slate-400 ml-6 tracking-[0.2em]">Code Unité</label>
                  <div className="relative">
                    <Hash size={16} className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-300" />
                    <input className="w-full bg-slate-50 border-2 border-slate-100 rounded-3xl pl-14 pr-8 py-6 text-[15px] font-black uppercase italic outline-none focus:border-blue-500 shadow-inner"
                      value={formData.OU_Code} onChange={(e) => setFormData({...formData, OU_Code: e.target.value.toUpperCase()})} placeholder="EX: SL-01" />
                  </div>
                </div>
                <button type="submit" disabled={submitting} className="w-full bg-slate-950 hover:bg-blue-600 text-white font-black uppercase py-8 rounded-4xl shadow-3xl transition-all italic flex items-center justify-center gap-6 cursor-pointer border-none active:scale-95">
                  {submitting ? <Loader2 className="animate-spin" /> : <ShieldCheck size={28} />} Sceller l&apos;Unité
                </button>
              </form>
            </div>
          </div>
            ...
            <div className="bg-white rounded-[5rem] shadow-4xl border border-slate-100 overflow-hidden min-h-200 flex flex-col">
              <div className="p-12 border-b-4 border-slate-50 flex justify-between items-center bg-slate-50/40 backdrop-blur-md">
                <div className="relative w-full max-w-xl group">
                  <Search className="absolute left-8 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-blue-600 transition-colors" size={28} />
                  <input placeholder="FILTRER LE MAILLAGE ORGANIQUE..." className="w-full bg-white border-2 border-slate-100 rounded-[2.5rem] py-6 pl-20 pr-10 text-xs font-black uppercase italic outline-none focus:border-blue-500 shadow-inner"
          ...
          <div className="bg-white rounded-[6rem] w-full max-w-6xl shadow-4xl animate-in slide-in-from-bottom-24 font-black italic overflow-hidden border-none text-left">
            <div className="p-20 border-b-4 border-slate-50 flex justify-between items-center bg-slate-50/40">
              <div className="flex items-center gap-10">
                <div className="w-28 h-28 rounded-[2.5rem] bg-blue-600 flex items-center justify-center text-white shadow-3xl rotate-3"><Briefcase size={56} strokeWidth={2.5} /></div>
                <div>
                  <span className="text-[13px] text-blue-600 uppercase tracking-[0.5em] font-black bg-blue-50 px-6 py-2 rounded-full border border-blue-100">{selectedUnit.OU_Type?.OUT_Label}</span>
                  <h2 className="text-7xl uppercase tracking-tighter text-slate-900 leading-none mt-4">{selectedUnit.OU_Name}</h2>
                </div>
              </div>
              <button onClick={() => setIsDetailModalOpen(false)} className="p-8 hover:bg-slate-200 rounded-full text-slate-400 transition-colors border-none cursor-pointer bg-transparent"><X size={64} strokeWidth={1} /></button>
            </div>
            <div className="p-20 space-y-16">
              <h3 className="text-2xl uppercase text-slate-900 flex items-center gap-6 italic font-black">
                <Users size={40} className="text-blue-600" /> Capital Humain ({selectedUnit._count?.OU_Users || 0})
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-8 max-h-120 overflow-y-auto pr-10 custom-scrollbar p-4">
                {selectedUnit.OU_Users && selectedUnit.OU_Users.length > 0 ? selectedUnit.OU_Users.map((u) => (
                  <div key={u.U_Id} className="flex items-center gap-6 p-8 bg-white border-2 border-slate-100 rounded-[3rem] shadow-sm hover:shadow-xl transition-all group">
                    ...
                    <div className="w-16 h-16 rounded-3xl bg-slate-950 text-white flex items-center justify-center font-black text-sm group-hover:bg-blue-600 transition-colors">
                      {(u.U_FirstName?.charAt(0) || '').toUpperCase()}{(u.U_LastName?.charAt(0) || '').toUpperCase()}
                    </div>
                    <div className="truncate flex-1">
                      <p className="font-black text-slate-900 uppercase truncate text-[16px] tracking-tight mb-2">{u.U_FirstName} {u.U_LastName}</p>
                      <p className="text-blue-600 text-[11px] font-bold italic opacity-70 uppercase tracking-widest">{u.U_Role}</p>
                    </div>
                  </div>
                )) : (
                  <div className="col-span-3 py-32 text-center text-slate-300 border-4 border-dashed border-slate-100 rounded-[5rem]">
                    <Activity size={72} className="mx-auto mb-8 opacity-10 animate-pulse" />
                    <p className="uppercase text-sm font-black tracking-[0.5em] opacity-40 italic">Aucune affectation scellée</p>
                  </div>
                )}
              </div>
            </div>
