...
import { 
  ...
  LayoutGrid, MapPin, Database, ChevronRight, Globe, Server
} from 'lucide-react';
 ...
 * -------------------------------------------------------------------------
 * ARCHITECTURE : CTO-GRADE / PWA / HIGH-DENSITY
 * RELATION : Réflexive (Self-relation) OU_ParentId
 * -------------------------------------------------------------------------
 */
...
export default function OrgUnitsPage() {
  ...
  // État du formulaire strictement aligné sur CreateOrgUnitDto
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      toast.error("RUPTURE KERNEL SDE : Synchronisation impossible."); 
    } finally { 
      setLoading(false); 
    }
  }, []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const payload: any = {
        ...
        // Conversion UUID : String vide -> NULL (Critique pour NestJS Validation)
      ...
      toast.success(isEdit ? "SCELLAGE RÉUSSI" : "DÉPLOYÉ DANS LA MATRIX");
      setShowEditor(false);
      fetchData();
    } catch (err: any) {
      const msg = err.response?.data?.message || "ERREUR DE VALIDATION KERNEL";
  ...
  if (loading) return (
    <div className="h-screen w-full flex items-center justify-center bg-[#0B0F1A] text-blue-500 ml-72">
      <Loader2 className="animate-spin" size={40} />
    </div>
  );

  return (
    <div className="fixed inset-0 ml-72 bg-[#0B0F1A] text-white flex flex-col overflow-hidden font-sans italic border-l border-white/10">
      <Toaster position="top-right" richColors theme="dark" />

      {/* 🔝 HEADER SOUVERAIN (Z-index 50) */}
      <header className="h-24 border-b border-white/5 flex justify-between items-center px-10 shrink-0 bg-[#0B0F1A] z-50">
        ...
        <div className="flex gap-4 items-center">
          ...
          <button 
            ...
            className="bg-blue-600 hover:bg-white hover:text-blue-600 px-8 py-3 rounded-2xl text-[10px] font-black uppercase italic transition-all flex items-center gap-3 shadow-2xl shadow-blue-600/30 cursor-pointer active:scale-95 z-50 whitespace-nowrap"
          >
      ...
      {/* 📊 CONTENT WRAPPER (Zéro Scroll Global) */}
                    ...
                    <td className="px-10 py-4">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="flex flex-col">
                          ...
                          <span className="text-[9px] text-blue-500 font-bold uppercase tracking-widest leading-none mt-1">{u.OU_Code || 'SANS CODE'}</span>
                        </div>
                      </div>
                    </td>
                    <td className="px-10 py-4">
      ...
      <footer className="h-14 border-t border-white/5 flex justify-between items-center px-10 opacity-40 italic shrink-0 z-40 bg-[#0B0F1A]">
        <div className="flex items-center gap-5">
          ...
          <span className="text-[9px] font-black uppercase tracking-widest text-white italic">Matrix SDE Engine • v4.0 Elite</span>
        </div>
          ...
          <div className="w-[480px] h-full bg-[#0B0F1A] border-l border-white/10 p-12 flex flex-col gap-10 shadow-5xl animate-in slide-in-from-right duration-500">
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-8 overflow-y-auto pr-4 custom-scrollbar">
              <div className="flex flex-col gap-3">
                <label className="text-[9px] font-black text-slate-500 uppercase italic ml-3 tracking-[0.2em]">Désignation Organique</label>
              ...
              <div className="flex flex-col gap-3">
                ...
                <input value={formData.OU_Code} onChange={(e) => setFormData({...formData, OU_Code: e.target.value.toUpperCase()})} className="bg-white/5 border border-white/10 rounded-2xl p-5 text-sm font-bold italic outline-none focus:border-blue-600 transition-all text-white" placeholder="CODE (EX: DIR-PROD)" />
              </div>
              
              <div className="flex flex-col gap-3">
                <label className="text-[9px] font-black text-slate-500 uppercase italic ml-3 tracking-[0.2em]">Typologie Structurelle</label>
              ...
              <div className="flex flex-col gap-3">
                <label className="text-[9px] font-black text-slate-500 uppercase italic ml-3 tracking-[0.2em]">Ancrage Territorial (Site)</label>
              ...
              <div className="flex flex-col gap-3">
                <label className="text-[9px] font-black text-slate-500 uppercase italic ml-3 tracking-[0.2em]">Unité Parente (Réflexif)</label>
                <select value={formData.OU_ParentId} onChange={(e) => setFormData({...formData, OU_ParentId: e.target.value})} className="w-full bg-[#151A2D] border border-white/10 rounded-2xl p-5 text-sm font-bold italic outline-none focus:border-blue-600 transition-all text-white appearance-none cursor-pointer">
                  ...
                  {/* ✅ ANTI-BOUCLE : On ne peut pas être son propre parent */}
