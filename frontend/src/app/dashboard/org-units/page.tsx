...
export default function OrgUnitsPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("⚠️ ARCHIVAGE CRITIQUE : Confirmer la révocation ?")) return;
    try {
      await apiClient.delete(`/org-units/${id}`);
      toast.success("UNITÉ ARCHIVÉE");
      fetchData();
    } catch (err: any) {
      toast.error(err.response?.data?.message || "ERREUR D'ARCHIVAGE");
    }
  };
  ...
  return (
    <div className="flex h-screen w-full bg-[#0B0F1A] text-white italic font-sans overflow-hidden">
      ...
      {/* ZONE CENTRALE (Ajustée à 72 du menu) */}
      <div className="flex-1 flex flex-col ml-72 overflow-hidden bg-[#0B0F1A]">
              ...
              <table className="w-full text-left border-collapse">
                <thead className="sticky top-0 bg-[#151A2D] z-20 border-b border-white/10">
                  <tr className="text-[8px] text-slate-500 uppercase font-black italic tracking-widest">
                    <th className="px-6 py-4">Unité & Code</th>
                ...
                <tbody className="divide-y divide-white/5 text-[10px]">
                  {filteredUnits.map(u => (
                    <tr key={u.OU_Id} className="group hover:bg-blue-600/5 transition-all">
                      <td className="px-6 py-3 font-black text-white uppercase italic">
                        {u.OU_Name} <span className="ml-2 text-blue-500 text-[8px]">{u.OU_Code}</span>
                      </td>
                      ...
                      <td className="px-6 py-3 text-slate-500 italic uppercase">
                         <div className="flex items-center gap-1"><GitBranch size={10} /> {u.OU_Parent?.OU_Name || 'ROOT'}</div>
                      </td>
                      <td className="px-6 py-3 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          <button onClick={() => handleEdit(u)} className="p-2 bg-white/5 rounded-md text-slate-400 hover:text-blue-500 border border-white/10 cursor-pointer"><Edit3 size={14}/></button>
                          <button onClick={() => handleDelete(u.OU_Id)} className="p-2 bg-white/5 rounded-md text-slate-400 hover:text-red-500 border border-white/10 cursor-pointer"><Trash2 size={14}/></button>
                        </div>
                      </td>
                    </tr>
                  ))}
        ...
        <footer className="h-10 border-t border-white/5 flex justify-between items-center px-8 opacity-40 italic shrink-0">
          <div className="flex items-center gap-3">
            ...
            <span className="text-[8px] font-black uppercase text-white">SDE Matrix Engine v4.0</span>
          </div>
          <Activity size={16} className="text-emerald-500 animate-pulse" />
        </footer>
      </div>

      {/* 🛠️ MODALE ÉDITEUR (Drawer Matrix) */}
      {showEditor && (
        <div className="fixed inset-0 z-100 flex justify-end bg-black/80 backdrop-blur-sm">
          <div className="w-105 h-full bg-[#0B0F1A] border-l border-white/10 p-10 flex flex-col gap-8 shadow-5xl animate-in slide-in-from-right duration-300">
            ...
            <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-5 overflow-y-auto pr-2 custom-scrollbar">
              ...
              <div className="space-y-1">
                <label className="text-[8px] font-black text-slate-500 uppercase italic ml-2">Parent</label>
              ...
              <button disabled={submitting} type="submit" className="mt-auto bg-blue-600 hover:bg-white hover:text-blue-600 p-5 rounded-2xl font-black uppercase italic transition-all flex items-center justify-center gap-3 text-[10px] shadow-3xl">
