...
export default function TiersPage() {
  ...
  const handleDelete = async (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    if (!confirm("⚠️ RÉVOCATION : Confirmer la suppression définitive de ce tiers ?")) return;
    try {
      await apiClient.delete(`/tiers/${id}`);
      toast.success("Tiers révoqué du registre.");
      fetchData();
    } catch (e) { 
      toast.error("Erreur de révocation : le tiers est peut-être lié à des données SMI."); 
    }
  };
  ...
  if (loading && tiers.length === 0) return (
    <div className="ml-72 h-screen flex flex-col items-center justify-center bg-[#0B0F1A] gap-6 text-blue-500">
      ...
      <p className="font-black uppercase italic text-[10px] tracking-[0.5em] animate-pulse">Intelligence Tiers en cours...</p>
    </div>
  );
    ...
    <div className="p-10 bg-[#0B0F1A] min-h-screen ml-72 text-white italic font-sans relative flex flex-col items-center selection:bg-blue-600/30 overflow-x-hidden">
        ...
        {filteredTiers.length > 0 ? filteredTiers.map((tier) => (
          ...
          >
            <div className="absolute top-10 right-10 flex gap-3 opacity-0 group-hover:opacity-100 transition-all z-10 translate-y-2 group-hover:translate-y-0">
              <button onClick={(e) => handleEdit(e, tier)} className="p-3 bg-white/5 hover:bg-blue-600 rounded-xl text-white transition-colors border-none cursor-pointer"><Edit3 size={16}/></button>
              <button onClick={(e) => handleDelete(e, tier.TR_Id)} className="p-3 bg-white/5 hover:bg-red-600 rounded-xl text-white transition-colors border-none cursor-pointer"><Trash2 size={16}/></button>
            </div>
            ...
            <div className="flex justify-between items-start mb-10">
              <div className="w-16 h-16 bg-blue-600/10 rounded-2xl flex items-center justify-center text-blue-500 border border-blue-500/20 shadow-inner group-hover:bg-blue-600 group-hover:text-white transition-all">
                {tier.TR_Type === 'CLIENT' ? <Users size={32}/> : tier.TR_Type === 'FOURNISSEUR' ? <Briefcase size={32}/> : <Building size={32}/>}
              </div>
              <ChevronRight size={24} className="text-slate-800 group-hover:text-blue-500 mt-5 group-hover:translate-x-2 transition-transform" />
            </div>

            <h3 className="text-3xl font-black uppercase italic mb-8 group-hover:text-blue-400 transition-colors text-white tracking-tighter leading-none">{tier.TR_Name}</h3>
            ...
            <div className="flex items-center gap-4 border-t border-white/5 pt-8">
               ...
               <p className="text-[9px] font-bold text-slate-600 uppercase italic truncate max-w-37.5">{tier.TR_Email || 'PAS D\'EMAIL SCÉLLÉ'}</p>
            </div>
          </div>
        )) : (
          <div className="col-span-full py-40 text-center opacity-30">
            <Users size={80} className="mx-auto text-slate-700 mb-8" />
            <p className="text-slate-500 font-black uppercase italic text-sm tracking-[0.5em]">Aucune partie intéressée au registre</p>
          </div>
        )}
      </div>
      ...
      {isDetailOpen && selectedTier && (
        <div className="fixed inset-0 z-150 flex justify-end">
          <div className="absolute inset-0 bg-black/80 backdrop-blur-2xl animate-in fade-in duration-500" onClick={() => setIsDetailOpen(false)} />
          <div className="relative w-full max-w-2xl bg-[#0B0F1A] border-l border-white/10 h-full p-16 shadow-4xl animate-in slide-in-from-right duration-700 overflow-y-auto italic text-left">
            <button onClick={() => setIsDetailOpen(false)} className="absolute top-10 right-10 text-slate-500 hover:text-white transition-colors border-none bg-transparent cursor-pointer"><X size={40}/></button>
            
            <div className="mb-16">
                <div className="flex items-center gap-4 text-blue-500 mb-4">
                    <Activity size={20} />
                    <span className="font-black uppercase text-[11px] tracking-[0.5em] italic">Intelligence Parties Intéressées</span>
                </div>
                <h2 className="text-6xl font-black uppercase italic mt-2 text-white leading-none tracking-tighter">{selectedTier.TR_Name}</h2>
                <div className="flex items-center gap-4 mt-6">
                    <Mail size={16} className="text-slate-600" />
                    <p className="text-slate-400 text-sm font-bold">{selectedTier.TR_Email || 'contact@non-defini.sn'}</p>
                </div>
            </div>
            ...
            <div className="space-y-12">
              ...
              <div className="grid grid-cols-2 gap-8">
                <div className="bg-white/5 border border-white/5 p-10 rounded-[3rem] shadow-inner group hover:border-blue-500/30 transition-all">
                  <MessageSquare className="text-blue-500 mb-6 group-hover:scale-110 transition-transform" size={32} />
                  <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-2 italic">Réclamations / NC</p>
                  <p className="text-5xl font-black italic text-white tracking-tighter">{selectedTier.stats?.reclamations || 0}</p>
                </div>
                <div className="bg-white/5 border border-white/5 p-10 rounded-[3rem] shadow-inner group hover:border-emerald-500/30 transition-all">
                  <Target className="text-emerald-500 mb-6 group-hover:scale-110 transition-transform" size={32} />
                  <p className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-2 italic">Actions SMI</p>
                  <p className="text-5xl font-black italic text-white tracking-tighter">{selectedTier.stats?.actions || 0}</p>
                </div>
              </div>

              {/* TERMINAL D'ACTIONS RAPIDES */}
              <div className="space-y-6 pt-12 border-t border-white/10">
                <h4 className="text-[11px] font-black uppercase text-slate-600 ml-4 tracking-[0.3em] mb-6 italic">Terminal de pilotage connecté</h4>
                <button 
                ...
                >
                  <span className="text-xs font-black uppercase italic tracking-widest">Saisir une réclamation client</span>
                ...
                >
                  <span className="text-xs font-black uppercase italic tracking-widest">Lancer une action préventive</span>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-200 bg-black/95 backdrop-blur-2xl flex items-center justify-center p-6 text-left">
          ...
          >
            <button type="button" onClick={() => setIsModalOpen(false)} className="absolute top-12 right-12 text-slate-500 hover:text-white transition-colors border-none bg-transparent cursor-pointer"><X size={40}/></button>
            <div className="mb-12">
                <h2 className="text-5xl font-black uppercase italic text-white leading-none tracking-tighter">Registre <span className="text-blue-500">Tiers</span></h2>
                <p className="text-[10px] font-black uppercase text-slate-500 tracking-[0.4em] mt-4 italic">Indexation de Partie Intéressée ISO §4.2</p>
            </div>
            
            <div className="space-y-8">
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase text-slate-500 ml-6 italic tracking-widest">Raison Sociale / Nom *</label>
                <input 
                    required 
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase text-slate-500 ml-6 italic tracking-widest">Email de contact principal</label>
                <input 
                    type="email" 
                    ...
                    value={form.TR_Email} 
                    onChange={e => setForm({...form, TR_Email: e.target.value})} 
                    placeholder="contact@entreprise.sn" 
                />
              </div>

              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase text-slate-500 ml-6 italic tracking-widest">Catégorie Stratégique *</label>
                <select 
                    required 
              ...
              >
                {editingId ? "Mettre à jour le registre" : "Sceller dans le registre Master"}
              </button>
            </div>
          </form>
        </div>
