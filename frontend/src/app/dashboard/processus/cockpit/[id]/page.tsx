    ...
    <div className="h-screen bg-[#0B0F1A] text-white italic font-sans ml-72 flex flex-col overflow-hidden text-left">
      ...
      <header className="px-10 py-6 border-b border-white/5 bg-[#0F172A]/80 backdrop-blur-3xl flex justify-between items-center shrink-0">
        <div className="flex items-center gap-8">
          <div className="w-16 h-16 rounded-4xl bg-blue-600 flex items-center justify-center shadow-2xl shadow-blue-900/40">
            <GitBranch size={32} />
          </div>
          <div>
            ...
            <p className="text-slate-500 font-bold text-[9px] uppercase tracking-[0.5em] mt-3 italic flex items-center gap-3">
              PILOTE : {process?.PR_Pilote?.U_FirstName} {process?.PR_Pilote?.U_LastName} • VERSION V{process?.PR_Version}.0
            </p>
          </div>
        </div>
        
        <div className="flex items-center gap-6">
          <button className="p-4 bg-white/5 hover:bg-blue-600/20 rounded-2xl transition-all border border-white/5 cursor-pointer">
            <Bell size={20} className="text-slate-500" />
          </button>
          <button className="bg-blue-600 hover:bg-white hover:text-slate-900 px-8 py-4 rounded-2xl font-black uppercase text-[10px] italic shadow-xl transition-all border-none cursor-pointer">
            ACTION DE PILOTAGE
          </button>
        </div>
      </header>
          ...
          <div className="flex-1 overflow-y-auto p-12 custom-scrollbar">
                ...
                <div className="grid grid-cols-12 gap-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
                    <div className="col-span-8 space-y-12">
                        <section className="bg-slate-900/40 border border-white/5 p-12 rounded-[4rem] relative overflow-hidden group shadow-2xl">
                            <h2 className="text-xl font-black uppercase italic mb-8 flex items-center gap-4 text-blue-500 tracking-tight">
                                <Target size={28} /> Finalités & Objectifs Stratégiques
                            </h2>
                            <p className="text-sm leading-relaxed text-slate-300 font-bold uppercase italic opacity-90">
                                {process?.PR_Objectifs || "Aucun objectif formalisé dans le SMI pour ce processus."}
                            </p>
                        </section>
              ...
              {activeModule !== 'ID' && (
                <div className="h-full flex flex-col items-center justify-center opacity-10">
                    <RefreshCcw size={100} className="animate-spin-slow mb-8" />
                    <h2 className="text-4xl font-black uppercase italic tracking-tighter">Initialisation du Hub {activeModule}</h2>
                </div>
              )}
          </div>
        </main>
...
function NavBtn({ active, icon: Icon, label, onClick }: any) {
  return (
    <button onClick={onClick} className={`flex items-center gap-5 px-6 py-4 rounded-2xl transition-all duration-500 group relative border-none cursor-pointer text-left w-full ${active ? "bg-blue-600 text-white shadow-2xl translate-x-2" : "text-slate-500 hover:text-white hover:bg-white/5"}`}>
      <Icon size={18} className={active ? "scale-110" : "group-hover:text-blue-500"} />
      <span className="text-[11px] font-black uppercase italic tracking-tight">{label}</span>
      {active && <div className="absolute left-0 w-1 h-5 bg-white rounded-full" />}
    </button>
  );
}
...
function IdentityCard({ title, icon: Icon, content }: any) {
  return (
    <div className="bg-white/2 border border-white/5 p-10 rounded-[3.5rem] group hover:border-blue-600/30 transition-all shadow-xl">
       <div className="flex items-center gap-5 mb-8">
          <div className="p-4 bg-white/5 rounded-2xl text-blue-500 group-hover:bg-blue-600 group-hover:text-white transition-all"><Icon size={24} /></div>
          <h4 className="text-[12px] font-black uppercase italic text-slate-400 tracking-widest">{title}</h4>
       </div>
       <p className="text-xs font-bold text-white uppercase italic leading-relaxed opacity-70">{content || "Donnée manquante."}</p>
    </div>
  );
}
...
function ProgressItem({ label, val, color = "bg-blue-500" }: any) {
  return (
    <div>
      <div className="flex justify-between text-[9px] font-black uppercase italic mb-3 tracking-widest"><span>{label}</span><span>{val}%</span></div>
      <div className="h-2 bg-white/5 rounded-full overflow-hidden"><div className={`h-full transition-all duration-1000 ${color}`} style={{ width: `${val}%` }} /></div>
    </div>
  );
}