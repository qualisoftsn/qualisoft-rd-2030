...
import { 
  ...
  Calendar, User, Link2, Info, Loader2, AlertTriangle, ShieldAlert
} from 'lucide-react';
...
export default function NouvelleReclamationPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Erreur de scellage documentaire");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (fetching) return (
    <div className="ml-72 h-screen flex flex-col items-center justify-center bg-[#0B0F1A] gap-6">
      ...
      <span className="text-blue-500 font-black uppercase tracking-[0.4em] text-[10px] italic animate-pulse">Scanning Infrastructure...</span>
    </div>
  );
      ...
      <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-8 duration-700">
        ...
        <header className="flex justify-between items-center border-b border-white/5 pb-10">
          ...
          <div className="p-4 bg-blue-600/5 border border-blue-500/20 rounded-2xl flex items-center gap-4">
             ...
             <span className="text-[10px] font-black uppercase tracking-widest text-blue-400">Canal de Traitement Certifié</span>
          </div>
        </header>

        {/* 📟 FORMULAIRE DE QUALIFICATION (§6.4 ISO 10002) */}
        <form onSubmit={handleSubmit} className="space-y-10 bg-slate-900/40 p-12 rounded-[4rem] border border-white/5 shadow-2xl backdrop-blur-3xl relative overflow-hidden text-left">
          
          <div className="grid grid-cols-2 gap-12">
            {/* BLOC A : IDENTITÉ & SOURCE */}
            <div className="space-y-8">
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase text-slate-500 ml-4 italic tracking-widest leading-none">Objet de la réclamation (Libellé Radical)</label>
                <input 
                  required
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase text-slate-500 ml-4 italic tracking-widest leading-none">Canal de réception</label>
            ...
            <div className="space-y-8 text-left">
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase text-slate-500 ml-4 italic tracking-widest leading-none">Organisme / Tiers Émetteur</label>
                <select 
                  required
              ...
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase text-slate-500 ml-4 italic tracking-widest leading-none">Gravité estimée (Impact Opérationnel)</label>
                <select 
                  className="w-full bg-[#0B0F1A] border border-white/10 rounded-2xl p-6 text-[11px] font-black uppercase italic text-white outline-none focus:border-blue-500 transition-all cursor-pointer shadow-inner appearance-none"
                ...
                >
                  ...
                  <option value="MEDIUM" className="text-blue-500">MOYENNE (Nécessite analyse métier)</option>
                  <option value="HIGH" className="text-orange-500">ÉLEVÉE (Action Corrective ISO requise)</option>
                  <option value="CRITICAL" className="text-rose-500">CRITIQUE (Urgence Direction / Alerte)</option>
                </select>
              </div>
            </div>
          </div>
          ...
          <div className="bg-blue-600/5 p-10 rounded-[3rem] border border-blue-500/10 space-y-8 shadow-inner relative overflow-hidden">
            <div className="absolute top-0 right-0 p-8 opacity-5 text-blue-500 rotate-12"><Link2 size={120}/></div>
            
            <div className="flex items-center gap-5 mb-4 leading-none relative z-10">
              <Link2 className="text-blue-500" size={24} />
              <h3 className="text-xl font-black uppercase italic tracking-tighter">Imputation au Processus Pilote</h3>
            </div>
            
            <div className="grid grid-cols-2 gap-12 relative z-10 text-left">
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase text-slate-400 ml-4 italic tracking-widest leading-none">Processus Responsable</label>
                <select 
                ...
                >
                  <option value="">DÉTERMINATION AUTOMATIQUE (NC QUALITÉ)</option>
                  {processus.map(p => <option key={p.PR_Id} value={p.PR_Id}>{p.PR_Libelle}</option>)}
                </select>
              </div>
              <div className="space-y-3">
                <label className="text-[10px] font-black uppercase text-slate-400 ml-4 italic tracking-widest leading-none">Échéance de traitement impérative</label>
                <div className="relative">
                  <input 
                    type="date"
                  ...
                  />
                  <Calendar className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-700 pointer-events-none" size={20} />
                </div>
              </div>
            </div>
            ...
            {!form.REC_ProcessusId && (
              <div className="flex items-start gap-5 bg-rose-500/10 p-6 rounded-2xl border border-rose-500/20 animate-pulse relative z-10 text-left">
                ...
                <p className="text-[10px] font-black text-rose-500 uppercase italic tracking-tight leading-relaxed">
                  Attention : L&apos;absence d&apos;imputation à un processus métier générera automatiquement une <strong>Non-Conformité (NC)</strong> globale. Le dossier sera transmis à l&apos;équipe Qualité pour analyse des causes racines conformément à l&apos;ISO 9001 §10.2.
                </p>
              </div>
            )}
          </div>
          ...
          <div className="space-y-3 text-left">
            <label className="text-[10px] font-black uppercase text-slate-500 ml-4 italic tracking-widest leading-none">Exposé détaillé des faits & Preuves invoquées</label>
            <textarea 
              required
              rows={5}
          ...
          <div className="flex flex-col gap-6 pt-10 border-t border-white/5">
            ...
            >
                ...
                SCELLER ET TRANSMETTRE AU PROCESSUS
            </button>
            <button 
                type="button" 
                onClick={() => router.back()} 
        ...
        <div className="flex items-center gap-6 p-10 bg-white/2 border border-white/5 rounded-[3.5rem] shadow-2xl">
           <div className="p-4 bg-amber-500/10 rounded-2xl text-amber-500 shadow-lg"><Info size={28}/></div>
           <p className="text-[11px] text-slate-500 font-bold italic leading-relaxed uppercase tracking-tighter text-left">
              <span className="text-amber-500 font-black">RAPPEL NORMATIF (§ISO 10002) :</span> Toute réclamation doit être traitée sans délai indu. L&apos;enregistrement initial doit être complet pour permettre une analyse objective. Si la plainte concerne la sécurité, augmentez la gravité à <span className="text-white">CRITIQUE</span> pour un traitement immédiat.
           </p>
        </div>
      </div>
    </div>
