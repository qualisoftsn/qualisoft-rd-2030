...
import { useAuthStore } from '@/store/authStore'; // ✅ Correction : Utilisation du Store Souverain
...
export default function ReclamationsPage() {
  // ✅ Correction : Récupération de l'identité via Zustand (Zéro NextAuth)
...
// Modal de création simplifié pour le build
function CreateModal({ onClose, onRefresh, tiers, processus }: any) {
  ...
  return (
    <div className="fixed inset-0 bg-black/95 backdrop-blur-2xl z-200 flex items-center justify-center p-8 italic">
      <form onSubmit={handleSubmit} className="bg-[#0F172A] w-full max-w-2xl rounded-[4rem] border border-white/10 p-16 space-y-10 shadow-2xl">
        <h2 className="text-4xl font-black text-white uppercase italic tracking-tighter">Déclaration <span className="text-blue-600">Incident</span></h2>
        <input required placeholder="OBJET DE LA PLAINTE" className="w-full bg-white/5 border border-white/10 p-7 rounded-4xl font-black text-white outline-none focus:border-blue-600 uppercase" value={form.REC_Object} onChange={(e) => setForm({ ...form, REC_Object: e.target.value })} />
        <div className="flex gap-4">
          <button type="submit" disabled={saving} className="flex-1 py-8 bg-blue-600 text-white rounded-[2.5rem] font-black uppercase text-xs italic cursor-pointer">
            {saving ? <Loader2 className="animate-spin" /> : "Valider"}
          </button>
          <button type="button" onClick={onClose} className="px-8 bg-transparent text-slate-500 font-black uppercase text-xs cursor-pointer">Annuler</button>
        </div>
      </form>
    </div>
  );
