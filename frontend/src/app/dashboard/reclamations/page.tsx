...
import { useAuthStore } from '@/store/authStore'; // ✅ Source unique de vérité
import React, {
  useCallback,
  useEffect,
  useMemo,
...
export default function ReclamationsPage() {
  // ✅ Utilisation du Store Souverain Matrix (Zéro NextAuth)
  const { user } = useAuthStore();
  
  // Barrière de sécurité pour le typage et l'accès
  ...
  useEffect(() => {
    // On ne déclenche la synchro que si l'utilisateur est authentifié dans le store
    if (user) fetchData();
  }, [fetchData, user]);
    ...
    try {
      const payload = {
        ...
        REC_Deadline: selectedRec.REC_Deadline
          ? new Date(selectedRec.REC_Deadline).toISOString()
          : null,
      };
  ...
  /**
   * 🚩 BARRIÈRE DE SÉCURITÉ (Early Return)
   * On empêche le rendu si le store n'est pas prêt, résolvant l'erreur de Sidebar.
   */
  if (loading || !user) {
    return (
      <div className="h-screen flex flex-col items-center justify-center bg-[#0B0F1A] gap-4">
        <Loader2 className="animate-spin text-blue-500" size={50} />
        <span className="font-black italic uppercase tracking-widest text-blue-500 text-[10px]">
  ...
  return (
    <div className="flex min-h-screen bg-[#0B0F1A] italic selection:bg-blue-600/30">
      {/* ✅ 'user' est maintenant garanti non-null ici */}
      <Sidebar user={user} isSuperAdmin={isSuperAdmin} />

      <main className="flex-1 ml-72 p-12 text-white font-sans overflow-y-auto h-screen scrollbar-hide">
        {/* HEADER D'ACTION */}
        ...
        <div className="bg-slate-950/40 rounded-[4rem] border border-white/5 overflow-hidden shadow-2xl backdrop-blur-3xl">
            ...
            <tbody className="divide-y divide-white/5 italic uppercase font-black text-xs">
              {filteredRecs.map((r) => (
                <tr key={r.REC_Id} className="hover:bg-blue-600/5 transition-all group">
                  <td className="p-10">
                    <p className="text-sm font-black tracking-tight group-hover:text-blue-400 transition-all leading-none">
                      {r.REC_Object}
                    </p>
                    <p className="text-[9px] text-slate-600 mt-2 tracking-widest font-bold">
                      REF: {r.REC_Reference}
                    </p>
                  </td>
                  ...
                  <td className="p-10 text-right">
                    <button onClick={() => setSelectedRec(r)} className="p-4 bg-white/5 rounded-2xl text-blue-500 hover:bg-blue-600 hover:text-white transition-all border-none cursor-pointer">
                      <Eye size={20} />
                    </button>
                  </td>
                </tr>
        ...
        {isCreateModalOpen && (
          <CreateModal onClose={() => setIsCreateModalOpen(false)} onRefresh={fetchData} tiers={tiers} processus={processus} />
        )}
        ...
        {selectedRec && (
          <>
            ...
            <div className="fixed top-0 right-0 h-screen w-160 bg-[#0F172A] z-120 border-l border-white/10 p-16 flex flex-col italic shadow-4xl animate-in slide-in-from-right duration-700">
              <div className="flex justify-between items-center mb-14 border-b border-white/5 pb-8">
                ...
                <button onClick={() => setSelectedRec(null)} className="p-5 bg-white/5 rounded-2xl hover:bg-red-500/20 hover:text-red-500 transition-all border-none cursor-pointer text-white">
                  <X size={28} />
                </button>
              </div>

              <div className="flex-1 space-y-10 overflow-y-auto pr-6 scrollbar-hide font-bold italic">
                <div className="grid grid-cols-2 gap-6">
                  <div className="bg-white/5 p-8 rounded-[2.5rem] border border-white/10">
                    <p className="text-[9px] font-black text-slate-500 uppercase mb-3 tracking-widest">Statut du Flux</p>
                    <select disabled={!isEditing} className="bg-transparent text-sm font-black uppercase text-blue-500 outline-none w-full cursor-pointer disabled:opacity-100" value={selectedRec.REC_Status} onChange={(e) => setSelectedRec({ ...selectedRec, REC_Status: e.target.value })}>
                ...
                <div className={`p-10 rounded-[3.5rem] border transition-all ${isEditing ? "bg-blue-600/5 border-blue-500/20" : "bg-white/5 border-white/10 opacity-60"}`}>
                  <h4 className="text-[10px] font-black text-slate-500 uppercase mb-8 flex items-center gap-3 tracking-[0.4em] italic leading-none">
                    <ShieldCheck size={18} className="text-blue-500" /> Responsabilité & Ancrage
                  </h4>
                ...
                <div className="p-12 bg-slate-900/60 rounded-[4rem] border border-white/5 space-y-8">
                  <h4 className="text-[11px] font-black text-blue-500 uppercase italic flex items-center gap-3 tracking-[0.3em]"><BarChart3 size={20} /> Analyse des Causes</h4>
                  <textarea readOnly={!isEditing} className={`w-full bg-[#0B0F1A] border p-8 rounded-[2.5rem] text-sm font-bold min-h-48 outline-none italic leading-relaxed transition-all ${isEditing ? "border-blue-500 shadow-inner" : "border-white/10"}`} value={selectedRec.REC_SolutionProposed || ""} onChange={(e) => setSelectedRec({ ...selectedRec, REC_SolutionProposed: e.target.value })} placeholder="Saisir l'analyse formelle..." />
                  
                  {selectedRec.REC_PreuveURL ? (
                    <div className="flex items-center justify-between p-8 bg-emerald-500/5 border border-emerald-500/20 rounded-[2.5rem]">
                      <div className="flex items-center gap-6">
                        <FileText size={32} className="text-emerald-500" />
                        <div className="flex flex-col font-black italic">
                          <span className="text-[11px] truncate max-w-48 text-white uppercase">{selectedRec.REC_PreuveName || "DOCUMENT_PREUVE"}</span>
                          <a href={`${API_BASE_URL}${selectedRec.REC_PreuveURL}`} target="_blank" rel="noreferrer" className="text-[10px] text-emerald-400 underline mt-2 flex items-center gap-2"><ExternalLink size={14} /> Visualiser</a>
                        </div>
                      </div>
                    ...
                    <div onClick={() => isEditing && fileInputRef.current?.click()} className={`border-3 border-dashed rounded-[3rem] p-12 flex flex-col items-center justify-center gap-4 transition-all ${isEditing ? "border-white/10 cursor-pointer hover:border-blue-500/50" : "border-white/5 opacity-20"}`}>
                      <input type="file" ref={fileInputRef} className="hidden" onChange={handleFileUpload} />
                      <UploadCloud size={40} className="text-slate-700" />
                      <p className="text-[10px] font-black uppercase text-slate-500 italic tracking-[0.4em]">Déposer la preuve</p>
                    </div>
                  )}
                </div>
                ...
                <div className="pt-8 flex flex-col gap-6">
                  {isEditing ? (
                    <button onClick={handleUpdate} disabled={submitting} className="w-full py-8 bg-blue-600 hover:bg-blue-500 text-white rounded-[3rem] font-black uppercase text-sm italic shadow-2xl flex items-center justify-center gap-5 border-none cursor-pointer transition-all active:scale-95">
                      {submitting ? <Loader2 className="animate-spin" size={24} /> : <Save size={24} />} Valider les modifications
                    </button>
                  ) : (
...
function CreateModal({ onClose, onRefresh, tiers, processus }: any) {
  ...
  const handleSubmit = async (e: any) => {
    e.preventDefault();
    if (!form.REC_TierId || !form.REC_ProcessusId) { toast.error("Ancrage Tier/Processus requis"); return; }
    setSaving(true);
    try {
      const payload = { ...form, REC_Object: form.REC_Object.toUpperCase(), REC_Deadline: form.REC_Deadline ? new Date(form.REC_Deadline).toISOString() : null };
      await apiClient.post("/reclamations", payload);
      toast.success("Réclamation enregistrée");
      onRefresh(); onClose();
    } catch (err) { toast.error("Échec initialisation"); } finally { setSaving(false); }
  };

  return (
    <div className="fixed inset-0 bg-black/98 backdrop-blur-3xl z-200 flex items-center justify-center p-8 italic">
      <form onSubmit={handleSubmit} className="bg-[#0F172A] w-full max-w-3xl rounded-[4.5rem] border border-white/10 p-20 space-y-10 animate-in zoom-in-95 duration-500 shadow-2xl">
        <h2 className="text-5xl font-black uppercase italic tracking-tighter text-white">Saisie <span className="text-blue-500">Registre</span></h2>
        <div className="space-y-6">
          <input required placeholder="OBJET RÉCLAMATION" className="w-full bg-white/5 border border-white/10 p-7 rounded-2xl font-black italic outline-none text-white uppercase text-sm" value={form.REC_Object} onChange={(e) => setForm({ ...form, REC_Object: e.target.value.toUpperCase() })} />
          <div className="grid grid-cols-2 gap-6">
            <select required className="bg-white/5 border border-white/10 p-7 rounded-2xl font-black outline-none text-white text-[11px] uppercase italic cursor-pointer" value={form.REC_TierId} onChange={(e) => setForm({ ...form, REC_TierId: e.target.value })}>
              <option value="">-- PARTIE INTÉRESSÉE --</option>
              {tiers.map((t: any) => (<option key={t.TR_Id} value={t.TR_Id}>{t.TR_Name}</option>))}
            </select>
            <select required className="bg-white/5 border border-white/10 p-7 rounded-2xl font-black outline-none text-white text-[11px] uppercase italic cursor-pointer" value={form.REC_ProcessusId} onChange={(e) => setForm({ ...form, REC_ProcessusId: e.target.value })}>
              <option value="">-- PROCESSUS --</option>
              {processus.map((p: any) => (<option key={p.PR_Id} value={p.PR_Id}>{p.PR_Libelle}</option>))}
            </select>
          </div>
          <textarea required placeholder="DESCRIPTION ÉCART" rows={4} className="w-full bg-white/5 border border-white/10 p-7 rounded-3xl font-bold outline-none text-white italic text-sm" value={form.REC_Description} onChange={(e) => setForm({ ...form, REC_Description: e.target.value })} />
        </div>
        <div className="flex flex-col gap-6 pt-4">
          <button type="submit" disabled={saving} className="w-full py-8 bg-blue-600 text-white rounded-[3rem] font-black uppercase text-xs italic shadow-3xl flex items-center justify-center gap-4 border-none cursor-pointer active:scale-95">
            {saving ? <Loader2 className="animate-spin" size={24} /> : <ShieldCheck size={24} />} Initialiser
          </button>
          <button type="button" onClick={onClose} className="text-slate-600 text-[11px] font-black uppercase tracking-[0.4em] hover:text-white transition-all text-center border-none bg-transparent cursor-pointer">Annuler</button>
        </div>
      </form>
    </div>
  );
