...
export default function ReclamationsPage() {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      fetchData(); 
      setSelectedRec(null);
    } catch (e) { toast.error("Échec de la persistance"); }
  };
  ...
  if (loading) return (
    <div className="h-screen flex flex-col items-center justify-center bg-[#0B0F1A] gap-4">
      ...
      <span className="text-blue-500 font-black uppercase tracking-[0.4em] text-[10px] italic">Initialisation du Registre...</span>
    </div>
  );
          ...
          <div className="fixed inset-0 bg-black/95 backdrop-blur-md z-110 flex justify-end animate-in fade-in duration-300">
            <div className="h-screen w-160 bg-[#0F172A] border-l border-white/10 p-16 flex flex-col shadow-4xl animate-in slide-in-from-right duration-500">
              <div className="flex justify-between items-center mb-12 border-b border-white/5 pb-8">
                <h2 className="text-4xl font-black uppercase italic tracking-tighter">Détails <span className="text-blue-500">Dossier</span></h2>
                <button 
                  onClick={() => { setSelectedRec(null); setIsEditing(false); }} 
              ...
              <div className="flex-1 space-y-8 overflow-y-auto pr-6 italic scrollbar-hide">
                  ...
                  <div className="mt-12">
                    {isEditing ? (
                      <button onClick={handleUpdate} className="w-full py-7 bg-blue-600 hover:bg-blue-500 text-white rounded-4xl font-black uppercase text-xs italic transition-all shadow-2xl shadow-blue-900/40 border-none cursor-pointer">SCELLER LA MISE À JOUR</button>
                    ) : (
                      <button onClick={() => setIsEditing(true)} className="w-full py-7 bg-white/5 border border-white/10 text-white rounded-4xl font-black uppercase text-xs italic hover:bg-white/10 transition-all border-none cursor-pointer">OUVRIR LE DOSSIER EN ÉDITION</button>
                    )}
                  </div>
              </div>
            </div>
...
function CreateModal({ onClose, onRefresh, tiers, processus }: any) {
    ...
    return (
        <div className="fixed inset-0 bg-black/95 backdrop-blur-2xl z-200 flex items-center justify-center p-8 italic animate-in zoom-in duration-300">
            <form onSubmit={handleSubmit} className="bg-[#0F172A] w-full max-w-2xl rounded-[4rem] border border-white/10 p-16 space-y-10 shadow-[0_50px_100px_rgba(0,0,0,0.5)]">
                ...
                <div className="space-y-6">
                  ...
                  <div className="bg-white/5 p-6 rounded-4xl border border-white/10 shadow-inner">
                     <label className="text-[10px] font-black text-slate-500 uppercase italic tracking-widest block mb-2">Échéance de traitement souhaitée</label>
                     <input type="date" className="w-full bg-transparent text-white outline-none mt-1 text-sm font-black" value={form.REC_Deadline} onChange={e => setForm({...form, REC_Deadline: e.target.value})} />
                  </div>
                </div>
                ...
                <div className="flex flex-col gap-4">
                  <button type="submit" disabled={saving} className="w-full py-8 bg-blue-600 hover:bg-blue-500 text-white rounded-[2.5rem] font-black uppercase text-xs italic shadow-2xl flex justify-center items-center border-none cursor-pointer">
                      {saving ? <Loader2 className="animate-spin" /> : "SCELLER LA DÉCLARATION"}
                  </button>
                  <button type="button" onClick={onClose} className="text-slate-500 text-[10px] font-black uppercase italic tracking-widest border-none bg-transparent cursor-pointer hover:text-white transition-all">Abandonner la saisie</button>
                </div>
            </form>
        </div>
    );
