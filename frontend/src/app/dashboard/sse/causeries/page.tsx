...
import { 
  Users, Calendar, Plus, ShieldCheck, Leaf, 
  Trash2, QrCode, Shield, Loader2, GitCommit,
  Clock, AlertTriangle,
  Mic2
} from 'lucide-react';
...
export default function CauseriesSSEPage() {
  ...
  const handleDelete = async (id: string) => {
    if (!window.confirm("⚠️ RÉVOCATION DÉFINITIVE DE LA SESSION ? ACTION IRRÉVERSIBLE AU SEIN DU TENANT.")) return;
  ...
  if (loading) return (
    <div className="ml-72 h-screen flex flex-col items-center justify-center bg-[#0B0F1A] gap-10 font-sans italic">
      <div className="relative">
        ...
        <ShieldCheck className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-blue-500/50" size={32} />
      </div>
  ...
  return (
    <div className="p-12 bg-[#0B0F1A] min-h-screen ml-72 text-white font-sans uppercase italic font-black selection:bg-blue-600/30 overflow-x-hidden">
        ...
        <div className="text-left flex items-center gap-6">
          ...
          <div>
            <h1 className="text-6xl tracking-tighter italic leading-none text-white">
              CAUSERIES <span className="text-blue-600 drop-shadow-[0_0_20px_rgba(37,99,235,0.5)]">SÉCURITÉ</span>
            </h1>
            <p className="text-slate-500 text-[10px] tracking-[0.5em] mt-4 italic uppercase">
              Management de la Sensibilisation ISO 45001 §7.3 • CULTURE SÉCURITÉ PRÉVENTIVE
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-20">
        <CStatsCard label="TOTAL SESSIONS" value={stats?.total || 0} icon={<ShieldCheck size={28} className="text-blue-400"/>} color="bg-blue-900/10 border-blue-500/20" />
        <CStatsCard label="SESSIONS / MOIS" value={stats?.monthCount || 0} icon={<Calendar size={28} className="text-emerald-400"/>} color="bg-emerald-900/10 border-emerald-500/20" />
        <CStatsCard label="FOCUS ÉCOLOGIQUE" value={`${stats?.envRatio || 0}%`} icon={<Leaf size={28} className="text-green-400"/>} color="bg-green-900/10 border-green-500/20" />
        <CStatsCard label="INTÉGRITÉ ISO" value="100%" icon={<Shield size={28} className="text-purple-400"/>} color="bg-purple-900/10 border-purple-500/20" />
      </div>

      {/* --- REGISTRE DES SESSIONS ACTIVES --- */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 text-left">
        {causeries.map((c) => (
          <div key={c.CS_Id} className={`bg-[#0F172A]/80 border-2 ${c.CS_IsActive ? 'border-white/5 hover:border-blue-600/30' : 'border-red-500/20 opacity-60'} p-12 rounded-[4rem] relative group transition-all shadow-[0_0_50px_rgba(0,0,0,0.5)] backdrop-blur-xl flex flex-col`}>
             ...
             <div className="flex justify-between items-start mb-10 flex-1">
                <div className="flex flex-col pr-8">
                   <div className="flex items-center gap-3 mb-4 text-[10px] text-blue-500 font-black tracking-[0.3em]">
                     <Clock size={14} />
                     {new Date(c.CS_Date).toLocaleDateString('fr-FR', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' })}
                   </div>
                   <h3 className="text-2xl tracking-tighter text-white uppercase italic leading-tight">{c.CS_Theme}</h3>
                </div>
                <div className="p-5 bg-white/5 rounded-4xl group-hover:bg-blue-600/20 transition-all shadow-inner border border-white/5 shrink-0">
                  <Users size={28} className="text-slate-500 group-hover:text-blue-400"/>
                </div>
             </div>
             ...
             <div className="flex items-center gap-8 mb-10 border-t border-white/5 pt-10">
                <div className="flex flex-col">
                   ...
                   <span className="text-xs text-slate-200 italic tracking-widest uppercase">
                     {c.CS_Animateur?.U_FirstName || 'NON'} {c.CS_Animateur?.U_LastName || 'DÉFINI'}
                   </span>
                </div>
                <div className="flex flex-col border-l border-white/10 pl-8">
                   <span className="text-[9px] text-slate-500 uppercase font-black tracking-widest mb-2">Contrôle Émargement</span>
        ...
        {causeries.length === 0 && (
          <div className="col-span-1 lg:col-span-2 p-32 text-center border-2 border-dashed border-white/10 rounded-[4rem] bg-white/5">
            <Shield size={64} className="text-slate-700 mx-auto mb-6 opacity-50" />
            <p className="text-slate-500 font-black italic uppercase tracking-[0.5em] opacity-80 leading-relaxed text-[11px]">
              Le registre des causeries est vierge.<br/> Initiez votre première session pour sceller la conformité.
            </p>
          </div>
        )}
      </div>
      ...
      {isFormModalOpen && <CauserieForm onClose={() => setIsFormModalOpen(false)} onRefresh={fetchData} />}
...
function CStatsCard({ label, value, icon, color }: any) {
  return (
    <div className={`${color} p-8 rounded-[3rem] shadow-[0_20px_40px_rgba(0,0,0,0.3)] group backdrop-blur-xl transition-all hover:-translate-y-2 border text-left`}>
      ...
      <p className="text-[10px] text-slate-500 tracking-[0.3em] uppercase mb-3 font-black italic leading-none">{label}</p>
      <p className="text-4xl font-black italic tracking-tighter text-white leading-none drop-shadow-lg">{value}</p>
    </div>
  );
}
