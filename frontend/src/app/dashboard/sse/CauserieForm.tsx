...
export default function CauserieForm({ onClose, onRefresh }: SSEFormProps) {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        console.error(
          "Qualisoft Kernel: Échec de lecture de l'annuaire.",
          error,
        );
        toast.error(
          "Erreur de liaison : Impossible de charger les animateurs.",
        );
      }
    };
    fetchUsers();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(
        err.response?.data?.message || "Rejet du Kernel : Échec de création.",
        { id: tid },
      );
    } finally {
      setLoading(false);
      ...
      <div className="bg-[#0F172A] border border-blue-500/20 w-full max-w-2xl rounded-[4rem] shadow-[0_0_150px_rgba(37,99,235,0.15)] relative overflow-hidden flex flex-col max-h-[90vh]">
          ...
          >
            ...
            <div className="space-y-4">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, CS_Theme: e.target.value })
                }
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="space-y-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, CS_Date: e.target.value })
                  }
                />
              </div>
              ...
              <div className="space-y-4">
                <label className="text-[11px] font-black uppercase text-slate-400 tracking-[0.3em] ml-2 flex items-center gap-3">
                  <ShieldAlert size={16} className="text-blue-500" /> Périmètre
                  ISO
                </label>
                <select
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, CS_Type: e.target.value })
                  }
                >
            ...
            <div className="space-y-4">
              <label className="text-[11px] font-black uppercase text-slate-400 tracking-[0.3em] ml-2 flex items-center gap-3">
                <Users size={16} className="text-blue-500" /> Animateur /
                Référent
              </label>
              <select
                required
                ...
                onChange={(e) =>
                  setFormData({ ...formData, CS_AnimateurId: e.target.value })
                }
              >
            ...
            <div className="space-y-4">
              ...
              <textarea
                ...
                onChange={(e) =>
                  setFormData({ ...formData, CS_Description: e.target.value })
                }
              />
            </div>
          </form>
        ...
        <div className="p-10 border-t border-white/5 bg-[#0B0F1A] relative z-10 shrink-0">
          ...
          >
            ...
            {loading
              ? "SCELLAGE EN COURS..."
              : "Valider au registre des Causeries"}
          </button>
        </div>
      </div>
    </div>
