...
export default function PilotageKPIPage() {
   ...
   */
  const shouldDisplayThisMonth = useCallback(
    (freq: Frequence, month: number): boolean => {
      switch (freq) {
        case "MENSUEL":
          return true;
        case "BIMENSUEL":
          return month % 2 !== 0; // Jan, Mar, May, etc.
        case "TRIMESTRIEL":
  ...
  const canEdit = useCallback(
    (indicator: Indicator, process: Processus): boolean => {
      if (!user) return false;
      if (isAdmin) return true;

      const isOwner =
        process.PR_PiloteId === user.U_Id ||
        process.PR_CoPiloteId === user.U_Id;
  ...
  const handleOpenModal = async (indicator: Indicator, process: Processus) => {
    ...
    try {
      const historyRes = await apiClient.get(
        `/indicators/${indicator.IND_Id}/history`,
      );
      setSelectedIndicator((prev) =>
        prev ? { ...prev, history: historyRes.data } : null,
      );
    } catch (err) {
    ...
    <div className="p-8 lg:p-12 italic bg-[#0B0F1A] min-h-screen text-white pb-32 text-left font-sans selection:bg-blue-600/30">
      ...
      <div className="flex flex-wrap gap-4 mb-12 bg-slate-900/40 p-6 rounded-[3rem] border border-white/5 backdrop-blur-md shadow-2xl items-center">
        <div className="flex items-center gap-3 ml-4">
          ...
          <span className="text-[11px] font-black uppercase text-slate-400 tracking-widest italic">
            Axe de Processus :
          </span>
        </div>
      ...
      <div className="space-y-16">
        {filteredProcesses.map((process) => {
          ...
          const hasDraft = activeIndicators.some(
            (ind) =>
              !ind.currentValue?.IV_Actual ||
              ind.currentValue.IV_Status === "BROUILLON" ||
              ind.currentValue.IV_Status === "RENVOYE",
          );

          return (
            <section
              key={process.PR_Id}
              className="bg-slate-900/30 border border-white/5 rounded-[4rem] overflow-hidden shadow-2xl shadow-black animate-in fade-in duration-700"
            >
              {/* HEADER DU PROCESSUS */}
              ...
              >
                {activeIndicators.map((indicator) => {
                  const status =
                    indicator.currentValue?.IV_Status || "BROUILLON";
                  ...
                  if (viewMode === "list") {
                    return (
                      <div
                        ...
                        className="flex items-center justify-between p-8 hover:bg-white/2 transition-all group border-none bg-transparent"
                      >
                        <div className="flex-1 grid grid-cols-12 gap-8 items-center text-left">
                          <div className="col-span-4">
                            ...
                            <span className="text-xl font-black text-white italic block truncate leading-none uppercase">
                              {indicator.IND_Libelle}
                            </span>
                            <span className="text-[9px] text-slate-500 uppercase mt-2 block font-black tracking-widest italic">
                              CIBLE : {indicator.IND_Cible}{" "}
                              {indicator.IND_Unite}
                            </span>
                          </div>
                          ...
                          <div className="col-span-2 flex justify-center">
                            <span
                              className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase italic tracking-widest border flex items-center gap-2 ${statusInfo.color}`}
                            >
                          ...
                          <div className="col-span-3 text-center">
                            {actual != null ? (
                              <div
                                className={`text-3xl font-black italic tracking-tighter ${getPerformanceColor(actual, indicator.IND_Cible)}`}
                              >
                                {actual}{" "}
                              ...
                              <span className="text-slate-800 text-sm font-black italic uppercase tracking-widest">
                                Défaut Relevé
                              </span>
                            )}
                          </div>
                          ...
                          <div className="col-span-3 flex justify-end pr-4">
                            <button
                              onClick={() =>
                                handleOpenModal(indicator, process)
                              }
                  ...
                  return (
                    <div
                      key={indicator.IND_Id}
                      className="bg-black/40 border border-white/5 rounded-[3.5rem] p-10 hover:border-blue-500/30 hover:bg-slate-900/50 transition-all group relative flex flex-col shadow-2xl text-left backdrop-blur-sm"
                    >
                      <div className="absolute top-0 right-0 p-8">
                        <div
                          className={`px-4 py-2 rounded-2xl text-[9px] font-black uppercase italic tracking-widest border backdrop-blur-md flex items-center gap-2 ${statusInfo.color}`}
                        >
                          <StatusIcon size={12} /> {statusInfo.label}
                        </div>
                      </div>
                      ...
                      <div className="mb-10 pr-24">
                        <span className="text-[10px] font-black text-blue-500 uppercase tracking-[0.4em] block mb-4 italic leading-none">
                          {indicator.IND_Code}
                        </span>
                      ...
                      <div className="bg-slate-950/60 rounded-[3rem] p-8 mb-8 border border-white/5 group-hover:border-blue-500/20 shadow-inner transition-all">
                        <div className="flex justify-between items-end mb-4">
                          <span className="text-[10px] font-black text-slate-600 uppercase tracking-widest italic leading-none">
                            Valeur Relevée
                          </span>
                          <span className="text-[10px] font-black text-slate-700 uppercase italic tracking-widest">
                            {indicator.IND_Unite}
                          </span>
                        </div>

                        <div
                          className={`text-5xl font-black italic tracking-tighter mb-4 leading-none ${actual != null ? getPerformanceColor(actual, indicator.IND_Cible) : "text-slate-800"}`}
                        >
                          {actual != null ? actual : "--"}
                        </div>
                        ...
                        {variation !== null && (
                          <div
                            className={`flex items-center gap-2 text-[11px] font-black uppercase italic tracking-widest ${Number(variation) >= 0 ? "text-emerald-500" : "text-red-500"}`}
                          >
                            <TrendingUp
                              size={16}
                              className={
                                Number(variation) < 0 ? "rotate-180" : ""
                              }
                            />
                            <span>
                              {Number(variation) > 0 ? "+" : ""}
                      ...
                      <div className="grid grid-cols-2 gap-4 mb-10">
                        <div className="bg-white/2 rounded-3xl p-6 border border-white/5 flex flex-col justify-center">
                          <span className="text-[9px] font-black text-slate-600 uppercase tracking-widest block mb-2 italic">
                            Cible SMI
                          </span>
                        ...
                        <div className="bg-white/2 rounded-3xl p-6 border border-white/5 flex flex-col justify-center">
                          ...
                          <div className="flex items-center gap-2 text-blue-400 font-black italic text-xs tracking-widest leading-none">
                            <Clock size={18} />{" "}
                            {indicator.IND_Frequence.slice(0, 4)}
                          </div>
                        </div>
                      </div>
              ...
              <div className="text-left">
                ...
                <div className="flex items-center gap-6 mt-6">
                  <span className="flex items-center gap-3 text-xs font-black uppercase italic text-emerald-400 tracking-widest">
                    <Target size={18} /> Cible : {selectedIndicator.IND_Cible}{" "}
                    {selectedIndicator.IND_Unite}
                  </span>
                  <span className="w-1.5 h-1.5 rounded-full bg-slate-700" />
            ...
            <div className="p-12">
              {activeTab === "saisie" ? (
                <div className="space-y-12 max-w-3xl mx-auto animate-in slide-in-from-bottom-4 duration-500">
                  <div
                    className={`p-8 rounded-[3rem] border flex items-center gap-6 shadow-inner ${
                      targetPeriod.isEditable
                  ...
                  <div className="space-y-6 text-left">
                    <label className="text-[11px] font-black text-slate-500 uppercase tracking-[0.4em] ml-8 block leading-none">
                      Valeur Constatée ({selectedIndicator.IND_Unite})
                    </label>
                    <input
                      type="number"
                      value={inputValue}
                  ...
                  <div className="space-y-6 text-left">
                    <label className="text-[11px] font-black text-slate-500 uppercase tracking-[0.4em] ml-8 block leading-none">
                      Commentaire d&apos;Analyse & Justification
                    </label>
                    <textarea
                      value={inputComment}
                ...
                <div className="space-y-12 animate-in fade-in duration-500">
                  ...
                  <div className="grid gap-6">
                    ...
                    selectedIndicator.history.length === 0 ? (
                      <div className="py-24 bg-black/40 rounded-[4rem] border border-dashed border-white/10 text-center">
                        <AlertCircle
                          size={64}
                          className="mx-auto mb-6 text-slate-800"
                        />
                        <p className="text-xl font-black uppercase italic text-slate-700 tracking-[0.4em]">
                          Défaut d&apos;Antériorité Documentaire
                        </p>
                      </div>
                    ) : (
                      <div className="space-y-4">
                        {selectedIndicator.history.map((val) => (
                          <div
                            key={val.IV_Id}
                            className="flex items-center justify-between p-8 bg-black/30 rounded-[3rem] border border-white/5 hover:border-blue-500/20 transition-all group shadow-xl text-left"
                          >
                            <div className="flex items-center gap-10">
                              <div className="w-24 h-24 bg-slate-900 rounded-4xl flex flex-col items-center justify-center border border-white/10 shadow-inner group-hover:border-blue-600/30 transition-colors">
                                <span className="text-3xl font-black text-white italic leading-none mb-1">
                                  {val.IV_Month.toString().padStart(2, "0")}
                                </span>
                              ...
                              <div className="text-left">
                                <div
                                  className={`text-5xl font-black italic tracking-tighter leading-none mb-4 ${
                                    val.IV_Actual != null &&
                                    val.IV_Actual >=
                                      selectedIndicator.IND_Cible * 0.95
                                      ? "text-emerald-400"
                                      : val.IV_Actual != null &&
                                          val.IV_Actual >=
                                            selectedIndicator.IND_Cible * 0.8
                                        ? "text-amber-400"
                                        : "text-red-400"
                                  }`}
                                >
                                  {val.IV_Actual}{" "}
                                  <span className="text-xl text-slate-700 ml-1">
                                    {selectedIndicator.IND_Unite}
                                  </span>
                                </div>
                                <div className="flex items-center gap-4">
                                  <span
                                    className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase italic tracking-widest border ${getStatusConfig(val.IV_Status).color}`}
                                  >
                                    {getStatusConfig(val.IV_Status).label}
                                  </span>
                                  {val.IV_Comment && (
                                    <span className="text-[10px] text-slate-600 italic font-bold uppercase truncate max-w-md">
                                      &quot;{val.IV_Comment}&quot;
                                    </span>
                                  )}
                                </div>
                            ...
                            <div className="text-right pr-6">
                              <span className="text-[10px] font-black text-slate-700 uppercase tracking-widest block mb-2 italic">
                                Efficacité
                              </span>
                              <span
                                className={`text-4xl font-black italic leading-none ${
                                  val.IV_Actual &&
                                  val.IV_Actual >= selectedIndicator.IND_Cible
                  ...
                  {selectedIndicator.history &&
                    selectedIndicator.history.length > 1 && (
                      <div className="mt-12 p-12 bg-black/40 rounded-[4.5rem] border border-white/5 shadow-inner">
                        <h4 className="text-[11px] font-black text-slate-600 uppercase tracking-[0.5em] mb-12 italic text-left">
                          ...
                          {selectedIndicator.history
                            ...
                            .map((val, idx) => {
                              ...
                              return (
                                <div
                                  key={idx}
                                  className="flex-1 flex flex-col items-center gap-4 group cursor-pointer z-10"
                                >
                                  <div className="opacity-0 group-hover:opacity-100 transition-all bg-white text-black px-2 py-1 rounded text-[9px] font-black mb-1">
                                    {val.IV_Actual}
                                  </div>
                                  <div
