...
export default function SystemSettingsPage() {
  ...
  const config = {
    units: { endpoint: '/org-unit-types', title: 'Unités d\'Organisation', label: 'EX: DIRECTION, DÉPARTEMENT, ATELIER', key: 'OUT_Label' },
    processes: { endpoint: '/process-types', title: 'Types de Processus', label: 'EX: MANAGEMENT, SUPPORT, MÉTIER', key: 'PT_Label' },
    risks: { endpoint: '/risk-types', title: 'Typologies de Risques', label: 'EX: QUALITÉ, SSE, ENVIRONNEMENT', key: 'RT_Label' }
  };
  ...
  const handleAdd = async () => {
    ...
    } catch (err) { toast.error("Erreur d'écriture serveur."); }
  };
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("⚠️ Cette action peut impacter les documents liés. Confirmer ?")) return;
    try {
      await apiClient.delete(`${config[activeTab].endpoint}/${id}`);
      fetchData();
    } catch (err) { toast.error("L'option est probablement utilisée dans un processus actif."); }
  };
    ...
    <div className="p-12 bg-slate-50 min-h-screen italic font-sans text-left ml-72">
      <header className="mb-12 border-b border-slate-200 pb-10">
        <h1 className="text-4xl font-black uppercase tracking-tighter text-slate-900 flex items-center gap-6 leading-none">
          <Settings2 className="text-blue-600" size={48} /> Paramétrage <span className="text-blue-600">Structure SMI</span>
        </h1>
        ...
        {(['units', 'processes', 'risks'] as const).map((tab) => (
          <button
            ...
            className={`px-10 py-5 rounded-2xl font-black uppercase text-[10px] transition-all border-none cursor-pointer tracking-widest italic ${
              activeTab === tab ? 'bg-blue-600 text-white shadow-2xl scale-105' : 'bg-white text-slate-400 border border-slate-200 hover:border-blue-300'
            }`}
          >
            {config[tab].title}
          </button>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <div className="bg-white p-10 rounded-[3rem] border border-slate-200 shadow-2xl h-fit">
          <h2 className="text-[10px] font-black uppercase mb-8 flex items-center gap-3 text-slate-900 tracking-widest italic">
            <Plus size={18} className="text-blue-600" /> Ajouter un {activeTab === 'units' ? 'Niveau d\'unité' : 'Type'}
          </h2>
          <div className="space-y-6">
            <div className="space-y-2">
                <label className="text-[9px] font-black uppercase text-slate-400 ml-4">Libellé Radical</label>
                <input
            ...
            {activeTab === 'processes' && (
              <div className="space-y-2">
                <label className="text-[9px] font-black uppercase text-slate-400 ml-4">Code Couleur Identitaire</label>
                <input 
                    type="color" 
            ...
            >
              <Save size={18} /> Sceller dans le système
            </button>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 bg-white rounded-[3.5rem] border border-slate-200 shadow-2xl overflow-hidden backdrop-blur-3xl">
          {loading ? (
            <div className="p-32 text-center flex flex-col items-center gap-6">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-400 italic">Lecture du Noyau...</span>
            </div>
          ) : (
            <table className="w-full text-left">
              <thead className="bg-slate-50 text-[10px] font-black uppercase text-slate-400 border-b border-slate-100 italic">
                <tr>
                  <th className="px-10 py-8 tracking-widest">Désignation de l&apos;Option Référencée</th>
                  {activeTab === 'processes' && <th className="px-10 py-8 text-center">Identité Visuelle</th>}
                    ...
                    <tr key={id} className="hover:bg-slate-50 transition-all group">
                      ...
                      {activeTab === 'processes' && (
                        <td className="px-10 py-8">
                          <div className="w-10 h-10 rounded-2xl border border-slate-200 mx-auto shadow-inner" style={{ backgroundColor: item.PT_Color }}></div>
                        </td>
                      )}
