...
import { 
  AlertCircle, Crown, Loader2, X, Check, 
  ShieldCheck, CreditCard, Send, Calendar 
} from "lucide-react";
    ...
    <div className="p-10 bg-[#0B0F1A] min-h-screen ml-72 text-white italic font-sans overflow-x-hidden relative selection:bg-blue-600/30">
      ...
      {error && (
        <div className="mb-10 p-8 bg-red-500/10 border border-red-500/20 rounded-[2.5rem] flex items-center gap-6 text-red-500 animate-in slide-in-from-top-4">
          <AlertCircle size={32} />
          <div className="text-left">
              <p className="text-sm font-black uppercase tracking-tighter">Interruption du Noyau Billing</p>
              <p className="text-[10px] font-bold opacity-60 uppercase mt-1 italic tracking-widest">{error}</p>
          </div>
        </div>
      )}
          ...
          return (
            <div
              ...
              className={`p-10 rounded-[3.5rem] border flex flex-col bg-slate-900/40 transition-all duration-500 group relative overflow-hidden ${
                isCurrent ? "border-blue-600 scale-105 shadow-2xl z-10" : "border-white/5 hover:border-white/10"
              }`}
            >
              {isCurrent && (
              ...
              <ul className="space-y-4 mb-12 flex-1 text-left">
                ...
                {plan.features.map((feature, idx) => (
                  <li key={idx} className="flex items-center gap-3 text-[11px] font-bold text-slate-300 uppercase italic tracking-tight">
          ...
          <div className="bg-[#0B0F1A] border border-white/10 w-full max-w-xl rounded-[4rem] p-12 relative shadow-4xl text-left">
            ...
            <div className="space-y-8">
                ...
                <div className="p-8 bg-white/5 rounded-3xl border border-white/5 flex justify-between items-center shadow-inner">
                    <span className="text-[10px] font-black text-slate-500 uppercase italic">Montant à transférer</span>
                    <span className="text-3xl font-black italic">{selectedPlanForPay?.price} XOF</span>
                </div>
                ...
                <div className="space-y-3">
                    <label className="text-[10px] font-black uppercase text-slate-500 ml-4 italic tracking-widest">Référence de Transaction SMS</label>
                    <input 
                        required
                ...
                <div className="p-6 bg-amber-500/5 border border-amber-500/10 rounded-2xl italic">
                    <p className="text-[9px] text-amber-500 font-black uppercase tracking-widest leading-relaxed">
                        Note : Votre instance sera activée dans un délai de 24h à 48h après confirmation physique du crédit.
                    </p>
                </div>
