 ...
 * VERSION : 10.3.0 (Zero Defect - All Menus Restored)
 * CONFORMITÉ : ISO 9001 (Approche Processus) & ISO 27001 (Sécurité).
 * RÉVISION : 02 Mars 2026 | 23:55 GMT
 */
...
export default function Sidebar({ isSuperAdmin }: { isSuperAdmin: boolean }) {
  ...
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["pilotage", "processus", "audit"]);
  const [isImpersonated, setIsImpersonated] = useState<boolean>(false);

  useEffect(() => {
    // Détection Master Authority
  ...
  const handleLogout = () => {
    ...
    document.cookie = `qualisoft_token=; path=/; domain=${domain}; expires=Thu, 01 Jan 1970 00:00:00 UTC; SameSite=Lax`;
  ...
  /**
   * 🗺️ CARTOGRAPHIE INTÉGRALE DES PROCESSUS
   */
  const navigation: MenuGroup[] = useMemo(() => [
    ...
    },
    {
      ...
      items: [
        ...
        { title: "Workflow d'Approbation", path: "/dashboard/workflows", icon: GitBranch, access: [Role.ADMIN, Role.RQ, Role.MANAGER], badge: "3" },
        ...
        { title: "Sénégal Légal", path: "/dashboard/senegal-legal", icon: History, access: ["ALL"] },
      ]
    },
    {
      id: "processus",
  ...
  return (
    <aside className={`w-80 h-screen fixed left-0 top-0 z-100 flex flex-col border-r-2 transition-all duration-500 font-sans italic shadow-4xl 
      ...
      {/* 1. MASTER OVERRIDE BANNER */}
      {isImpersonated && (
        <div className="bg-amber-600 px-6 py-2.5 flex items-center justify-between animate-pulse shrink-0">
          <div className="flex items-center gap-3">
            <Fingerprint size={14} className="text-white" />
            <span className="text-[10px] font-black uppercase text-white tracking-widest leading-none">Master Authority Protocol Active</span>
          </div>
