...
export default function Sidebar({ user, isSuperAdmin }: { user: AuthUser; isSuperAdmin: boolean }) {
  const pathname = usePathname();
  const logout = useAuthStore((state) => state.logout);
  const [expandedGroups, setExpandedGroups] = useState<string[]>(["ch4_ch5", "ch8"]);
    ...
    const baseGroups: MenuGroup[] = [
      {
        ...
        icon: Building,
        items: [
          { title: "Cockpit SMI", path: "/dashboard", icon: LayoutDashboard, access: "ALL" },
          { title: "Parties Intéressées", path: "/dashboard/parties-interessees", icon: Gavel, access: "DIRECTION" },
          { title: "Sites & Unités", path: "/dashboard/org-units", icon: Network, access: "ALL" },
          { title: "Revues Direction", path: "/dashboard/management-review", icon: Presentation, access: "DIRECTION" },
        ]
      },
      {
        id: "ch6",
        ...
        icon: Target,
        items: [
          { title: "Objectifs & Kpis", path: "/dashboard/objectifs", icon: LineChart, access: "ALL" },
          { title: "Gestion des Risques", path: "/dashboard/risks", icon: Target, access: "MANAGER" },
          { title: "Instances COPIL", path: "/dashboard/gouvernance/copil", icon: Users2, access: "MANAGER" },
        ]
      },
      {
        id: "ch7",
        ...
        icon: HardHat,
        items: [
          { title: "GPEC & Compétences", path: "/dashboard/rh", icon: Award, access: "RH" },
          { title: "Dossiers RH", path: "/dashboard/users", icon: UserCircle, access: "RH" },
          { title: "Formations", path: "/dashboard/formations", icon: GraduationCap, access: "RH" },
          { title: "GED Qualité", path: "/dashboard/ged", icon: FolderOpen, access: "ALL" },
          { title: "Bibliothèque", path: "/dashboard/bibliotheque", icon: BookOpen, access: "ALL" },
          { title: "Équipements", path: "/dashboard/equipment", icon: Building2, access: "MANAGER" },
        ]
      },
      {
        id: "ch8",
        ...
        items: [
          ...
          { title: "Workflows", path: "/dashboard/workflows", icon: Workflow, access: "MANAGER" },
          { title: "Tiers / Achats", path: "/dashboard/tiers", icon: Truck, access: "ALL" },
      ...
      },
      {
        ...
        icon: RefreshCw,
        items: [
          { title: "Saisie Mesures", path: "/dashboard/indicators", icon: Activity, access: "ALL" },
          { title: "Audits Internes", path: "/dashboard/audits", icon: ClipboardCheck, access: "QSE" },
          { title: "Non-Conformités", path: "/dashboard/non-conformites", icon: AlertTriangle, access: "ALL", badge: 3 },
          { title: "PAQ (Actions)", path: "/dashboard/improvement", icon: CheckSquare, access: "ALL" },
          { title: "Archives", path: "/dashboard/archives", icon: FileSearch, access: "QSE" },
        ]
      },
      {
        id: "admin",
        ...
        items: [
          ...
          { title: "Configuration", path: "/dashboard/settings", icon: Settings2, access: "ADMIN" },
        ]
      }
    ];
        ...
        {menuGroups.map((group) => {
          ...
          return (
            <div key={group.id} className="mb-2">
              ...
              <div className={`overflow-hidden transition-all duration-500 ${isExpanded ? "max-h-screen opacity-100 mt-2" : "max-h-0 opacity-0"}`}>
