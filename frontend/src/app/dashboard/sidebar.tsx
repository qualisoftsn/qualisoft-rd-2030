...
export default function Sidebar({
  ...
  // 🔓 ÉTAT INITIAL : On garde ta logique d'ouverture complète pour la vue d'ensemble
  const [expandedGroups, setExpandedGroups] = useState<string[]>([
    "ch4_ch5", "ch6", "ch7", "ch8", "ch9_ch10", "certification", "ENV-01", "admin", "superadmin"
  ]);
    ...
    const baseGroups: MenuGroup[] = [
      {
        ...
        items: [
          ...
          { title: "Gestion des Actions", path: getPath("/dashboard/actions", "ACTIONS"), icon: Gavel, access: "ALL" },
          { title: "Alertes & Echéances", path: getPath("/dashboard/alerts", "Alertes et échéances"), icon: Gavel, access: "ALL" },
      ...
      },
      {
        ...
        icon: RefreshCw,
        items: [
          { title: "Indicateurs (KPI)", path: getPath("/dashboard/indicators", "KPI"), icon: Activity, access: "ALL" },
          ...
          { title: "Archives", path: "/dashboard/archives", icon: FileSearch, access: "ADMIN" },
        ],
      },
      {
        id: "certification",
  ...
  return (
    <aside className="w-80 h-screen bg-[#0F172A] text-white flex flex-col fixed left-0 top-0 z-40 border-r border-white/5 shadow-2xl font-sans overflow-hidden">
        ...
        {menuGroups.map((group) => {
          ...
          return (
            <div key={group.id} className="space-y-1">
              {/* Header du groupe (Style Premium Italic conservé pour l'identité) */}
              <button
              ...
              {/* Liste des items (Typographie améliorée pour la lisibilité) */}
              <div className={`overflow-hidden transition-all duration-500 ease-in-out ${isExpanded ? "max-h-[500px] opacity-100" : "max-h-0 opacity-0"}`}>
                <div className="pl-2 space-y-0.5 border-l border-white/5 ml-4 mt-1">
                  {visibleItems.map((item, idx) => {
                    // Logique de routage active améliorée
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 overflow-hidden">
                          ...
                          {/* Retrait de 'italic' ici pour meilleure lecture, conservation 'uppercase' */}
        ...
        <div className="flex items-center justify-between p-3 rounded-2xl bg-linear-to-r from-white/5 to-transparent border border-white/5 hover:border-blue-500/30 transition-all group cursor-default">
          <div className="flex items-center gap-3 overflow-hidden">
            ...
            <div className="overflow-hidden">
              ...
              <p className={`text-[8px] font-black uppercase tracking-widest ${isSuperAdmin ? "text-amber-500/80" : "text-blue-400/80"}`}>{user?.U_Role}</p>
            </div>
          </div>
          <button 
