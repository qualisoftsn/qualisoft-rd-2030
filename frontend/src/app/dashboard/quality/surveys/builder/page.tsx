...
export default function SurveyBuilderMaster() {
  ...
  /** 💾 ACTION : SCELLER L'ENQUÊTE */
  const scellerEnquete = useCallback(() => {
  ...
  return (
    <div className="p-12 bg-[#0B0F1A] min-h-screen ml-72 text-white italic font-sans overflow-hidden selection:bg-emerald-600/30">
      ...
      <div className="flex justify-between items-center mb-16 animate-in slide-in-from-top-6 duration-700">
        <button onClick={() => router.back()} className="flex items-center gap-4 text-slate-500 font-black uppercase text-[11px] tracking-[0.3em] hover:text-white transition-all border-none bg-transparent cursor-pointer italic">
        ...
        <div className="flex gap-6">
           ...
           <button onClick={scellerEnquete} className="bg-blue-600 px-12 py-5 rounded-2xl text-[10px] font-black uppercase italic tracking-[0.4em] flex items-center gap-4 shadow-[0_20px_50px_rgba(37,99,235,0.3)] hover:bg-blue-500 transition-all border-none cursor-pointer active:scale-95">
              <Save size={20} /> Sceller l&apos;Architecture
           </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-16 text-left">
        ...
        <div className="lg:col-span-2 space-y-12">
           <div className="bg-slate-900/40 p-12 rounded-[4rem] border border-white/5 shadow-2xl relative overflow-hidden group">
              <div className="absolute -right-8 -top-8 text-emerald-500/5 group-hover:rotate-12 transition-transform duration-1000"><Layout size={200} /></div>
              <input 
                value={surveyTitle}
              ...
              <p className="text-[11px] text-slate-500 font-black uppercase tracking-[0.5em] mt-6 flex items-center gap-4 italic relative z-10 leading-none">
                 <Layout size={18} className="text-emerald-500"/> Matrice de collecte dynamique § ISO 9001
              </p>
           </div>

           <div className="space-y-6">
              {questions.map((q, idx) => (
                <div key={q.id} className="bg-slate-900/60 border border-white/5 p-10 rounded-[3rem] flex items-center gap-10 group hover:border-emerald-500/30 transition-all shadow-xl animate-in fade-in slide-in-from-left-4 duration-500">
                   <div className="text-5xl font-black text-slate-800 italic group-hover:text-emerald-500/30 transition-colors leading-none tracking-tighter">0{idx + 1}</div>
                   <input 
        ...
        <div className="space-y-12">
           <div className="bg-linear-to-br from-blue-600/10 to-transparent border border-blue-600/20 p-12 rounded-[4.5rem] backdrop-blur-3xl shadow-3xl relative overflow-hidden group">
              <div className="absolute -right-6 -top-6 text-blue-500/5 group-hover:scale-110 transition-transform duration-1000"><Globe size={180} /></div>
              ...
              <div className="space-y-8 relative z-10">
                 <div className="text-left">
                    <label className="text-[10px] font-black text-slate-500 uppercase italic tracking-[0.4em] block mb-4 ml-4">Point d&apos;entrée Public</label>
           ...
           <div className="bg-slate-900/40 border border-white/5 p-12 rounded-[4rem] shadow-2xl text-left">
              <div className="flex items-center gap-4 text-amber-500 mb-8 leading-none">
                 <AlertCircle size={24} />
                 <h4 className="text-base font-black uppercase italic tracking-[0.3em]">Conseil ISO 9001</h4>
              </div>
