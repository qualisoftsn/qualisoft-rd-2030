 ...
 * DESIGN : Elite Industrial Dark • 5 Plans • Showcase • Quiz Magnet
 */
  ...
  return (
    <div className="min-h-screen bg-[#0B0F1A] text-white font-sans italic selection:bg-blue-600/30 overflow-x-hidden text-[11px]">
      ...
      <nav className="h-16 border-b border-white/5 flex items-center justify-between px-8 lg:px-12 sticky top-0 bg-[#0B0F1A]/90 backdrop-blur-2xl z-100">
        <Link href="https://qualisoft.sn" className="flex items-center gap-3 no-underline group">
          <Image src="/images/qslogo.png" alt="QS" width={28} height={28} />
          <span className="font-black uppercase tracking-widest text-slate-400 group-hover:text-white transition-colors">Qualisoft</span>
        </Link>
        ...
        <div className="flex items-center gap-4">
          <Link href="/norms/iso-9001" className="hidden sm:flex items-center gap-2 px-5 py-2 bg-blue-600 rounded-xl font-black uppercase tracking-widest text-[9px] hover:bg-white hover:text-blue-600 transition-all shadow-lg animate-pulse no-underline">
            <Zap size={12} /> Quiz Conformité
          </Link>
          <Link href="/auth/login" className="px-5 py-2 bg-blue-600/10 border border-blue-500/20 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-blue-600 transition-all no-underline">
            CONNEXION SDE
          </Link>
        </div>
      </nav>
      ...
      <section className="pt-20 pb-12 px-8 lg:px-12 max-w-7xl mx-auto text-left">
        ...
        <div className="flex gap-4">
           <Link href="#trial" className="px-6 py-3 bg-blue-600 rounded-xl font-black uppercase hover:scale-105 transition-all shadow-xl no-underline">J&apos;ESSAYE QS ELITE</Link>
           <Link href="#norms" className="px-6 py-3 bg-white/5 border border-white/10 rounded-xl font-black uppercase hover:bg-white/10 transition-all no-underline">DOCUMENTATION</Link>
        </div>
      </section>

      {/* 🖼️ SHOWCASE */}
      <section className="py-12 px-8 lg:px-12 bg-white/5 border-y border-white/5">
        <div className="max-w-7xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8">
          <ShowcaseItem src="/images/qs_cockpit.jpg" title="Cockpit Opérationnel" icon={Monitor} />
          <ShowcaseItem src="/images/qs_revuedirection.jpg" title="Revue de Direction" icon={Smartphone} active />
          <ShowcaseItem src="/images/qs_qhse.jpg" title="Registre QHSE" icon={Tablet} />
        </div>
      </section>
      ...
      <section className="py-16 px-8 lg:px-12 max-w-7xl mx-auto">
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-5 gap-4">
          ...
          <PlanCard title="Entreprise" price="175.000" desc="XOF / Mois" features={["Risques §6.1", "Veille Légale", "Sites"]} active />
      ...
      <section id="trial" className="py-20 px-8">
        <div className="max-w-3xl mx-auto bg-[#0F172A]/60 border border-white/5 rounded-[2.5rem] p-10 backdrop-blur-3xl shadow-4xl text-left">
           ...
           <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <input required value={formData.company} onChange={e => setFormData({...formData, company: e.target.value})} placeholder="ENTREPRISE" className="bg-white/5 border border-white/10 p-4 rounded-xl uppercase font-black text-white outline-none focus:border-blue-500" />
              <input required value={formData.address} onChange={e => setFormData({...formData, address: e.target.value})} placeholder="ADRESSE" className="bg-white/5 border border-white/10 p-4 rounded-xl uppercase font-black text-white outline-none focus:border-blue-500" />
              <input required value={formData.requesterName} onChange={e => setFormData({...formData, requesterName: e.target.value})} placeholder="DEMANDEUR" className="bg-white/5 border border-white/10 p-4 rounded-xl uppercase font-black text-white outline-none focus:border-blue-500" />
              <input required value={formData.function} onChange={e => setFormData({...formData, function: e.target.value})} placeholder="FONCTION" className="bg-white/5 border border-white/10 p-4 rounded-xl uppercase font-black text-white outline-none focus:border-blue-500" />
              <input required value={formData.landline} onChange={e => setFormData({...formData, landline: e.target.value})} placeholder="TÉL FIXE" className="bg-white/5 border border-white/10 p-4 rounded-xl uppercase font-black text-white outline-none focus:border-blue-500" />
              <input required value={formData.mobile} onChange={e => setFormData({...formData, mobile: e.target.value})} placeholder="MOBILE" className="bg-white/5 border border-white/10 p-4 rounded-xl uppercase font-black text-white outline-none focus:border-blue-500" />
              <input required type="email" value={formData.email} onChange={e => setFormData({...formData, email: e.target.value})} placeholder="EMAIL VÉRIFIABLE" className="md:col-span-2 bg-white/5 border border-white/10 p-4 rounded-xl uppercase font-black text-white outline-none focus:border-blue-500" />
              <button disabled={!isFormValid || isSubmitting} className={`md:col-span-2 py-5 rounded-2xl font-black uppercase tracking-widest transition-all ${isFormValid ? 'bg-blue-600 hover:bg-blue-500 cursor-pointer' : 'bg-white/5 text-slate-700 cursor-not-allowed'}`}>
                {isSubmitting ? <Loader2 className="animate-spin" /> : "ACTIVER MON ESSAI 14 JOURS"}
              </button>
           </form>
        </div>
      </section>
      ...
      <footer className="py-12 border-t border-white/5 bg-[#080B14] text-center">
        <p className="text-[9px] font-black text-slate-600 uppercase tracking-widest italic m-0 leading-relaxed">
...
function ShowcaseItem({ src, title, icon: Icon, active }: any) {
  return (
    <div className="space-y-4">
      <div className={`relative aspect-video rounded-2xl overflow-hidden border ${active ? 'border-2 border-blue-600 shadow-2xl' : 'border-white/10 opacity-80'}`}>
        <Image src={src} alt={title} fill className="object-cover" />
      </div>
      <div className="flex items-center gap-2">
        <Icon size={14} className={active ? 'text-white' : 'text-blue-500'} />
        <p className={`font-black uppercase m-0 ${active ? 'text-white' : 'text-slate-400'}`}>{title}</p>
      </div>
    </div>
  );
}
...
function ISOLink({ icon: Icon, title, href, color }: any) {
  return (
    <Link href={href} className="p-5 bg-white/5 border border-white/5 rounded-xl hover:border-blue-600/30 transition-all flex items-center gap-4 no-underline group text-left">
      ...
      <span className="font-black uppercase tracking-widest text-white text-[9px] group-hover:text-blue-500 transition-colors">{title}</span>
    </Link>
  );
}
