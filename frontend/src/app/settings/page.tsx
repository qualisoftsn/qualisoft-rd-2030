...
"use client";
/**
 * 🛠️ MODULE : PARAMÈTRES SMI & IDENTITÉ ORGANISME
 * -------------------------------------------------------------------------
 * FONCTION : Gestion des métadonnées du Tenant (Organisation).
 * RÔLE : Configuration du contexte de l'organisme (§4.1 ISO 9001).
 * SÉCURITÉ : Accès réservé aux administrateurs master.
 */
...
export default function SettingsPage() {
  ...
  /**
   * 📡 SYNCHRONISATION DES PARAMÈTRES MASTER
   * Récupère l'état actuel de l'instance depuis le microservice admin.
   */
   ...
   * Met à jour le référentiel de l'organisation dans le Cloud Qualisoft.
   */
  const handleSave = async () => {
    ...
    try {
      ...
      toast.success("PROTOCOLE DE MISE À JOUR RÉUSSI");
    } catch (err) {
      toast.error("ERREUR : Échec de l'enregistrement sur le noyau.");
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="flex-1 bg-[#0B0F1A] min-h-screen p-10 ml-72 text-white font-sans italic text-left relative overflow-x-hidden">
      ...
      <header className="flex justify-between items-end mb-12 border-b border-white/5 pb-10">
        <div>
          ...
          <p className="text-slate-500 text-[10px] font-bold uppercase tracking-[0.4em] mt-4">
            ID INSTANCE : {tenant.T_Id} &bull; QUALISOFT ELITE RD 2026
          </p>
        </div>
        <button
          onClick={handleSave}
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-10">
        <div className="xl:col-span-2 space-y-8">
          ...
          {/* SECTION : SÛRETÉ (§9.1 ISO) */}
          <section className="bg-slate-900/40 border border-white/5 p-10 rounded-[3.5rem] shadow-2xl backdrop-blur-xl text-left">
        ...
        <div className="space-y-8">
          <div className="bg-blue-600 p-12 rounded-[4rem] shadow-2xl relative overflow-hidden group text-left">
            ...
            <div className="absolute -bottom-16 -right-16 w-64 h-64 bg-white/10 rounded-full blur-3xl group-hover:scale-125 transition-transform duration-1000" />
          </div>

          <div className="p-10 border border-red-500/20 bg-red-500/5 rounded-[3.5rem] space-y-8 text-left">
