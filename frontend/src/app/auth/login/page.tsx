...
/**
 * 🛰️ LOGIN KERNEL - QUALISOFT ELITE RD 2030
 * Rôle : Identification territoriale et authentification souveraine.
 */
...
function LoginPortal() {
  ...
  // 1. 🔍 DÉTECTION TERRITORIALE (Subdomain Kernel)
  useEffect(() => {
    const identifyNode = async () => {
      const hostname = window.location.hostname;
      const parts = hostname.split('.');
      
      // Cas : sde.qualisoft.sn
      if (parts.length > 2 && !['www', 'api', 'app', 'elite', 'localhost'].includes(parts[0])) {
        ...
        } catch (e) { console.error("Node Identification Error"); }
      } 
  ...
  // 2. 🏗️ CHARGEMENT DU REGISTRE CLIENT (Si mode manuel)
  useEffect(() => {
    if (mode === "LOGIN_FORM" && loginType === "TENANT" && !detectedTenant) {
      matrixApi.getPublicTenants().then(setPublicTenants).catch(() => toast.error("Registre indisponible"));
    }
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) throw new Error("Kernell : Accès révoqué. Identifiants invalides.");

      toast.success(`Accès autorisé. Bienvenue dans la Matrix.`, { id: toastId });
  ...
  if (mode === "LOADING") {
    return (
      <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center space-y-4">
        ...
        <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.4em] animate-pulse italic">Identification du nœud...</p>
      </div>
    );
  }

  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-6 font-sans italic selection:bg-blue-500/30 overflow-hidden relative">
      
      {/* DECOR SOUVERAIN */}
      <div className="absolute inset-0 pointer-events-none opacity-20">
        <Cpu className="absolute -top-20 -left-20 text-slate-800" size={600} />
      ...
      <div className="w-full max-w-md relative z-10 animate-in fade-in zoom-in duration-500">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.5em] mt-3 italic">
            {detectedTenant ? `Nœud : ${detectedTenant.T_Domain}` : "Portail Elite RD 2030"}
          </p>
        </div>
        ...
        {mode === "CHOICE" && (
          <div className="space-y-6 animate-in slide-in-from-bottom-8 duration-500">
            <button onClick={() => { setLoginType("MASTER"); setMode("LOGIN_FORM"); }} className="w-full bg-slate-900 border-2 border-slate-800 p-8 rounded-[3rem] flex items-center justify-between hover:border-blue-600 transition-all cursor-pointer group">
              <div className="text-left"><p className="text-[9px] font-black text-blue-500 uppercase tracking-widest">Protocol</p><p className="text-xl font-black text-white uppercase italic">Master Console</p></div>
            ...
            <button onClick={() => { setLoginType("TENANT"); setMode("LOGIN_FORM"); }} className="w-full bg-white p-8 rounded-[3rem] border-4 border-transparent flex items-center justify-between hover:border-blue-600 transition-all cursor-pointer shadow-xl group">
              <div className="text-left"><p className="text-[9px] font-black text-slate-400 uppercase tracking-widest">Service</p><p className="text-xl font-black text-slate-900 uppercase italic">Portail Client</p></div>
              <Globe className="text-slate-400 group-hover:text-blue-600 group-hover:scale-110 transition-transform" size={28} />
            </button>
          </div>
        )}

        {/* 🔐 MODE : FORMULAIRE SCELLÉ */}
        {mode === "LOGIN_FORM" && (
            ...
            <form onSubmit={handleAuth} className="space-y-7">
              ...
              {!detectedTenant && (
                <button type="button" onClick={() => setMode("CHOICE")} className="flex items-center gap-2 text-[10px] font-black text-blue-500 uppercase hover:text-white transition-colors mb-4 cursor-pointer">
                  <ChevronLeft size={16} /> Changer d&apos;accès
                </button>
              )}
              ...
              <div className="space-y-4">
                {loginType === "TENANT" && (
                  detectedTenant ? (
                    <div className="relative opacity-60">
                      ...
                      <input disabled value={detectedTenant.T_Name} className="w-full pl-14 pr-12 py-5 bg-blue-900/10 border-2 border-blue-900/20 rounded-2xl font-black uppercase text-xs text-blue-400 italic outline-none" />
                ...
                <div className="relative group">
                  ...
                  <input required type="email" placeholder="IDENTIFIANT (EMAIL)" className="w-full pl-14 pr-6 py-5 bg-slate-950 border-2 border-slate-800 rounded-2xl font-black text-white outline-none focus:border-blue-600 transition-all text-sm placeholder:text-slate-800" onChange={(e) => setForm({...form, email: e.target.value})} />
                </div>
              ...
              <button disabled={isLoading} className="w-full py-6 bg-blue-600 text-white rounded-4xl font-black uppercase tracking-[0.3em] shadow-[0_0_30px_rgba(37,99,235,0.3)] hover:bg-blue-500 transition-all flex justify-center items-center gap-4 cursor-pointer border-none active:scale-95 group">
                {isLoading ? <Loader2 className="animate-spin" size={24} /> : (
                  <>SÉCURISER LA CONNEXION <ArrowRight size={20} className="group-hover:translate-x-2 transition-transform" /></>
                )}
              </button>
            </form>
          </div>
        ...
        <footer className="mt-12 text-center text-[10px] font-black text-slate-600 uppercase tracking-[0.5em] italic">
          Qualisoft Elite Sovereign System RD 2030
        </footer>
      </div>
    </div>
  );
...
// 📦 WRAPPER POUR LE KERNEL NEXT.JS (Suspense obligatoire pour searchParams)
