...
function LoginPortal() {
  ...
  // 🛰️ IDENTIFICATION TERRITORIALE (Middleware Sync)
  useEffect(() => {
    const identifyNode = async () => {
      ...
      if (parts.length > 2 && !['www', 'api', 'app', 'elite', 'localhost'].includes(parts[0])) {
        ...
        } catch (e) { console.warn("Node identification fail"); }
      } 
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 italic font-sans relative overflow-hidden">
      <div className="absolute inset-0 opacity-10 pointer-events-none">
        <Cpu className="absolute -top-20 -left-20" size={600} />
        <Fingerprint className="absolute -bottom-40 -right-20" size={500} />
      </div>
      ...
      <div className="w-full max-w-md bg-slate-900/40 border-2 border-slate-800 rounded-[3rem] p-10 shadow-2xl backdrop-blur-xl relative z-10 animate-in zoom-in duration-500">
        ...
        {mode === "CHOICE" ? (
          ...
          <form onSubmit={handleAuth} className="space-y-6">
            {!detectedTenant && <button type="button" onClick={() => setMode("CHOICE")} className="text-[10px] font-black text-blue-500 uppercase flex items-center gap-2 bg-transparent border-none cursor-pointer"><ChevronLeft size={16} /> Revenir</button>}
            ...
            <div className="space-y-4">
              ...
              <div className="relative group">
                ...
                <input required type={showPassword ? "text" : "password"} placeholder="MOT DE PASSE" className="w-full p-5 pl-14 pr-14 bg-slate-950 border-2 border-slate-800 rounded-2xl text-white font-bold outline-none focus:border-blue-600 italic transition-all" onChange={e => setForm({...form, password: e.target.value})} />
