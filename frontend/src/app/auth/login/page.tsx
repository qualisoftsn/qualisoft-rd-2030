 ...
 * FIX ULTIME : Correction du SAS (Sovereign Access Service) pour ne pas crasher
 * sur les domaines publics (www, app) et éviter les redirections mortelles.
 * FIX FETCH : Remplacement total d'Axios par Native Fetch pour le GET et le POST.
 * DESIGN : ClickUp High-Density, Split-Screen, Zero-Scroll, PWA Ready.
 * SÉCURITÉ : Zustand + HttpOnly.
 * RÉVISION : 09 Mars 2026 | 18:25 GMT
 * -------------------------------------------------------------------------
 */
...
function LoginFormContent() {
  ...
  const fetchAllTenants = async (slug: string) => {
    try {
      const response = await fetch('https://api.qualisoft.sn/api/tenants/public/list', {
        ...
        // Ajout de cache: no-store pour éviter les vieux résultats
        cache: 'no-store' 
      });
      ...
      if (list.length === 0) {
        toast.warning("Le serveur OVH a répondu, mais la liste des locataires est vide.");
      }

      setTenantList(list);

      // Si le slug de l'URL correspond à un tenant, on le présélectionne
      if (slug) {
  ...
  /**
   * 🧭 AIGUILLAGE DU SAS (SOVEREIGN ACCESS SERVICE)
   */
  const initSAS = useCallback(async () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (masterNodes.includes(slug)) {
        ...
        // Sinon, c'est le login d'un client spécifique (ex: sde.qualisoft.sn/auth/login)
        setLoginType('TENANT');
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 👑 BYPASS MASTER ARCHITECTE (Seule faille volontaire)
...
// -------------------------------------------------------------------------------------------------
// PAGE PRINCIPALE : LAYOUT DU TERMINAL LOGIN
// -------------------------------------------------------------------------------------------------
    ...
    <div className="fixed inset-0 w-full h-dvh bg-[#0B0F1A] flex flex-col lg:flex-row overflow-hidden italic font-sans select-none">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-[#050810] relative flex-col justify-between p-16 xl:p-24 border-r border-white/5 overflow-hidden shrink-0">
        ...
        <div className="relative z-10 space-y-12">
          <h2 className="text-4xl xl:text-4xl font-black text-white uppercase italic tracking-tighter leading-[0.85] m-0">
            ELITE <br/><span className="text-blue-500 underline decoration-8 underline-offset-16">QHSE</span>
          </h2>
      ...
      {/* 🔐 MOITIÉ DROITE : LE TERMINAL DE CONNEXION */}
      <div className="flex-1 flex flex-col justify-center p-6 sm:p-12 md:p-16 lg:p-24 bg-[#0B0F1A] relative h-full overflow-y-auto custom-scrollbar">
        <Suspense fallback={<LoadingMatrix label="Séquençage du Tunnel..." />}>
          <LoginFormContent />
        </Suspense>
        
        {/* LOGO DE FOND SUBLIMINAL */}
      ...
      {/* 🧪 PWA & OVERRIDES (Désactive les comportements de swipe mobile natifs) */}
      <style jsx global>{`
        body { 
...
// -------------------------------------------------------------------------------------------------
// SOUS-COMPOSANTS ATOMIQUES (INPUTS & LOADER)
// -------------------------------------------------------------------------------------------------
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="relative group">
        <Icon className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-600 group-focus-within:text-blue-500 transition-colors pointer-events-none" size={18} />
        <input 
          type={type} 
          required 
          placeholder={placeholder} 
          className="w-full bg-black/40 border border-white/10 rounded-2xl py-5 pl-16 pr-14 text-white font-black italic text-xs outline-none focus:border-blue-600 transition-all placeholder:text-slate-800 shadow-inner"
          value={value} 
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => onChange(e.target.value)} 
        />
        {showPasswordToggle && (
          <button type="button" onClick={onTogglePassword} className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white transition-colors bg-transparent border-none cursor-pointer outline-none p-2 rounded-lg hover:bg-white/5">
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full gap-8">
      <div className="relative flex items-center justify-center">
        <div className="absolute inset-0 border-4 border-blue-600/20 border-t-blue-600 rounded-full w-24 h-24 animate-spin -m-2" />
        <Loader2 className="animate-spin text-blue-500" size={60} strokeWidth={1.5} />
        <Zap className="absolute text-blue-400/20" size={24} />
      </div>
