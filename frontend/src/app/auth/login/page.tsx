...
function LoginPortal() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) throw new Error("Accès refusé. Vérifiez vos clés.");
      
      // ⚡ FORCE REDIRECT : Purge session OVH
  ...
  if (mode === "LOADING") return <div className="min-h-screen bg-slate-950 flex items-center justify-center italic text-slate-500 text-[10px] font-black uppercase"><Loader2 className="animate-spin mr-3" /> Node Sync...</div>;
    ...
    <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 italic font-sans selection:bg-blue-500/30 relative overflow-hidden">
      ...
      <div className="w-full max-w-md bg-slate-900/40 border-2 border-slate-800 rounded-[3rem] p-10 shadow-2xl backdrop-blur-xl relative z-10">
        <div className="text-center mb-10">
          <ShieldCheck className={loginType === "MASTER" ? "text-blue-500" : "text-emerald-500"} size={48} className="mx-auto mb-4" />
        ...
        {mode === "CHOICE" ? (
          <div className="space-y-4">
            <button onClick={() => { setLoginType("MASTER"); setMode("LOGIN_FORM"); }} className="w-full bg-slate-900 p-8 rounded-3xl border-2 border-slate-800 flex justify-between items-center hover:border-blue-600 cursor-pointer group shadow-xl">
            ...
            <button onClick={() => { setLoginType("TENANT"); setMode("LOGIN_FORM"); }} className="w-full bg-white p-8 rounded-3xl border-none flex justify-between items-center hover:scale-105 transition-all cursor-pointer group shadow-xl">
          ...
          <form onSubmit={handleAuth} className="space-y-6">
            ...
            <button disabled={isLoading} className="w-full py-6 bg-blue-600 text-white rounded-4xl font-black uppercase tracking-[0.3em] hover:bg-blue-500 transition-all border-none cursor-pointer shadow-xl flex justify-center items-center gap-4">{isLoading ? <Loader2 className="animate-spin" /> : <>DÉVERROUILLER L&apos;ACCÈS <ArrowRight /></>}</button>
          </form>
        )}
      </div>
    </div>
