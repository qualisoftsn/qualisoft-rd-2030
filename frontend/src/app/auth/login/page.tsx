...
interface LoginProps {
  tenantSlug?: string;
  isMaster?: boolean; // ✅ Ajouté pour correspondre au RootPage
}
...
function LoginFormContent({ tenantSlug, isMaster }: LoginProps) {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 👑 BYPASS MASTER ROOT (Abdoulaye Thiongane) - CONSERVÉ
      if (payload.email === 'ab.thiongane@qualisoft.sn' && payload.password === 'Qualisoft@2026') {
          const masterUser = { U_Id: "ROOT_MASTER", U_Email: payload.email, U_Role: "SUPER_ADMIN", tenantId: "MATRIX_CORE", U_TenantName: "QUALISOFT MASTER CONSOLE", U_FirstName: "Abdoulaye", U_LastName: "Thiongane" };
      ...
      toast.error(err.response?.data?.message || 'Code d’accès invalide', { id: tid });
    } finally { setIsLoading(false); }
  };

  if (mode === 'LOADING') return <div className="flex flex-col items-center justify-center p-20"><Loader2 className="animate-spin text-blue-600 h-12 w-12 mb-4" /><p className="text-[10px] font-black text-blue-500 tracking-widest animate-pulse italic">Identification du nœud...</p></div>;
  ...
  return (
    <div className="w-full max-w-lg bg-slate-900/40 border border-white/5 rounded-[3rem] p-12 shadow-2xl backdrop-blur-3xl relative">
      <div className="text-center mb-10">
        <div className="mb-6 flex justify-center"><Image src="/images/qslogo.png" alt="Qualisoft Logo" width={200} height={64} className="h-16 w-auto object-contain" /></div>
        <h1 className="text-3xl font-black text-white uppercase tracking-tighter italic">{detectedTenant ? detectedTenant.T_Name : 'QUALISOFT ELITE'}</h1>
        <p className="text-slate-500 text-[9px] font-black uppercase tracking-[0.4em] mt-3">{detectedTenant ? `Instance : ${detectedTenant.T_Domain}.sn` : 'Souveraineté Numérique RD 2030'}</p>
      </div>
      ...
      {mode === 'CHOICE' ? (
        <div className="space-y-6">
          <button onClick={() => { setLoginType('MASTER'); setMode('LOGIN_FORM'); }} className="w-full bg-white/5 p-8 rounded-4xl border border-white/10 flex justify-between items-center hover:bg-indigo-500/10 cursor-pointer group">
            <div className="text-left"><p className="text-[10px] text-indigo-400 font-black uppercase tracking-[0.3em] mb-1">Système</p><p className="text-xl text-white font-black uppercase">Console Matrix</p></div>
          ...
          <button onClick={() => { setLoginType('TENANT'); setMode('LOGIN_FORM'); }} className="w-full bg-blue-600 p-8 rounded-4xl flex justify-between items-center hover:bg-blue-500 cursor-pointer shadow-lg">
            <div className="text-left"><p className="text-[10px] text-blue-200 font-black uppercase mb-1">Organisation</p><p className="text-xl text-white font-black uppercase">Portail Elite</p></div>
        ...
        <form onSubmit={handleAuth} className="space-y-8">
          <div className="space-y-6">
            <div className="relative"><Mail className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500" size={20} /><input required type="email" placeholder="COURRIEL" className="w-full pl-16 pr-6 py-6 bg-white/5 border border-white/10 rounded-3xl text-white outline-none focus:border-blue-500 font-bold text-sm" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} /></div>
            <div className="relative"><Lock className="absolute left-6 top-1/2 -translate-y-1/2 text-slate-500" size={20} /><input required type={showPassword ? 'text' : 'password'} placeholder="CODE D'ACCÈS" className="w-full pl-16 pr-16 py-6 bg-white/5 border border-white/10 rounded-3xl text-white outline-none focus:border-blue-500 font-bold text-sm" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-6 top-1/2 -translate-y-1/2 text-slate-500 hover:text-white cursor-pointer">{showPassword ? <EyeOff size={20} /> : <Eye size={20} />}</button></div>
          </div>
          <div className="flex flex-col gap-6">
            <button disabled={isLoading} type="submit" className={`w-full py-6 text-white rounded-4xl font-black uppercase text-xs tracking-widest flex justify-center items-center gap-4 cursor-pointer transition-all ${loginType === 'MASTER' ? 'bg-indigo-600 hover:bg-indigo-500' : 'bg-blue-600 hover:bg-blue-500'}`}>{isLoading ? <Loader2 className="animate-spin" /> : <><ShieldCheck size={20} /> AUTORISER L&apos;ACCÈS</>}</button>
            <button type="button" onClick={() => setMode('CHOICE')} className="w-full text-[9px] font-black text-slate-600 uppercase tracking-widest hover:text-white">← Changer de portail d&apos;accès</button>
          </div>
        </form>
      )}
    </div>
...
export default function LoginPage({ tenantSlug, isMaster }: LoginProps) {
  return (
    <div className="min-h-screen bg-[#0B0F1A] flex items-center justify-center p-4 italic font-sans relative overflow-hidden">
      ...
      <Suspense fallback={<div className="flex flex-col items-center"><Loader2 className="animate-spin text-blue-600 h-12 w-12" /></div>}><LoginFormContent tenantSlug={tenantSlug} isMaster={isMaster} /></Suspense>
    </div>
  );
}