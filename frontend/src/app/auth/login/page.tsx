...
import { ChevronLeft, Loader2, ShieldCheck, Terminal, Globe, Mail, Eye, EyeOff, ArrowRight, Building2, Cpu, Fingerprint, Lock } from "lucide-react";
...
function LoginPortal() {
    ...
    const identifyNode = async () => {
      ...
      if (parts.length > 2 && !['www', 'api', 'app', 'elite', 'localhost'].includes(parts[0])) {
        try {
          ...
          if (tenant) { setDetectedTenant(tenant); setForm(f => ({ ...f, tenantId: tenant.T_Id })); setLoginType("TENANT"); setMode("LOGIN_FORM"); return; }
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      const result = await signIn("credentials", { email: form.email.toLowerCase().trim(), password: form.password, tenantId: form.tenantId, redirect: false });
      if (result?.error) throw new Error("Accès refusé.");
      window.location.href = loginType === "MASTER" ? "/admin/matrix" : "/dashboard";
    } catch (err: any) { toast.error(err.message, { id: tid }); setIsLoading(false); }
  };

  if (mode === "LOADING") return <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center italic text-slate-500 text-[10px] font-black uppercase"><Loader2 className="animate-spin mb-4" /> Identification...</div>;
      ...
      <div className="w-full max-w-md bg-slate-900/40 border-2 border-slate-800 rounded-[3rem] p-10 shadow-2xl backdrop-blur-xl relative z-10 animate-in zoom-in duration-500">
        <div className="text-center mb-10">
          <div className="inline-flex p-5 bg-slate-900 border-2 border-slate-800 rounded-[2.5rem] mb-6 shadow-2xl">
            {/* ✅ FIX : className unique fusionnée */}
            <ShieldCheck size={48} className={`mx-auto ${loginType === "MASTER" ? "text-blue-500" : "text-emerald-500"}`} />
          </div>
          <h1 className="text-4xl font-black text-white uppercase italic tracking-tighter leading-none">{detectedTenant ? detectedTenant.T_Name : "QUALI"}<span className="text-blue-600">{detectedTenant ? "" : "SOFT"}</span></h1>
        </div>
        ...
        {mode === "CHOICE" ? (
          <div className="space-y-4">
            <button onClick={() => { setLoginType("MASTER"); setMode("LOGIN_FORM"); }} className="w-full bg-slate-900 p-8 rounded-3xl border-2 border-slate-800 flex justify-between items-center hover:border-blue-600 cursor-pointer shadow-xl transition-all">
              <div className="text-left font-black uppercase"><p className="text-[9px] text-blue-500">Master Access</p><p className="text-xl text-white italic">Master Admin</p></div><Terminal className="text-blue-500" />
            </button>
            <button onClick={() => { setLoginType("TENANT"); setMode("LOGIN_FORM"); }} className="w-full bg-white p-8 rounded-3xl border-none flex justify-between items-center hover:scale-[1.02] cursor-pointer shadow-xl transition-all">
              <div className="text-left font-black uppercase"><p className="text-[9px] text-slate-400">Portal Access</p><p className="text-xl text-slate-900 italic">Client Access</p></div><Globe className="text-slate-400" />
            </button>
          </div>
        ) : (
          <form onSubmit={handleAuth} className="space-y-6">
            ...
            <div className="space-y-4">
              {loginType === "TENANT" && (
                ...
                : <select required className="w-full p-5 bg-slate-950 border-2 border-slate-800 rounded-2xl text-white font-bold italic outline-none cursor-pointer" onChange={e => setForm({...form, tenantId: e.target.value})}>
                    <option value="">Sélectionner Organisation</option>
                    {publicTenants.map(t => <option key={t.T_Id} value={t.T_Id}>{t.T_Name}</option>)}
                  </select>
              )}
              <div className="relative"><Mail className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-600" size={18} /><input required type="email" placeholder="EMAIL PROFESSIONNEL" className="w-full p-5 pl-12 bg-slate-950 border-2 border-slate-800 rounded-2xl text-white font-bold outline-none focus:border-blue-600 italic" onChange={e => setForm({...form, email: e.target.value})} /></div>
              <div className="relative"><Lock className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-600" size={18} /><input required type={showPassword ? "text" : "password"} placeholder="MOT DE PASSE" className="w-full p-5 pl-12 pr-14 bg-slate-950 border-2 border-slate-800 rounded-2xl text-white font-bold outline-none focus:border-blue-600 italic" onChange={e => setForm({...form, password: e.target.value})} /><button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-5 top-1/2 -translate-y-1/2 bg-transparent border-none text-slate-600 cursor-pointer">{showPassword ? <EyeOff size={20}/> : <Eye size={20}/>}</button></div>
            </div>
