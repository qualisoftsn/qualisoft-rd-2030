 ...
 * DESIGN : ClickUp Elite (Zero Global Scroll, Edge-to-Edge, PWA Ready).
 * FIX : Optimisation de l'occupation spatiale (suppression du max-w-400 invalide).
  ...
  return (
    <div className="h-dvh w-screen bg-[#050810] flex italic font-sans overflow-hidden selection:bg-blue-600/30 relative text-white">
      ...
      <div className="flex-1 flex flex-col min-w-0 relative h-full lg:pr-24">
        ...
        <header className="h-20 md:h-24 bg-[#0B0F1A]/60 backdrop-blur-3xl border-b border-white/5 flex items-center justify-between px-6 md:px-10 shrink-0 z-50">
          <div className="relative w-full max-w-xl group hidden md:block">
            ...
            <input
              ...
              className="w-full pl-16 pr-8 py-3.5 bg-black/40 border border-white/5 rounded-2xl text-[10px] font-black text-white outline-none focus:border-blue-600/50 transition-all placeholder:text-slate-700 tracking-widest italic shadow-inner"
            />
          </div>

          <div className="flex items-center gap-6 md:gap-8 ml-auto">
            {isSuperAdmin && (
              <div className="hidden sm:flex items-center gap-2 px-4 py-2 bg-amber-500/5 border border-amber-500/20 rounded-xl text-amber-500 text-[8px] font-black tracking-widest uppercase italic shadow-lg">
        ...
        {/* 📄 VIEWPORT : ZONE DE SCROLL ISOLÉE (Pleine largeur) */}
        <main className="flex-1 relative overflow-hidden bg-[#050810]">
          <div className="h-full w-full overflow-y-auto custom-scrollbar flex flex-col">
            {/* Remplacement du max-w-400 par un max-w-[1800px] pour occupation bord-à-bord sur grand écran */}
            <div className="w-full max-w-450 mx-auto p-6 md:p-8 lg:p-10 animate-in fade-in slide-in-from-bottom-4 duration-700 flex-1 flex flex-col">
              
              {/* TÉLÉMÉTRIE DE SÉCURITÉ */}
              <div className="mb-8 flex items-center justify-between opacity-30 hover:opacity-100 transition-opacity">
                <div className="flex items-center gap-3">
                  <Activity size={12} className="text-blue-500" />
                  <p className="text-[8px] font-black uppercase tracking-[0.4em] m-0">
                    NODE: {user?.U_TenantName || "SDE"}.CORE // ID: {user?.U_Id?.slice(0,8) || "N/A"}
                  </p>
                </div>
      ...
      <nav className="hidden lg:flex w-24 h-screen bg-[#0B0F1A] border-l border-white/5 flex-col items-center py-10 gap-10 fixed right-0 top-0 z-60 shadow-4xl">
        <div className="w-12 h-12 bg-blue-600/10 rounded-2xl flex items-center justify-center mb-2 shadow-inner">
           <Cpu className="text-blue-500" size={24} />
        </div>

        <SlimLink href="/dashboard/menu" icon={LayoutGrid} active={pathname === "/dashboard/menu"} label="Modules" />
        ...
        <button
          ...
          className="w-14 h-14 rounded-3xl bg-rose-600/5 border border-transparent text-rose-600 flex items-center justify-center hover:bg-rose-600/10 hover:border-rose-500/30 hover:text-rose-400 transition-all cursor-pointer group"
...
function SlimLink({ href, icon: Icon, active, label }: any) {
  return (
    <Link
      href={href}
      className={cn(
        "w-14 h-14 rounded-2xl flex items-center justify-center transition-all group relative border",
        active 
    ...
    >
      ...
      <div className="absolute right-20 bg-black border border-white/10 px-4 py-2 rounded-xl opacity-0 group-hover:opacity-100 transition-all pointer-events-none whitespace-nowrap text-[9px] font-black uppercase tracking-widest text-blue-400 shadow-2xl translate-x-4 group-hover:translate-x-0 z-50">
        {label}
      </div>
    </Link>
  );
  ...
  return (
    <div className="flex items-center gap-5 border-l border-white/5 pl-8">
      <div className="text-right leading-none hidden xl:block space-y-2">
        <p className="text-[12px] font-black text-white m-0 tracking-tighter uppercase italic truncate max-w-37.5">
          {user?.U_FirstName} {user?.U_LastName}
        </p>
        <p className={cn("text-[8px] m-0 font-black uppercase tracking-[0.3em] truncate max-w-37.5", isSuperAdmin ? "text-amber-500" : "text-blue-500")}>
          {isSuperAdmin ? "Matrix Architect" : user?.U_Role || "Opérateur"}
        </p>
      </div>
      <div className={cn(
        "w-12 h-12 md:w-14 md:h-14 rounded-2xl flex items-center justify-center border border-white/10 text-white shadow-2xl transform hover:scale-105 hover:rotate-3 transition-all cursor-pointer shrink-0",
        isSuperAdmin ? "bg-amber-600" : "bg-blue-600"
      )}>
        <span className="text-sm font-black not-italic tracking-tighter">
          {user?.U_FirstName?.[0] || ""}{user?.U_LastName?.[0] || "U"}
        </span>
      </div>
    </div>
  );
