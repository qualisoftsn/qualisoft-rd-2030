 ...
 * VERSION : 2.0 - Typing strict + CRUD activé + Design Elite + Accessibilité
 * API : apiClient Axios avec interceptors (Bearer + X-Tenant-Id)
 * RÉVISION : 19 Mars 2026 | 14:30 GMT
 * -------------------------------------------------------------------------
 */
...
const getDocTypeBadge = (type: DocumentGED['DOC_Type']): { label: string; color: string } => {
...
function DocumentCard({ 
  ...
  return (
    <button
      ...
      className={cn(
        "w-full p-4 text-left bg-[#0B0F1A] border rounded-2xl transition-all focus:outline-none focus:ring-2 focus:ring-blue-400",
        isSelected 
      ...
      <div className="flex items-start justify-between gap-3">
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 mb-2">
            <FileText size={14} className="text-blue-500 shrink-0" aria-hidden="true" />
        ...
        {isSelected && (
          <CheckCircle2 size={16} className="text-blue-500 shrink-0" aria-hidden="true" />
        )}
      </div>
...
function PreuveCard({ 
  ...
  return (
    <article className="p-5 bg-[#0F172A] rounded-2xl md:rounded-3xl border border-white/5 hover:border-blue-500/30 transition-all flex flex-col gap-4">
      {/* Header */}
      <div className="flex items-start justify-between gap-4">
        ...
        <button
          ...
          className="p-2 text-slate-500 hover:text-rose-500 hover:bg-rose-500/10 rounded-lg transition-all border-none cursor-pointer focus:outline-none focus:ring-2 focus:ring-rose-400"
      ...
      {preuve.PV_Commentaire && (
        <div className="p-4 bg-[#0B0F1A] rounded-xl border border-white/5 text-[10px] text-slate-400 leading-relaxed italic">
          <MessageSquare size={14} className="text-blue-500 mb-2 inline-block" aria-hidden="true" />
      ...
      <div className="flex items-center gap-3 pt-2 border-t border-white/5">
        {doc?.DOC_Url && (
          ...
          >
            <ExternalLink size={14} aria-hidden="true" /> Consulter l'original
          </a>
        )}
        <span className={cn(
...
export default function AuditPreuvesPage() {
  ...
  const handleLink = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const message = apiError?.message || error?.message || "Erreur de liaison";
  ...
  const handleUnlink = async (preuveId: string) => {
    ...
    try {
      ...
      const message = error?.response?.data?.message || "Erreur de détachement";
  ...
  if (loading) {
    return (
      <div className="h-full flex items-center justify-center bg-[#0B0F1A] text-blue-400 font-black italic uppercase gap-4" role="status" aria-live="polite">
  ...
  return (
    <div className="h-full flex flex-col bg-[#0B0F1A] italic font-sans overflow-hidden text-white w-full selection:bg-blue-600/30">
      ...
      {/* HEADER */}
      <header className="shrink-0 p-4 md:p-6 border-b border-white/5 bg-[#0B0F1A]/95 backdrop-blur-md flex items-center gap-4 md:gap-6">
        <button 
          onClick={() => router.back()} 
        ...
        <div className="min-w-0">
          ...
          <p className="text-[9px] md:text-[10px] text-slate-500 font-bold uppercase tracking-[0.3em] mt-1 m-0 truncate">
      ...
      <div className="flex-1 flex flex-col xl:flex-row overflow-hidden">
        ...
        <aside className="w-full xl:w-80 lg:w-96 p-4 md:p-6 bg-[#0F172A]/50 border-r border-white/5 overflow-y-auto custom-scrollbar shrink-0">
          <h2 className="text-lg font-black uppercase italic mb-4 md:mb-6 flex items-center gap-3 m-0 text-white">
            <UploadCloud className="text-blue-500" size={20} md:size={24} aria-hidden="true" /> 
            Lier un Document
          </h2>
          ...
          <form onSubmit={handleLink} className="space-y-4 md:space-y-5 mb-6 md:mb-8" noValidate>
            ...
            <div className="relative">
              <Search size={14} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 pointer-events-none" aria-hidden="true" />
              <input
                type="search"
            ...
            <div className="space-y-2 max-h-64 overflow-y-auto custom-scrollbar pr-2">
              {filteredDocuments.length > 0 ? (
                ...
                <p className="text-center text-[9px] text-slate-500 py-4 italic">
            ...
            {formErrors.documentId && (
              <p className="text-rose-400 text-[9px] flex items-center gap-1" role="alert">
                <AlertCircle size={10} aria-hidden="true" /> {formErrors.documentId}
              </p>
            )}
            ...
            <div className="space-y-2">
              ...
              {formErrors.comment && (
                <p className="text-rose-400 text-[9px] flex items-center gap-1" role="alert">
                  <AlertCircle size={10} aria-hidden="true" /> {formErrors.comment}
                </p>
              )}
            </div>
            ...
            <button 
              ...
              className={cn(
                "w-full py-4 bg-blue-600 hover:bg-white hover:text-blue-900 text-white rounded-2xl font-black uppercase italic text-xs shadow-xl transition-all border-none cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-3 focus:outline-none focus:ring-2 focus:ring-blue-400",
                isLinking && "cursor-wait"
              )}
              aria-busy={isLinking}
            >
              {isLinking ? (
                <><Loader2 size={16} className="animate-spin" aria-hidden="true" /> Liaison...</>
              ) : (
                <><LinkIcon size={16} aria-hidden="true" /> Lier à l'Audit</>
              )}
            </button>
          </form>
          
          {/* Info sécurité */}
          <div className="p-4 bg-blue-500/5 border border-blue-500/20 rounded-2xl text-[8px] text-slate-400 uppercase tracking-widest italic">
            <ShieldCheck size={12} className="inline mr-2 text-blue-500" aria-hidden="true" />
        ...
        <main className="flex-1 overflow-y-auto custom-scrollbar p-4 md:p-6 lg:p-8">
          <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl md:text-2xl font-black uppercase italic flex items-center gap-3 m-0 text-white">
              <FileText className="text-blue-500" size={24} aria-hidden="true" /> 
              Éléments Probants 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            {preuves.length > 0 ? (
              ...
              <div className="col-span-full h-40 md:h-48 border-2 border-dashed border-white/10 rounded-3xl flex flex-col items-center justify-center text-slate-500" role="status">
