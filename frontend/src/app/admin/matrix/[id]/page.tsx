...
import { ArrowLeft, Key, Loader2, Edit3, Globe, Users, ShieldCheck, UserCheck, RefreshCw } from "lucide-react";
...
export default function TenantCockpit() {
  ...
  const handleImpersonate = async () => {
    const tid = toast.loading("Ouverture Tunnel...");
    try {
    ...
    } catch (err) { toast.error("ÉCHEC TUNNEL", { id: tid }); }
  };
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      const payload: any = { firstName: editingUser.U_FirstName || editingUser.firstName, lastName: editingUser.U_LastName || editingUser.lastName, email: editingUser.U_Email || editingUser.email, role: editingUser.U_Role || editingUser.role };
      if (editingUser.U_Id) await matrixApi.updateUser(editingUser.U_Id, payload);
      else { payload.tenantId = tenantId; await matrixApi.createGlobalUser(payload); }
      setEditingUser(null); fetchTenantDetails(); toast.success("SCELLÉ", { id: tid });
    } catch (err: any) { toast.error("REJET MATRIX", { id: tid }); }
  };

  if (loading) return <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center italic text-slate-500 text-[10px] font-black uppercase"><Loader2 className="animate-spin mb-4" /> Node identification...</div>;
      ...
      <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 bg-slate-900 rounded-[3rem] p-12 shadow-2xl border-2 border-slate-800 relative overflow-hidden">
            ...
            <button onClick={handleImpersonate} className="mt-12 bg-blue-600 text-white py-7 rounded-2xl font-black uppercase text-xs tracking-[0.3em] hover:bg-white hover:text-slate-900 transition-all border-none cursor-pointer shadow-xl">Activer Tunnel d&apos;Incarnation</button>
          </div>
        ...
        <div className="bg-slate-900/40 rounded-[3rem] shadow-2xl border-2 border-slate-800 overflow-hidden backdrop-blur-sm">
          ...
          <table className="w-full text-left"><thead><tr className="bg-slate-950/60 text-slate-500 font-black uppercase text-[10px] tracking-widest border-b border-slate-800"><th className="px-12 py-8">Identité</th><th className="px-12 py-8 text-center">Rôle</th><th className="px-12 py-8 text-right">Actions</th></tr></thead><tbody>
              {tenant.T_Users?.map((user: any) => (<tr key={user.U_Id} className="hover:bg-blue-600/5 transition-all"><td className="px-12 py-8 font-black uppercase text-white italic text-sm">{user.U_Email}<span className="block text-[10px] text-slate-500">{user.U_FirstName} {user.U_LastName}</span></td><td className="px-12 py-8 text-center"><span className="bg-slate-950 border border-slate-800 px-4 py-2 rounded-xl text-[9px] font-black text-slate-300">{user.U_Role}</span></td><td className="px-12 py-8 text-right"><button onClick={() => setEditingUser(user)} className="p-3 bg-slate-800 rounded-xl text-slate-500 hover:text-white border-none cursor-pointer"><Edit3 size={16} /></button></td></tr>))}
          </tbody></table>
        </div>
      </div>
    </div>
