  ...
  const handleImpersonate = async () => {
    ...
    try {
      ...
      if (data?.access_token && data?.targetUser) {
        ...
        const targetSlug =
          data.targetUser.tenant?.T_Domain?.split(".")[0].toLowerCase() ||
          "app";
  ...
  const handleSaveUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingUser.U_Id) {
        ...
        await matrixApi.createGlobalUser({
          ...payload,
          password: "Qualisoft@2030",
        });
      }
      ...
      <div className="max-w-7xl mx-auto space-y-8 animate-in fade-in duration-700">
        <div className="flex justify-between items-center">
          <button
            ...
            className="flex items-center gap-3 text-slate-500 hover:text-white font-black uppercase text-[11px] bg-transparent border-none cursor-pointer group tracking-widest"
          >
            <ArrowLeft size={16} /> Retour au Registre
          </button>
          <button
            onClick={fetchTenantDetails}
            className="p-3 bg-slate-900 border border-slate-800 rounded-xl text-slate-500 hover:text-blue-500 cursor-pointer"
          >
            <RefreshCw size={16} />
          </button>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="relative z-10">
              ...
              <div className="grid grid-cols-2 gap-10 mt-12 text-slate-400 uppercase text-[11px] font-black italic">
                <div className="flex items-center gap-4">
                  <Globe className="text-blue-500" /> {tenant?.T_Domain}
                </div>
                <div className="flex items-center gap-4">
                  <UserCheck className="text-amber-500" /> Leader :{" "}
                  {tenant?.T_CeoName || "Inconnu"}
                </div>
              </div>
            </div>
            <button
              onClick={handleImpersonate}
              className="mt-12 w-full bg-blue-600 text-white py-7 rounded-2xl font-black uppercase text-xs tracking-[0.3em] hover:bg-white hover:text-slate-900 transition-all border-none cursor-pointer shadow-xl active:scale-95"
            >
          ...
          <div className="bg-white rounded-[3rem] p-12 shadow-2xl flex flex-col justify-center items-center gap-4 text-slate-900 border-8 border-slate-900">
            ...
            <p className="text-9xl font-black italic tracking-tighter leading-none">
              {tenant?._count?.T_Users || 0}
            </p>
          </div>
        </div>
        ...
        <div className="bg-slate-900/40 rounded-[3rem] shadow-2xl border-2 border-slate-800 overflow-hidden backdrop-blur-sm">
          <div className="p-10 border-b border-slate-800 bg-slate-950/50 flex justify-between items-center">
            <h3 className="font-black uppercase text-xs tracking-widest text-slate-500 italic flex items-center gap-4">
              <Users size={22} className="text-blue-600" /> Registre
              d&apos;Identité
            </h3>
            <button
              onClick={() =>
              ...
              className="bg-white text-slate-900 px-8 py-4 rounded-xl font-black uppercase text-[10px] border-none cursor-pointer hover:bg-blue-600 hover:text-white transition-all shadow-xl"
            >
              Enrôler Citoyen
            </button>
          </div>
          <table className="w-full text-left">
            ...
            <tbody>
              {tenant?.T_Users?.map((user: any) => (
                <tr
                  key={user.U_Id}
                  className="hover:bg-blue-600/5 transition-all border-b border-slate-800/30"
                >
                  <td className="px-12 py-8">
                    <p className="font-black uppercase text-white italic text-sm">
                      {user.U_Email}
                    </p>
                  ...
                  <td className="px-12 py-8 text-center">
                    <span
                      className={`px-4 py-2 rounded-xl text-[9px] font-black uppercase border ${user.U_Role === "ADMIN" ? "border-amber-500 text-amber-500 bg-amber-500/5" : "border-slate-700 text-slate-400 bg-slate-950"}`}
                    >
                      {user.U_Role}
                    </span>
                  </td>
                  <td className="px-12 py-8 text-right">
                    <button
                      ...
                      className="p-3 bg-slate-800 rounded-xl text-slate-500 hover:text-white border-none cursor-pointer transition-colors"
                    >
                      <Edit3 size={16} />
                    </button>
                  </td>
      ...
      {editingUser && (
        <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-xl z-50 flex items-center justify-center p-6">
          <form
            ...
            className="bg-slate-900 border-2 border-slate-800 w-full max-w-2xl rounded-[3rem] p-12 space-y-8 shadow-2xl animate-in zoom-in duration-300"
          >
            <div className="flex justify-between items-center border-b border-slate-800 pb-6">
              ...
              <button
                ...
                className="p-2 text-slate-500 hover:text-white bg-transparent border-none cursor-pointer transition-colors"
              >
                <X size={24} />
              </button>
            </div>
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase text-slate-500 ml-4 italic">
                  Prénom
                </label>
                <input
                  required
                  className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl text-white font-black italic outline-none focus:border-blue-600 transition-all"
              ...
              <div className="col-span-2 space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditingUser({ ...editingUser, U_Email: e.target.value })
                  }
                />
              </div>
              <div className="space-y-2">
                ...
                <select
                  className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl text-white font-black italic outline-none focus:border-blue-600 appearance-none transition-all cursor-pointer"
                  value={editingUser.U_Role}
                  onChange={(e) =>
                    setEditingUser({ ...editingUser, U_Role: e.target.value })
                  }
                >
              ...
              <div className="space-y-2">
                ...
                <select
                  className="w-full bg-slate-950 border border-slate-800 p-4 rounded-xl text-white font-black italic outline-none focus:border-blue-600 appearance-none transition-all cursor-pointer"
                  value={editingUser.U_IsActive ? "true" : "false"}
                  onChange={(e) =>
                    setEditingUser({
                      ...editingUser,
                      U_IsActive: e.target.value === "true",
                    })
                  }
                >
            ...
            <button
              type="submit"
              className="w-full py-6 bg-white text-slate-900 rounded-2xl font-black uppercase text-xs tracking-[0.3em] flex items-center justify-center gap-4 hover:bg-blue-600 hover:text-white transition-all border-none cursor-pointer shadow-xl"
            >
              <Save size={18} /> Valider les Modifications
            </button>
          </form>
        </div>
      )}
