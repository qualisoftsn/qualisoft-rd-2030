...
import { Send, Building2, Mail, MessageSquare, Loader2, CheckCircle } from 'lucide-react';
...
export default function ContactForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        setStatus("Requête transmise au Kernel. Nos analystes vous contacteront.");
        form.reset();
      } else {
  ...
  return (
    <div className="p-10 bg-white rounded-[3rem] shadow-2xl border border-slate-100 italic text-left relative overflow-hidden">
      ...
      <h2 className="text-3xl font-black mb-8 uppercase italic tracking-tighter text-slate-950 leading-none">
        Demander une démo <br/><span className="text-blue-600">Qualisoft Elite</span>
      </h2>
      ...
      <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
        <div className="space-y-2">
          ...
          <input type="text" name="company" required placeholder="EX: GLOBAL INDUSTRIES SA"
        ...
        <div className="space-y-2">
          ...
          <input type="email" name="email" required placeholder="directeur@entreprise.sn"
        ...
        <div className="space-y-2">
          <label className="text-[10px] font-black uppercase text-slate-400 ml-4 tracking-widest italic flex items-center gap-2">
            <MessageSquare size={12} /> Votre besoin stratégique
          </label>
          <textarea name="message" rows={4} placeholder="DÉTAILLEZ VOTRE PROJET ISO..."
                    className="w-full bg-slate-50 border-2 border-slate-100 p-5 rounded-3xl font-bold outline-none focus:border-blue-600 transition-all italic"></textarea>
        </div>
        ...
        {status && (
          <div className="mt-6 p-4 bg-blue-50 border border-blue-100 rounded-2xl flex items-center gap-3 animate-in fade-in zoom-in">
            ...
            <p className="text-[11px] font-black text-blue-700 uppercase tracking-tight italic">{status}</p>
          </div>
        )}
      </form>
    </div>
