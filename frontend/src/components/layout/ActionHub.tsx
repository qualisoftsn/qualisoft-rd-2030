...
export default function ActionHub() {
  ...
  const context = useMemo(() => {
    const map: Record<string, { title: string; icon: any; color: string; action?: string }> = {
    ...
    return map[pathname] || { title: 'Qualisoft Elite', icon: Zap, color: 'text-slate-400' };
  }, [pathname]);

  const Icon = context.icon;

  return (
    <div className="w-full h-24 border-b border-white/5 bg-[#0B0F1A]/60 backdrop-blur-2xl flex items-center justify-between px-6 md:px-12 shrink-0 animate-in fade-in slide-in-from-top-2 duration-500">
      
      {/* 🧭 SECTION GAUCHE : BREADCRUMBS & TITRE DYNAMIQUE */}
      <div className="flex flex-col gap-1.5 min-w-0">
        <div className="hidden sm:flex items-center gap-3 text-[8px] font-black uppercase tracking-[0.4em] text-slate-600 italic">
        ...
        <div className="flex items-center gap-4 min-w-0">
          <div className={cn("hidden sm:flex p-2 rounded-lg bg-white/5 border border-white/5 shrink-0", context.color)}>
            <Icon size={20} strokeWidth={2.5} />
          </div>
          <h2 className="text-lg md:text-3xl font-black uppercase italic tracking-tighter text-white m-0 truncate leading-none">
            {context.title}
          </h2>
        </div>
      </div>
      ...
      <div className="flex items-center gap-4">
        <div className="hidden lg:flex items-center gap-3 border-r border-white/5 pr-6 mr-2">
        ...
        {context.action && (
          <button className="bg-blue-600 text-white px-5 md:px-10 py-3.5 md:py-4 rounded-2xl font-black uppercase text-[9px] md:text-[10px] tracking-widest hover:bg-blue-500 transition-all shadow-xl shadow-blue-900/20 flex items-center gap-3 border-none cursor-pointer active:scale-95 group">
            <Plus size={18} strokeWidth={3} className="group-hover:rotate-90 transition-transform duration-500 shrink-0" />
            <span className="hidden sm:inline">{context.action}</span>
          </button>
        )}

        {/* INDICATEUR DE SÉCURITÉ */}
        <div className="hidden md:flex w-12 h-12 rounded-2xl bg-emerald-500/5 border border-emerald-500/10 items-center justify-center text-emerald-500 shadow-inner group cursor-help transition-all hover:bg-emerald-500/10 shrink-0" title="Session Sécurisée TLS 1.3">
