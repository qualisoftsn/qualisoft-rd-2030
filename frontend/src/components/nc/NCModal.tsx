...
import { AlertTriangle, Link2, Loader2, ShieldAlert, X } from "lucide-react";
      ...
      <div className="bg-white w-full max-w-xl rounded-[3rem] overflow-hidden shadow-4xl animate-in zoom-in duration-300 border border-slate-100">
          ...
          <div className="flex items-center gap-4 text-left">
            ...
            <div>
              <h2 className="text-2xl font-black uppercase italic text-slate-900 tracking-tighter leading-none">
                Déclarer un <span className="text-red-600">Écart</span>
              </h2>
              <p className="text-[9px] font-black text-red-600/60 uppercase tracking-widest mt-2 italic">
                Signalement d&apos;Anomalie Matrix
              </p>
            </div>
          </div>
          <button
            onClick={onClose}
            className="p-2 text-slate-400 hover:text-red-600 transition-all border-none bg-transparent cursor-pointer"
          >
            <X size={28} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8 text-left">
          <div className="space-y-2">
            <label className="text-[10px] font-black uppercase text-slate-400 block ml-2 italic tracking-widest">
              Intitulé du constat
            </label>
            <input
              required
              className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-5 text-sm font-bold text-slate-900 outline-none focus:border-red-500 focus:bg-white transition-all uppercase"
              ...
              placeholder="EX: ABSENCE DE MARQUAGE AU SOL - ZONE B"
            />
          </div>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <select
                className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-5 text-xs font-black outline-none appearance-none cursor-pointer uppercase italic focus:border-red-500"
                value={formData.NC_Gravite}
                onChange={(e) =>
                  setFormData({ ...formData, NC_Gravite: e.target.value })
                }
              >
            ...
            <div className="space-y-2">
              ...
              <select
                ...
                onChange={(e) =>
                  setFormData({ ...formData, NC_Source: e.target.value })
                }
              >
          ...
          <div className="space-y-2">
            ...
            <select
              className={`w-full bg-slate-50 border-2 rounded-2xl p-5 text-xs font-black outline-none appearance-none cursor-pointer uppercase italic transition-all ${formData.NC_AuditId ? "border-blue-500 bg-blue-50/30" : "border-slate-100"}`}
              value={formData.NC_AuditId}
              onChange={(e) =>
                setFormData({ ...formData, NC_AuditId: e.target.value })
              }
            >
              <option value="">-- Aucun lien audit détecté --</option>
              {audits.map((a) => (
                <option key={a.AU_Id} value={a.AU_Id}>
          ...
          <div className="space-y-2">
            ...
            <textarea
              required
              className="w-full bg-slate-50 border-2 border-slate-100 rounded-4xl p-6 text-sm font-bold text-slate-700 outline-none focus:border-red-500 focus:bg-white transition-all min-h-32 italic leading-relaxed"
              value={formData.NC_Description}
              onChange={(e) =>
                setFormData({ ...formData, NC_Description: e.target.value })
              }
              placeholder="Détaillez les preuves objectives de l'écart..."
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-slate-950 py-6 rounded-3xl font-black uppercase italic text-white flex items-center justify-center gap-4 hover:bg-red-600 transition-all shadow-3xl shadow-red-100 active:scale-95 border-none cursor-pointer mt-4"
          >
            {loading ? (
