...
import { Calendar, Loader2, Save, ShieldAlert, X } from "lucide-react";
      ...
      <div className="bg-white w-full max-w-2xl rounded-[3.5rem] shadow-4xl overflow-hidden text-left italic">
        ...
        <div className="p-10 border-b border-slate-50 flex justify-between items-center bg-blue-50/40">
          <div>
            ...
            <p className="text-[10px] font-bold text-blue-600 uppercase tracking-widest mt-1">
              Initialisation du flux d&apos;amélioration
            </p>
          </div>
          <button
            onClick={onClose}
            className="p-3 bg-white rounded-2xl text-slate-400 hover:text-red-500 hover:rotate-90 transition-all border-none cursor-pointer"
          >
            <X size={24} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-6">
          <div className="space-y-2">
            ...
            <input
              required
              placeholder="Titre explicite de l'action corrective..."
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <select
                required
                className="w-full bg-slate-50 border-none rounded-2xl p-6 text-[11px] font-black uppercase italic outline-none cursor-pointer shadow-inner"
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-slate-500 ml-4 tracking-widest flex items-center gap-2">
                <Calendar size={12} /> Échéance Requise
              </label>
              <input
                type="date"
                required
                className="w-full bg-slate-50 border-none rounded-2xl p-6 text-[11px] font-black italic outline-none shadow-inner"
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-slate-500 ml-4 tracking-widest flex items-center gap-2">
                <ShieldAlert size={12} /> Niveau de Priorité
              </label>
              <select
                className="w-full bg-slate-50 border-none rounded-2xl p-6 text-[11px] font-black uppercase italic outline-none cursor-pointer shadow-inner"
                value={formData.ACT_Priority}
                onChange={(e) =>
                  setFormData({ ...formData, ACT_Priority: e.target.value })
                }
              >
          ...
          <div className="space-y-2">
            ...
            <textarea
              ...
              className="w-full bg-slate-50 border-none rounded-3xl p-6 text-xs font-bold min-h-32 italic outline-none focus:ring-2 focus:ring-blue-600 shadow-inner"
              value={formData.ACT_Description}
              onChange={(e) =>
                setFormData({ ...formData, ACT_Description: e.target.value })
              }
            />
          </div>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-blue-600 hover:bg-blue-500 py-6 rounded-3xl font-black uppercase italic text-white flex items-center justify-center gap-4 shadow-2xl transition-all active:scale-95 disabled:opacity-50 border-none cursor-pointer"
          >
            {loading ? (
