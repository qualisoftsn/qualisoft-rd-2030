...
export default function MatrixUserModal({ isOpen, onClose, onSuccess, userToEdit }: any) {
  ...
  useEffect(() => {
    if (isOpen) {
      setErrorMsg(null);
      matrixApi.getTenants().then(setTenants).catch(() => toast.error("Sync Tenants Error"));
      if (userToEdit) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const msg = err.response ? decodeError(err) : (err.message || "Erreur Kernel");
      setErrorMsg(msg); toast.error("ÉCHEC", { id: tid });
  ...
  return (
    <div className="fixed inset-0 z-200 flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-4 italic">
      <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl border-4 border-slate-900 overflow-hidden flex flex-col max-h-[95vh] font-sans italic">
        <div className="p-8 bg-slate-50 border-b-2 border-slate-100 flex justify-between items-center shrink-0"><h2 className="text-3xl font-black text-slate-900 uppercase italic tracking-tighter">{userToEdit ? '📦 Rectifier Profil' : '🚀 Enrôler Agent'}</h2><button onClick={onClose} className="p-3 bg-white border border-slate-200 rounded-full hover:bg-red-50 hover:text-red-500 cursor-pointer border-none"><X size={24}/></button></div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-6 overflow-y-auto flex-1 pb-10">
          ...
          <button disabled={loading} className="w-full bg-slate-900 text-white py-6 rounded-4xl font-black uppercase text-xs tracking-widest hover:bg-blue-600 transition-all border-none cursor-pointer shadow-xl active:scale-95 flex justify-center items-center gap-4 group">
            {loading ? <Loader2 className="animate-spin" /> : <Save size={20} className="group-hover:rotate-12 transition-transform" />} SCELLER LE PROFIL
          </button>
        </form>
      </div>
    </div>
