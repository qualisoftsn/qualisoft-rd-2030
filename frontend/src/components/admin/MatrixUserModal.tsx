...
/**
 * 🚀 REGISTRE DES CITOYENS - QUALISOFT ELITE
 */
    ...
    <div className="fixed inset-0 z-200 flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-4 italic">
      <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl border-4 border-slate-900 overflow-hidden flex flex-col max-h-[95vh] font-sans">
        <div className="p-8 bg-slate-50 border-b-2 border-slate-100 flex justify-between items-center shrink-0">
          <h2 className="text-3xl font-black text-slate-900 uppercase italic tracking-tighter">{userToEdit ? '📦 Rectifier Profil' : '🚀 Enrôler Citoyen'}</h2>
        ...
        {errorMsg && (
          <div className="mx-8 mt-6 p-4 bg-red-50 border-l-4 border-red-600 flex items-start gap-3 animate-in fade-in">
            ...
            <p className="text-[10px] font-black text-red-800 uppercase italic">{errorMsg}</p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-6 overflow-y-auto flex-1 pb-10">
          ...
          <div className="relative">
            ...
            <input required type="email" placeholder="Email" className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl py-5 pl-14 pr-5 font-bold italic outline-none focus:border-blue-600" value={form.email} onChange={e => setForm({...form, email: e.target.value})} />
          </div>

          {!userToEdit && (
            <select 
              required 
              className="w-full bg-slate-100 border-2 border-slate-200 rounded-2xl p-5 font-black uppercase text-xs italic outline-none cursor-pointer" 
              value={form.tenantId} 
