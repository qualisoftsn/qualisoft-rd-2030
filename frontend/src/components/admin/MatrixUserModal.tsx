...
import { AlertCircle, Loader2, Mail, Save, ShieldCheck, X } from "lucide-react";
...
const PRISMA_ROLES = [
  {
    value: "SUPER_ADMIN",
    label: "👑 SUPER ADMIN",
    desc: "Contrôle total du Kernel Matrix",
  },
  {
    value: "ADMIN",
    label: "🏢 ADMIN TENANT",
    desc: "Administration locale de l'organisation",
  },
  {
    value: "RQ",
    label: "⭐ RESP. QUALITÉ (RQ)",
    desc: "Pilotage SMI et Conformité",
  },
  {
    value: "PILOTE",
    label: "✈️ PILOTE DE PROCESSUS",
    desc: "Gestion opérationnelle",
  },
  {
    value: "DIRECTION",
    label: "👔 DIRECTION",
    desc: "Consultation et Rapports",
  },
  {
    value: "HSE",
    label: "⛑️ HSE MANAGER",
    ...
    <div className="fixed inset-0 z-200 flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-4 italic">
      <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl border-4 border-slate-900 overflow-hidden flex flex-col max-h-[95vh] font-sans">
        <div className="p-8 bg-slate-50 border-b-2 border-slate-100 flex justify-between items-center shrink-0">
          ...
          <button
            onClick={onClose}
            className="p-3 bg-white border border-slate-200 rounded-full hover:bg-red-50 hover:text-red-500 transition-all border-none cursor-pointer"
          >
            <X size={24} />
          </button>
        </div>
        ...
        {errorMsg && (
          <div className="mx-8 mt-6 p-4 bg-red-50 border-l-4 border-red-600 flex items-start gap-3 animate-in fade-in">
            ...
            <p className="text-[10px] font-black text-red-800 uppercase italic text-left">
              {errorMsg}
            </p>
          </div>
        )}
        ...
        <form
          ...
          className="p-10 space-y-6 overflow-y-auto flex-1 pb-10 text-left"
        >
          <div className="grid grid-cols-2 gap-4">
            <input
              required
              placeholder="Prénom"
              className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-5 font-bold italic outline-none focus:border-blue-600"
              value={form.firstName}
              onChange={(e) =>
                setForm({ ...form, firstName: e.target.value.toUpperCase() })
              }
            />
            <input
              required
              ...
              value={form.lastName}
              onChange={(e) =>
                setForm({ ...form, lastName: e.target.value.toUpperCase() })
              }
            />
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl py-5 pl-14 pr-5 font-bold italic outline-none focus:border-blue-600"
              value={form.email}
          ...
          <div className="space-y-4">
            <p className="text-[9px] font-black text-slate-400 uppercase tracking-widest italic ml-4">
              Niveau d&apos;accréditation Matrix
            </p>
            <div className="grid grid-cols-2 gap-4">
              {PRISMA_ROLES.map((r) => (
                <label
                  key={r.value}
                  className={`p-4 rounded-3xl border-2 cursor-pointer transition-all ${form.role === r.value ? "border-blue-600 bg-blue-50 shadow-inner" : "border-slate-100 hover:border-slate-300"}`}
                >
                  <input
                    type="radio"
                    className="hidden"
                    ...
                    onChange={() =>
                      setForm({ ...form, role: r.value as MatrixRole })
                    }
                  />
          ...
          <button
            disabled={loading}
            className="w-full bg-slate-900 text-white py-6 rounded-4xl font-black uppercase text-xs tracking-widest hover:bg-blue-600 transition-all border-none cursor-pointer shadow-xl active:scale-95 flex justify-center items-center gap-4 group italic"
          >
            {loading ? (
