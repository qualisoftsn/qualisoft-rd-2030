...
export default function MatrixUserModal({ isOpen, onClose, onSuccess, userToEdit }: any) {
  ...
  const decodeError = (err: any): string => {
    ...
    return "Violation du protocole Matrix.";
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (userToEdit) {
        ...
        if (!form.tenantId) throw new Error("Ancrage requis.");
    ...
    <div className="fixed inset-0 z-200 flex items-center justify-center bg-slate-950/90 backdrop-blur-md p-4 italic">
      <div className="bg-white rounded-[3rem] w-full max-w-2xl shadow-2xl border-4 border-slate-900 overflow-hidden flex flex-col max-h-[95vh] font-sans">
        <div className="p-8 bg-slate-50 border-b-2 border-slate-100 flex justify-between items-center shrink-0">
          ...
          <button onClick={onClose} className="p-3 bg-white border border-slate-200 rounded-full hover:bg-red-50 hover:text-red-500 transition-all cursor-pointer"><X size={24}/></button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-6 overflow-y-auto flex-1">
          <div className="grid grid-cols-2 gap-4">
            <input required placeholder="Prénom" className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-5 font-bold italic outline-none focus:border-blue-600" value={form.firstName} onChange={e => setForm({...form, firstName: e.target.value})} />
            <input required placeholder="Nom" className="w-full bg-slate-50 border-2 border-slate-100 rounded-2xl p-5 font-bold italic outline-none focus:border-blue-600" value={form.lastName} onChange={e => setForm({...form, lastName: e.target.value})} />
          </div>
