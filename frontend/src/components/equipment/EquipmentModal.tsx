...
export default function EquipmentModal({
  ...
  useEffect(() => {
    if (equipment) {
      setFormData({
        ...
        EQ_DateService: equipment.EQ_DateService
          ? new Date(equipment.EQ_DateService).toISOString().split("T")[0]
          : "",
        EQ_ProchaineVGP: equipment.EQ_ProchaineVGP
          ? new Date(equipment.EQ_ProchaineVGP).toISOString().split("T")[0]
          : "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.error(err.response?.data?.message || "Rejet de l'enregistrement", {
        id: tid,
      });
    } finally {
      setLoading(false);
    ...
    <div className="fixed inset-0 bg-slate-950/90 backdrop-blur-md z-100 flex items-center justify-center p-4 italic">
      <div className="bg-[#0F172A] border border-white/10 w-full max-w-xl rounded-[2.5rem] overflow-hidden shadow-2xl animate-in zoom-in duration-300 font-sans">
        ...
        <div className="p-8 border-b border-white/5 flex justify-between items-center bg-white/5">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-2xl font-black uppercase italic text-white tracking-tighter leading-none">
                {equipment ? "Rectifier" : "Nouvel"}{" "}
                <span className="text-blue-500">Matériel</span>
              </h2>
              <p className="text-[9px] font-black uppercase text-slate-500 tracking-widest mt-2">
          ...
          <button
            onClick={onClose}
            className="p-2 hover:bg-white/10 rounded-full text-slate-500 hover:text-white transition-all border-none cursor-pointer"
          >
            <X size={24} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="p-10 space-y-8 text-left">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase text-slate-500 block ml-2 tracking-widest">
                Référence / SN
              </label>
              <input
                required
                className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm text-white font-bold outline-none focus:border-blue-500 transition-all uppercase"
                value={formData.EQ_Reference}
                onChange={(e) =>
                  setFormData({
                    ...formData,
                    EQ_Reference: e.target.value.toUpperCase(),
                  })
                }
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, EQ_Name: e.target.value })
                }
                placeholder="EX: CHARIOT ÉLÉVATEUR TOYOTA"
              />
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-2">
              ...
              <input
                type="date"
                required
                className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm text-white font-bold outline-none focus:border-blue-500"
                value={formData.EQ_DateService}
                onChange={(e) =>
                  setFormData({ ...formData, EQ_DateService: e.target.value })
                }
              />
            </div>
            <div className="space-y-2">
              ...
              <input
                type="date"
                required
                className="w-full bg-white/5 border border-orange-500/30 rounded-2xl p-4 text-sm text-orange-500 font-bold outline-none focus:border-orange-500"
                value={formData.EQ_ProchaineVGP}
                onChange={(e) =>
                  setFormData({ ...formData, EQ_ProchaineVGP: e.target.value })
                }
              />
            </div>
          </div>
          ...
          <div className="space-y-2">
            ...
            <select
              className="w-full bg-white/5 border border-white/10 rounded-2xl p-4 text-sm text-white font-black outline-none appearance-none cursor-pointer uppercase italic"
              value={formData.EQ_Status}
              onChange={(e) =>
                setFormData({ ...formData, EQ_Status: e.target.value })
              }
            >
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full bg-blue-600 py-6 rounded-3xl font-black uppercase italic text-white flex items-center justify-center gap-4 hover:bg-blue-500 transition-all shadow-xl shadow-blue-900/30 active:scale-95 border-none cursor-pointer"
          >
            {loading ? (
