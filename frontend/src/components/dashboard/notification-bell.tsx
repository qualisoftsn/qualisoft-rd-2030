...
import { Activity, Bell, BellRing, Info, ShieldAlert } from "lucide-react";
        ...
        <>
          ...
          <div className="absolute right-0 mt-6 w-96 bg-[#0F172A] border border-white/10 rounded-[2.5rem] shadow-2xl z-110 overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-white/5 bg-white/5 flex justify-between items-center">
              ...
              <span className="text-[8px] text-slate-500 font-bold uppercase tracking-widest animate-pulse italic">
                Temps Réel Actif
              </span>
            </div>
            ...
            <div className="max-h-87.5 overflow-y-auto custom-scrollbar">
              {notifications.length > 0 ? (
                notifications.map((n, i) => (
                  <div
                    key={i}
                    className={`p-5 border-b border-white/5 hover:bg-white/5 transition-colors flex gap-4 ${!n.N_IsRead ? "bg-blue-600/5" : ""}`}
                  >
                    <div className="mt-1">
                      {n.N_Type === "CRITICAL" ? (
                    ...
                    <div className="space-y-1 text-left">
                      <p className="text-[10px] font-black text-white m-0 italic uppercase">
                        {n.N_Title || n.title}
                      </p>
                      <p className="text-[9px] text-slate-500 font-bold m-0 leading-relaxed">
                        {n.N_Message || n.message}
                      </p>
                    </div>
                  </div>
                ))
