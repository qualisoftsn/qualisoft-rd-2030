...
const LandingPage: React.FC = ({ navigation }: any) => {
  ...
  const handleVerifyDomain = async () => {
    if (!domain.trim()) {
      setError("Veuillez saisir le domaine de notre entreprise.");
      return;
    }
    ...
    try {
      // Appel vers notre Backend NestJS basé sur le schéma Prisma (T_Domain)
      // Remplacez par notre instance axios configurée
      const response = await fetch(
        `https://api.qualisoft.sn/v1/tenants/verify/${domain.trim().toLowerCase()}`,
      );
      ...
      if (data.isValid) {
        ...
        setError(
          "Domaine introuvable ou compte inactif. Contactez notre administrateur.",
        );
      }
    } catch (err) {
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.tagline}>
            L'Excellence Opérationnelle QSE & RSE
          </Text>
        </View>
        ...
        <View style={styles.card}>
          ...
          <Text style={styles.cardSubtitle}>
            Saisissez le nom de domaine fourni lors de notre souscription
            Qualisoft.
          </Text>
        ...
        <View style={styles.footer}>
          <Text style={styles.footerText}>
            Plateforme certifiée conforme zone UEMOA
          </Text>
