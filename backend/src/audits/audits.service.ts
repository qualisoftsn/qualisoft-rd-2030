/**
 * 🛰️ MODULE : AuditsService
 * -------------------------------------------------------------------------
 * RÔLE : Logique transactionnelle d'audit et liaison automatique CAPA.
...
export class AuditsService {
  ...
  async create(data: any, tenantId: string) {
    return this.prisma.audit.create({
      data: {
        ...
        AU_Scope: data.AU_Scope || "Périmètre SMI Global",
   ...
   * Scelle l'ordre de mission par la signature du pilote.
   */
  async signAcceptance(auditId: string, userId: string, tenantId: string, hash: string) {
    ...
    if (!audit || audit.tenantId !== tenantId) throw new NotFoundException("Audit inexistant.");
    
    // Vérification (§5.3 ISO 9001)
    const isPilote = audit.AU_Processus?.PR_PiloteId === userId || audit.AU_Processus?.PR_CoPiloteId === userId;
    if (!isPilote) throw new ForbiddenException("Signature réservée à l'autorité du processus.");

    return this.prisma.$transaction(async (tx) => {
      await tx.signature.create({
        data: { SIG_EntityType: 'AUDIT', SIG_EntityId: auditId, SIG_UserId: userId, tenantId, SIG_Hash: hash }
      });
      return tx.audit.update({ where: { AU_Id: auditId }, data: { AU_Status: AuditStatus.EN_COURS } });
    });
  }

  /**
   * 🏁 CLÔTURE ET LIAISON CAPA AUTOMATIQUE
   * Enregistre les constats, crée les NC et génère les actions dans le PAQ.
   */
  async closeAuditWithReport(auditId: string, reportData: any, tenantId: string, auditorId: string) {
    return this.prisma.$transaction(async (tx) => {
      ...
      // 2. Traitement des NC et Actions (§10.2)
      for (const nc of (reportData.nonConformites || [])) {
        ...
        // Liaison PAQ Automatique
        const paq = await tx.pAQ.findFirst({ 
          where: { PAQ_ProcessusId: audit.AU_ProcessusId as string, PAQ_Year: new Date().getFullYear(), tenantId } 
        });
        ...
        if (paq) {
          await tx.action.create({
            data: {
              ...
              ACT_Origin: ActionOrigin.AUDIT, ACT_Type: ActionType.CORRECTIVE, 
              ACT_Status: ActionStatus.A_FAIRE, ACT_PAQId: paq.PAQ_Id, 
              ACT_AuditId: auditId, ACT_NCId: createdNc.NC_Id,
              tenantId, ACT_ResponsableId: audit.AU_LeadId || auditorId, ACT_CreatorId: auditorId,
              ACT_Deadline: new Date(new Date().setMonth(new Date().getMonth() + 3)) // Echéance +3 mois par défaut
            }
          });
        }
      }
