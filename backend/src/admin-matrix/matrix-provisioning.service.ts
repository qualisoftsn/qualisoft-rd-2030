...
export class MatrixProvisioningService {
  ...
  async initializeNewTenant(payload: ProvisioningDto) {
    ...
    // 1. Calcul du domaine souverain (ex: corporate.qualisoft.sn)
    const domain = `${customSlug.toLowerCase().trim()}.qualisoft.sn`;

    // 2. Vérification d'unicité (Domaine et Email)
    ...
    try {
      return await this.prisma.$transaction(async (tx) => {
        ...
        // A. Création du Nœud (Tenant) - Plan Entreprise par défaut pour RD-2030
        ...
        // B. Création de l'Ancrage Territorial (Site)
        ...
        // C. Injection de la Structure Organisationnelle (ISO 9001)
        ...
        // D. Création de la Cellule Mère (Direction Générale)
        ...
        // E. Enrôlement de l'Autorité Racine (Admin) avec Mot de passe personnalisé
